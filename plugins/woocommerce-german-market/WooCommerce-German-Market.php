...
class Woocommerce_German_Market {
	...
	public function general_init() {
		...
		*/
		 ...
		 * Payment settings
		 */
		WGM_Payment_Settings::get_instance();
	...
	public function frontend_init() {
		...
		**/
		...
		add_action( 'wp', 														array( 'WGM_Template', 'hide_flat_rate_shipping_when_free_is_available_wc_9_9' ) );
		...
		*/
		...
		add_filter( 'the_content', array( 'WGM_Template', 'remove_os_platform_from_imprint' ) );
	...
	/**
	 * Make sure German Market payment gateways are registered before other plugins
	 * request the WooCommerce gateway list (e.g. Mollie).
	 *
	 * @since 3.52.0.1-Support
	 * @return void
	 */
	public static function ensure_payment_gateways_registered() {
		...
		if ( class_exists( 'WGM_Purchase_On_Demand' ) ) {
			WGM_Purchase_On_Demand::get_instance();
		}

		if ( class_exists( 'WGM_Sepa_Direct_Debit' ) ) {
			WGM_Sepa_Direct_Debit::get_instance();
		}
	...
	public static function enqueue_admin_scripts() {
		...
		if ( ! $do_not_load_gm_js ) {
			...
			if ( $always_load_gm_js || in_array( $screen_id, wc_get_screen_ids() ) ) {
				...
				wp_register_script( 'woocommerce_de_admin', plugins_url( '/js/WooCommerce-German-Market-Admin.' . $min . 'js', self::$plugin_base_name ), array( 'jquery', 'woocommerce_admin' ), Woocommerce_German_Market::$version );
				...
				wp_localize_script( 'woocommerce_de_admin', 'german_market_backend_translations', array(
						...
						'delete_template_question' => esc_js( __( 'Are you sure you want to delete this template file?', 'woocommerce-german-market' ) ),
				));
	...
	*/
	public static function enqueue_frontend_scripts() {
		...
		wp_localize_script( 'woocommerce_de_frontend', 'german_market_price_variable_products', array( 'val' => WGM_Customizable_Output_Product_Data::get_price_presentation_variable_products() ) );
...
if ( class_exists( 'Woocommerce_German_Market' ) ) {
	...
	add_action( 'plugins_loaded', array( 'Woocommerce_German_Market', 'ensure_payment_gateways_registered' ), 12 );
