jQuery( document ).ready( function( $ ) { 
    ...
    woocommerce_de = {
        ...
        init: function () {
        	...
        	this.customizable_product_data();
            this.refund_button_actions();
            ...
            this.special_tax_output();
            this.email_attachments_allmails();
            this.variable_price_prefix();
            this.manual_order_confirmation_payments();
            this.gpsr();
            this.charging_device();

            jQuery( '#german_market_charging_device_activation' ).change( function() {
            	woocommerce_de.charging_device();
            });

            jQuery( '#german_market_gpsr_automatic_output_manufacturer_and_responsible_person, #german_market_gpsr_automatic_output_warnings_and_safety_information' ).change( function() {
            	woocommerce_de.gpsr();
            });
            ...
            jQuery( '#german_market_special_tax_output_number' ).change( function() {
                woocommerce_de.special_tax_output();
            });
            ...
            jQuery( '#german_market_variable_price_range_from' ).change( function() {
                woocommerce_de.variable_price_prefix();
            });
            ...
            if ( jQuery( '.german-market-lexoffice-connection-exists' ).length ) {

            	jQuery( '#woocommerce_de_lexoffice_authorization_code' ).ready( function() {
            		jQuery( '#woocommerce_de_lexoffice_authorization_code' ).parent().parent().hide();
            	});
            	
            } else if ( jQuery( '.german-market-lexoffice-missing' ).length ) {

            	jQuery( '#woocommerce_de_lexoffice_revoke' ).ready( function() {
            		jQuery( '#woocommerce_de_lexoffice_revoke' ).parent().parent().parent().parent().hide();
            	});

            }
            if ( jQuery( '#woocommerce_de_lexoffice_voucher_or_invoice' ).length ) {
            	jQuery( '#woocommerce_de_lexoffice_voucher_or_invoice' ).change( function() {
            		let option_value_interface = jQuery( '#woocommerce_de_lexoffice_voucher_or_invoice' ).val();

            		if ( 'voucher' === option_value_interface ) {

            			jQuery( '.german-market-lexoffice-missing.invoice' ).hide();
            			jQuery( '.german-market-lexoffice-missing.voucher' ).show();

            		} else {

            			jQuery( '.german-market-lexoffice-missing.invoice' ).show();
            			jQuery( '.german-market-lexoffice-missing.voucher' ).hide();
            		}
            ...
            // refunds select all
            jQuery( '.gm-select-all-refunds' ).click( function() {
		    	var checked = jQuery( this ).is( ":checked" );
		    	jQuery( '.gm-select-refund, .gm-select-all-refunds' ).prop( 'checked', checked ); 
		    });
        },

        legacy_inline_js: function() {

        	jQuery( 'a.german-market-delete-email-template-button' ).click(function(){
        		console.log( 'test' );
				var answer = confirm( german_market_backend_translations.delete_template_question );
				if (answer)
					return true;
				return false;
			});
        ...
        charging_device: function() {

        	if ( jQuery( '#german_market_charging_device_activation' ).length ) {

        		let activation = jQuery( '#german_market_charging_device_activation' ).is( ":checked" );

        		if ( ! activation ) {

        			jQuery( '#german_market_charging_device_activation_loop' ).parent().parent().parent().parent().hide();
        			jQuery( '#german_market_charging_device_activation_single' ).parent().parent().parent().parent().hide();
        			jQuery( '#german_market_charging_device_size' ).parent().parent().hide();
        			jQuery( '#german_market_charging_device_unit' ).parent().parent().hide();

        		} else {

        			jQuery( '#german_market_charging_device_activation_loop' ).parent().parent().parent().parent().show();
        			jQuery( '#german_market_charging_device_activation_single' ).parent().parent().parent().parent().show();
        			jQuery( '#german_market_charging_device_size' ).parent().parent().show();
        			jQuery( '#german_market_charging_device_unit' ).parent().parent().show();
        		}
        ...
        gpsr: function() {
        	...
        	if ( jQuery( '#german_market_gpsr_automatic_output_manufacturer_and_responsible_person' ).length ) {

        		let option_value_person = jQuery( '#german_market_gpsr_automatic_output_manufacturer_and_responsible_person' ).val();
        		let option_value_warning= jQuery( '#german_market_gpsr_automatic_output_warnings_and_safety_information' ).val();

        		if ( 'tab' === option_value_person || 'tab' === option_value_warning ) {
        			jQuery( '#german_market_gpsr_tab_name' ).parent().parent().show();
        		} else {
        			jQuery( '#german_market_gpsr_tab_name' ).parent().parent().hide();
        		}

        		if ( 'summary' === option_value_person ) {
        			jQuery( '#german_market_gpsr_automatic_output_manufacturer_and_responsible_person_prio' ).parent().parent().show();
        		} else {
        			jQuery( '#german_market_gpsr_automatic_output_manufacturer_and_responsible_person_prio' ).parent().parent().hide();
        		}

        		if ( 'summary' === option_value_warning ) {
        			jQuery( '#german_market_gpsr_automatic_output_warnings_and_safety_information_prio' ).parent().parent().show();
        		} else {
        			jQuery( '#german_market_gpsr_automatic_output_warnings_and_safety_information_prio' ).parent().parent().hide();
        		}
        	}
        },

        customizable_product_data: function() {

        	jQuery( '#german-market-sortable-product-data-loop .switch .slider' ).click( function() {
				jQuery( this ).toggleClass( 'on' );
				woocommerce_de.sort_product_data( 'loop' );
			});

			jQuery( '#german-market-sortable-product-data-single .switch .slider' ).click( function() {
				jQuery( this ).toggleClass( 'on' );
				woocommerce_de.sort_product_data( 'single' );
			});

			jQuery( '#german-market-sortable-product-data-loop' ).sortable({
				axis: "y",
			...
			jQuery( '#german-market-sortable-product-data-single' ).sortable({
				axis: "y",
				update: function( event, ui ) {
					woocommerce_de.sort_product_data( 'single' );
				},
			...
			jQuery( '#german_market_customizable_product_data_customized' ).ready( function(){
				woocommerce_de.customizable_product_data_show_and_hide();
			});

			jQuery( '#german_market_customizable_product_data_customized' ).change( function() {
			    woocommerce_de.customizable_product_data_show_and_hide();
			});
            
        },

        customizable_product_data_show_and_hide: function() {

        	let option_value = jQuery( '#german_market_customizable_product_data_customized' ).is( ':checked' );
        	if ( ! option_value ) {

        		jQuery( '#sortable-data-loop-description' ).hide();
        		jQuery( '#sortable-data-loop-description' ).prev( 'h2' ).hide();
        		jQuery( '#sortable-data-loop-description' ).next( 'table' ).hide();

        		jQuery( '#sortable-data-single-description' ).hide();
        		jQuery( '#sortable-data-single-description' ).prev( 'h2' ).hide();
        		jQuery( '#sortable-data-single-description' ).next( 'table' ).hide();

        	} else {

        		jQuery( '#sortable-data-loop-description' ).show();
        		jQuery( '#sortable-data-loop-description' ).prev( 'h2' ).show();
        		jQuery( '#sortable-data-loop-description' ).next( 'table' ).show();

        		jQuery( '#sortable-data-single-description' ).show();
        		jQuery( '#sortable-data-single-description' ).prev( 'h2' ).show();
        		jQuery( '#sortable-data-single-description' ).next( 'table' ).show();

        	}

        },

        sort_product_data: function( loop_or_single ) {

			let sorted_elements = jQuery( "#german-market-sortable-product-data-" + loop_or_single ).children();
	      	let sorted_values = {};

	      	jQuery.each( sorted_elements, function( index, element ){
	      		let value = jQuery( element ).attr( 'data-element-key' );
	      		let slider_element = jQuery( '.' + value + '_activation_' + loop_or_single + ' .slider' );
	      		sorted_values[ value ] = jQuery( slider_element ).hasClass( 'on' ) ? 'on' : 'off';
	      	});

	      	let new_value = JSON.stringify( sorted_values );

	      	jQuery( '#german_market_customizable_product_data_' + loop_or_single ).val( new_value );

		},

        refund_button_actions: function() {

            jQuery( '#woocommerce-order-items' ).on( 'click', 'button.refund-items', function() {

                jQuery( 
                    '#order_shipping_line_items td.line_tax .view,' + 
                    '#order_shipping_line_items td.line_cost .view,' +
                    '#order_fee_line_items td.line_tax .view,' +
                    '#order_fee_line_items td.line_cost .view' 
                ).each( function( index ) {

                    let edit_input = jQuery( this ).parent().find( '.edit input' ).val();
                    let currency_object = jQuery( this ).parent().find( '.woocommerce-Price-currencySymbol' );
                    let currency = '';
                    if ( jQuery( currency_object ).length ) {
                        currency = ' ' + jQuery( currency_object ).html();
                    }

                    let first_element = jQuery( this ).children( 'span:first-child' );
                    
                    jQuery( first_element ).append( 
                        '<div class="german-market-additional-refund-fields"><span class="german-market-refund-value">' + 
                        edit_input + currency + 
                        '</span><span title="' + german_market_backend_translations.apply + '" class="german-market-refund-copy-button copy dashicons dashicons-admin-page" data-value="' +
                        edit_input +
                        '"></span></div>' 
                    );

                    jQuery( 
                        '#order_shipping_line_items .german-market-refund-copy-button,' + 
                        '#order_fee_line_items .german-market-refund-copy-button' 
                    ).click( function() {
                        let data_input = jQuery( this ).attr( 'data-value' );
                        jQuery( this ).parent().parent().parent().parent().find( '.refund input' ).val( data_input ).trigger( 'change' );
                    });
                });

                jQuery( '#woocommerce-order-items' ).on( 'click', '.refund-actions .cancel-action', function() {
                    jQuery( '.german-market-additional-refund-fields' ).each( function() {
                        jQuery( this ).remove();
                    });
                });
            });
        ...
        variation_settings: function() {
             ...
             jQuery(document).on('woocommerce_variations_loaded', function(event) {
               ...
               jQuery( '._v_used_setting_age_rating' ).change( function() {
               	...
               	// Charging device
                jQuery( '.german-market-variable-charging-device' ).change( function() {
                    
                   let option = jQuery( this ).val();
                   if ( -1 === option || "-1" === option ) {
                      
                        jQuery( this ).parent().parent().find( '.german-market-charging-device-special-variation-settings' ).hide();

                    } else {

                        jQuery( this ).parent().parent().find( '.german-market-charging-device-special-variation-settings' ).show();
        ...
        variable_price_prefix() {

        	if ( jQuery( '#german_market_variable_price_range_from' ).length ) {

                let option_value = jQuery( '#german_market_variable_price_range_from' ).is( ':checked' )

                if ( ! option_value ) {
                    jQuery( '#german_market_variable_price_range_from_prefix' ).parent().parent().hide();
                } else {
                    jQuery( '#german_market_variable_price_range_from_prefix' ).parent().parent().show();
                }
        ...
        special_tax_output: function() {

        	 if ( jQuery( '#german_market_special_tax_output_number' ).length ) {

                let number_of_special_tax_output = jQuery( '#german_market_special_tax_output_number' ).val();

                jQuery( '.german-market-special-tax-output' ).each( function() {

                    jQuery( this ).parents( 'tr' ).hide();

                });

                for ( var i = 1; i <= number_of_special_tax_output; i++ ) {

                    let id_of_the_element = '#german_market_special_tax_output_tax_class_' + i;
                    jQuery( id_of_the_element ).parents( 'tr' ).show();

                    id_of_the_element = '#german_market_special_tax_output_hide_default_' + i;
                    jQuery( id_of_the_element ).parents( 'tr' ).show();

                    id_of_the_element = '#german_market_special_tax_output_tax_information_' + i;
                    jQuery( id_of_the_element ).parents( 'tr' ).show();
