...
class WGM_Helper {
	...
	public static $run_time_cache = array();
	...
	*/
	...
	public static function get_translatable_options() {
		...
		$options = array(
			...
			'german_market_customizable_product_data_prefix_incl'							=> '',
			'german_market_customizable_product_data_prefix_excl'							=> '',
			'german_market_variable_price_range_from_prefix'								=> _x( 'From', 'From price prefix for variable products', 'woocommerce-german-market' ),
			'german_market_gpsr_label_manufacturer'											=> '',
			'german_market_gpsr_label_responsible_person'									=> '',
			'german_market_gpsr_label_warnings_and_safety_information'						=> '',
			'german_market_gpsr_default_manufacturer'										=> '',
			'german_market_gpsr_default_responsible_person'									=> '',
			'german_market_gpsr_default_warnings_and_safety_information'					=> '',
		);
		...
		for ( $i = 1; $i<= 5; $i++ ) {
			if ( get_option( 'german_market_special_tax_output_tax_information_' . $i ) != '' ) {
				$options[ 'german_market_special_tax_output_tax_information_' . $i ] = '';
			}
		}
		...
		if ( isset( $add_ons[ 'woocommerce-invoice-pdf' ] ) ) {
			...
			$options[ 'wp_wc_invoice_pdf_image_url_header' ]		= '';
			$options[ 'wp_wc_invoice_pdf_image_url_footer' ]		= '';
			$options[ 'wp_wc_invoice_pdf_image_url_background' ]	= '';
			$options[ 'wp_wc_invoice_pdf_cancel_stamp_text' ]		= _x( 'CANCELLED', 'stamp text in cancelled invoice', 'woocommerce-german-market' );
		}
		...
		if ( isset( $add_ons[ 'woocommerce-return-delivery-pdf' ] ) ) {
			...
			$options[ 'woocomerce_wcreapdf_wgm_pdf_logo_url_delivery' ]						= '';
			$options[ 'woocomerce_wcreapdf_wgm_pdf_logo_url' ]								= '';
		}
	...
	public static function wcvat_woocommerce_order_details_status( $order ) {
		...
		return apply_filters( 'german_market_order_details_status', $status, $order, $country, $postcode );
	}
	...
	*/
	public static function reset_textdomain() {
		...
		unload_textdomain( Woocommerce_German_Market::$textdomain );
		remove_filter( 'plugin_locale', array( __CLASS__, 'plugin_locale' ), 20, 2 );
	...
	*/
	...
	/**
	 * Get current refund by order
	 * Used in email attachments
	 * 
	 * @since 3.52
	 * @return ?WC_Order_Refund
	 */
	public static function get_current_refund_by_order( $order ) {
		...
		return apply_filters( 'german_market_get_current_refund_by_order', $refund, $order );
	}

	/**
	 * Check if email status is used for a refund mail
	 * 
	 * @since 3.52
	 * @param String $status
	 * @return Boolean
	 */
	public static function is_email_status_for_refund( $status ) {

		$is_email_status_for_refund = false;

		if ( 
			'customer_partially_refunded_order' === $status ||
			'customer_refunded_order' === $status 
		) {
			$is_email_status_for_refund = true;
		}

		return apply_filters( 'german_market_is_email_status_for_refund', $is_email_status_for_refund, $status );
	}
}

if ( 
	'off' === get_option( 'wgm_add_on_woocommerce_eu_vatin_check', 'off' ) && 
	( ! function_exists( 'wcvat_vat_exempt_first_login' ) ) && 
	! ( is_admin() && isset( $_REQUEST[ 'updated_wgm_add_ons' ] ) ) &&
	! ( is_admin() && isset( $_REQUEST[ 'tab' ] ) && 'add-ons' === $_REQUEST[ 'tab' ] )
) {
