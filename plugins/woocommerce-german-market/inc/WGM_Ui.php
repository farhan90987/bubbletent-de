...
class WGM_Ui {
	...
	*/
	public function wgm_ui_checkbox( $value ){
		...
		$is_disabled = false;
		if ( isset( $value[ 'custom_attributes' ][ 'disabled' ] ) && $value[ 'custom_attributes' ][ 'disabled' ] ) {
			$is_disabled = true;
		}
		...
		?>
			...
			<label class="switch" for="<?php echo $value['id'] ?>">
				<input
					...
					type="checkbox"
					class="<?php echo esc_attr( isset( $value['class'] ) ? $value['class'] : '' ); ?> <?php echo $is_disabled ? 'disabled-gm-checkbox' : ''; ?>"
					value="on"
					<?php
				...
				/>
				<div class="slider round gm-slider <?php echo $is_disabled ? 'disabled-gm-checkbox' : ''; ?>"></div>
			...
			<?php
				...
				?>
				<p class="screen-reader-buttons <?php echo $is_disabled ? 'disabled-gm-checkbox' : ''; ?>">
	...
	*/
	public function products(){
		...
		$price_presentation_variable_products_options = array(
		...
		$price_presentation_variable_products_info = '';

		if ( 'on' === get_option( 'german_market_customizable_product_data_disable_automatic_output', 'off' ) ) {
			unset( $price_presentation_variable_products_options[ 'gm_default' ] );
			unset( $price_presentation_variable_products_options[ 'gm_sepcial' ] );
			$price_presentation_variable_products_info = '<strong><em>' . 
				sprintf( 
					__( 'Since you have activated the option "WooCommerce -> German Market -> General -> Product Data -> %sDisable automatic output of product data%s", only the option "Do not change WooCommerce behaviour" is available.', 'woocommerce-german-market' ),
					'<a href="' . get_admin_url() . 'admin.php?page=german-market&tab=general&sub_tab=product-data#german_market_customizable_product_data_disable_automatic_output' . '">',
					'</a>'
				) . 
				'</strong></em><br>';
		}
		...
		$options[] = array(
			...
			'type' => 'title',
			'desc'=> $price_presentation_variable_products_info . WGM_Ui::get_video_layer( 'https://s3.eu-central-1.amazonaws.com/marketpress-videos/german-market/preisdarstellung-variable-produkte-und-varianten.mp4' ),
		...
		$options[] = array(
			...
			'id'       => 'german_market_price_presentation_variable_products',
			'type'     => 'select',
			'options'  => $price_presentation_variable_products_options,
			'class'    => 'wc-enhanced-select',
			'css'      => 'min-width: 350px;',
			'default'  => 'gm_default',
		);
		...
		$options[] = array(
			...
			'type'     => 'wgm_ui_checkbox',
			'default'  => 'on',
			'desc'	   => __( 'This setting does not work for block-based checkout, but only for shortcode-based checkout.', 'woocommerce-german-market' ),
		);
		...
		$options[] = array(
			...
			'default'  => 'standard',
			'options'  => array(
				'on' 		=> __( 'Show images in emails', 'woocommerce-german-market' ),
				'off'		=> __( 'Do not show images in emails', 'woocommerce-german-market' ),
				'standard'	=> __( 'Default behavior (determined by WooCommerce, your theme or third-party plugins)', 'woocommerce-german-market' )
			)
		);
	...
	public function cart_and_checkout() {
		...
		if ( function_exists( 'WC' ) ) {
		    ...
		    if ( version_compare( $woocommerce_version, '9.9.0' ) >= 0 ) {
		    	...
		    	$has_wc_9_9 = true;
		    	$desc_has_wc_9_9 = sprintf( 
		    		__( 'WooCommerce has provided this setting itself since version 9.9.0 and is therefore no longer available in German Market. You can find the WooCommerce setting <a href="%s">here</a>.', 'woocommerce-german-market' ),
		    		admin_url() . 'admin.php?page=wc-settings&tab=shipping&section=options'
		    	);
		    	$desc_has_wc_9_9 = '<span class="german-market-setting-info">' . $desc_has_wc_9_9 . '</span>';
		...
		$options[] = array(
				...
				'default'  => 'off',
				'custom_attributes' => $has_wc_9_9 ? array( 'disabled' => 'disabled' ) : array(),
				'desc'		=> $desc_has_wc_9_9,
			);
	...
	public function global_tab(){
		...
		$options = array(
			...
			array(
				...
				'desc_tip' => __( 'For each payment gateway you can specify a due date for orders in the submenu "Payment settings". The due date will be shown in the customer emails and in the invoice pdf.', 'woocommerce-german-market' ),
			...
			array(
				'name' =>  _x( 'Tax calculation for fees and shipping costs', 'backend-ui-heading', 'woocommerce-german-market' ),
				'type' => 'title',
				'id'   => 'tax_calc_heading'
			),
			...
			array(
				'name' 		=> __( 'Exclude fees', 'woocommerce-german-market' ),
				'id'		=> 'gm_tax_exclude_fees_from_tax_calulation',
				'type'     	=> 'wgm_ui_checkbox',
				'default'  	=> 'off',
				'desc'		=> __( 'If this setting is activated, German Market has no influence on the tax calculation of the fees, but displays the taxes calculated by other plugins. This is useful, for example, if a deposit plugin is used that adds the deposit as a fee.', 'woocommerce-german-market' ),
			),
	...
	public function emails() {
		...
		if ( file_exists( $theme_file ) ) {
			...
			if ( is_writable( $theme_file ) ) {
				...
				$template_file_desc	 = '<a href="' . $href . '" class="delete_template button german-market-delete-email-template-button" style="float: right; margin-top: -4px; margin-left: 10px;">' . __( 'Delete template file', 'woocommerce-german-market' ) . '</a>' . $template_file_desc;
			}
		} else {
	...
	private static function get_addons() {
		...
		$add_ons = array(
			...
			array(
				...
				'description'		=> __( 'This add-on is a Lexware Office API.', 'woocommerce-german-market' ) . '<br /><br />' . sprintf ( __( "You can register <a href=\"%s\" target=\"_blank\">here</a> if you don't have a Lexware Office account, yet", 'woocommerce-german-market' ), 'https://www.awin1.com/awclick.php?gid=508718&mid=13787&awinaffid=1909394&linkid=3827001&clickref=' ),
