...
class German_Market_Lexoffice_API_Contact {
	...
	*/
	public static function get_all_contacts() {
		...
		return apply_filters( 'german_market_lexoffice_contacts', $contacts );
	}
	...
	public static function build_customer_array( $wp_user, $order = null, $lexoffice_user_data = null ) {
		...
		foreach ( $addresses_pre as $pre ) {
			...
			foreach ( $address_meta_mapping as $woocommerce_key => $lexoffice_key ) {
                ...
                if ( $order && German_Market_Lexoffice_API_Shipping_Address::get_order_has_local_pickup( $order ) ) {

                        if ( ! is_object( $shipping_address ) ) {
                                $shipping_address = new stdClass();
                        }

                        $pickup_address_1 = German_Market_Lexoffice_API_Shipping_Address::get_shipping_address_1( $order );
                        $pickup_address_2 = German_Market_Lexoffice_API_Shipping_Address::get_shipping_address_2( $order );
                        $pickup_country  = German_Market_Lexoffice_API_Shipping_Address::get_shipping_country( $order );
                        $pickup_state    = German_Market_Lexoffice_API_Shipping_Address::get_shipping_state( $order );
                        $pickup_postcode = German_Market_Lexoffice_API_Shipping_Address::get_shipping_postcode( $order );
                        $pickup_city     = German_Market_Lexoffice_API_Shipping_Address::get_shipping_city( $order );

                        if ( '' !== $pickup_address_1 ) {
                                $shipping_address->street = $pickup_address_1;
                                $shipping_address_is_empty = false;
                        }

                        if ( '' !== $pickup_address_2 ) {
                                $shipping_address->supplement = $pickup_address_2;
                                $shipping_address_is_empty = false;
                        }

                        if ( '' !== $pickup_postcode ) {
                                $shipping_address->zip = $pickup_postcode;
                                $shipping_address_is_empty = false;
                        }

                        if ( '' !== $pickup_city ) {
                                $shipping_address->city = $pickup_city;
                                $shipping_address_is_empty = false;
                        }

                        if ( '' !== $pickup_country ) {
                                $shipping_address->countryCode = strtoupper( $pickup_country );
                                $shipping_address_is_empty = false;
                        }

                        if ( '' !== $pickup_state ) {
                                $shipping_address->state = $pickup_state;
                                $shipping_address_is_empty = false;
                        }

                        $shipping_address = apply_filters( 'german_market_lexoffice_pickup_shipping_address', $shipping_address, $order, $wp_user );

                        if ( $shipping_address instanceof stdClass ) {
                                foreach ( get_object_vars( $shipping_address ) as $value ) {
                                        if ( '' !== $value && null !== $value ) {
                                                $shipping_address_is_empty = false;
                                                break;
                                        }
