...
class German_Market_Lexoffice_Invoice_Email_Attachment {
	...
	public static function add_refund_attachment( $attachments, $refund_id, $order ) {
		...
		if ( 'on' === get_option( 'woocommerce_de_lexoffice_credit_note_email_attachment', 'off' ) ) {
			...
			if ( is_object( $refund ) && method_exists( $refund, 'get_meta' ) ) {
				...
				// get document
				$document_id 		= $refund->get_meta( '_lexoffice_woocomerce_has_transmission_invoice_api' );
				...
				if ( empty( $document_id ) ) {
					...
					$document_id 		= $refund->get_meta( '_lexoffice_woocomerce_has_transmission_invoice_api', true );
				...
				$pdf_binary = German_Market_Lexoffice_Invoice_API_General::download_pdf_document( $document_id, 'credit-note' );
	...
	*/
	public static function add_attachment( $attachments, $status, $order ) {
		...
		if ( WGM_Helper::is_email_status_for_refund( $status ) ) {
			if ( 'on' === get_option( 'woocommerce_de_lexoffice_credit_note_email_attachment', 'off' ) ) {
				...
				if ( is_object( $refund ) && method_exists( $refund, 'get_id' ) ) {
					$attachments = self::add_refund_attachment( $attachments, $refund->get_id(), $order );
				}
			}
		...
		// get document
		$document_id 		= $order->get_meta( '_lexoffice_woocomerce_has_transmission_invoice_api' );
		$send_to_lexoffice 	= false;
		...
		if ( empty( $document_id ) ) {
			...
			$send_to_lexoffice 	= true;
			$document_id 		= $order->get_meta( '_lexoffice_woocomerce_has_transmission_invoice_api', true );
		...
		$pdf_binary = German_Market_Lexoffice_Invoice_API_General::download_pdf_document( $document_id, 'invoice' );
