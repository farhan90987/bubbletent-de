...
function lexoffice_woocommerce_profile_fields( $user ) {
	...
	<h3><?php echo __( 'Lexware Office Contact', 'woocommerce-german-market' ); ?> </h3>
		...
		<tr>
			...
			<td class="lexoffice-contacts">
				<select name="lexoffice_contact" id="lexoffice_contact" class="wc-enhanced-select-nostd">
					...
					<?php
						$lexoffice_users = German_Market_Lexoffice_API_Contact::get_all_contacts();
						...
						foreach ( $lexoffice_users as $lexoffice_user ) {
							...
							if ( $display_name != '' ) {
								...
								if (
									isset( $lexoffice_user[ 'roles' ] ) &&
									isset( $lexoffice_user[ 'roles' ][ 'customer' ] ) &&
									isset( $lexoffice_user[ 'roles' ][ 'customer' ][ 'number' ] )
								) {
									$display_name .= ' (' . $lexoffice_user[ 'roles' ][ 'customer' ][ 'number' ]  . ')';
								}
				...
				<span class="description"><?php echo __( 'Assign a Lexware Office user to your WooCommerce user', 'woocommerce-german-market' ); ?></span>
			</td>
		</tr>
