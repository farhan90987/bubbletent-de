		...
		*/
		public static function get_address( $order, $show_errors = true ) {
			...
			if ( 'no' === get_option( 'woocommerce_de_lexoffice_invoice_api_use_lexoffice_contacts', 'no' ) ) {
				...
				if ( German_Market_Lexoffice_API_Shipping_Address::get_order_has_local_pickup( $order ) ) {
					...
					if ( '' !== $pickup_address_1 ) {
						$adress['street'] = $pickup_address_1;
					}

					if ( '' !== $pickup_address_2 ) {
						$adress['supplement'] = $pickup_address_2;
					}

					if ( '' !== $pickup_postcode ) {
						$adress['zip'] = $pickup_postcode;
					}

					if ( '' !== $pickup_city ) {
						$adress['city'] = $pickup_city;
					}

					if ( '' !== $pickup_country ) {
						$adress['countryCode'] = strtoupper( $pickup_country );
					}

					if ( '' !== $pickup_state ) {
						$adress['state'] = $pickup_state;
					}

					$adress = apply_filters( 'german_market_lexoffice_pickup_invoice_address', $adress, $order );
				}
