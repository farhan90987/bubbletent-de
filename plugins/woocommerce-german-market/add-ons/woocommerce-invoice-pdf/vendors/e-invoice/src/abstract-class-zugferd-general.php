	...
	*/
	...
	public function set_seller() {
		...
		if ( empty( $company_name ) ) {
			...
			$maybe_company_names = array(
				get_option( 'blogname' ),
				get_option( 'siteurl' ),
				__( 'Company name', 'woocommerce-german-market' )
			);

			foreach ( $maybe_company_names as $maybe_company_name ) {
				if ( ! empty( $maybe_company_name ) ) {
					$company_name = $maybe_company_name;
					break;
				}
			}
		}

		$this->zugferd_document->setDocumentSeller( $company_name );
	...
	public function add_order_items() {
		...
		foreach ( $this->order->get_items() as $key => $item ) {
			if ( method_exists( $item, 'get_quantity' ) ) {
				...
				// check tax rate if zero
				if ( 0.0 === floatval( $tax_rate_percent ) && is_object( $item ) && method_exists( $item, 'get_taxes' ) ) {
					$item_taxes = $item->get_taxes();
					if ( ! empty( $item_taxes[ 'total' ] ) ) {
						foreach ( array_keys( $item_taxes[ 'total' ] ) as $the_rate_id ) {
							$rate_id = $the_rate_id;
							$tax_rate_percent = trim( \WC_Tax::get_rate_percent_value( $rate_id ) );
							break;
						}
					}
				}
