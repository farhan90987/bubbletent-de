...
//////////////////////////////////////////////////
// custom output before order table
//////////////////////////////////////////////////
if ( $can_use_order ) {
	ob_start();
	do_action( 'wp_wc_invoice_pdf_custom_before_order_table', $order, $args );
	$custom_before_order_table = ob_get_clean();
} else {
	$custom_before_order_table = '';
}

if ( trim ( $custom_before_order_table != '' ) ) {

	?>
	<table class="welcome-text custom-before-order-table" cellspacing="0" cellpadding="0" border="0">
		<tr>
			<td><?php echo ( trim( wp_kses_post( $custom_before_order_table ) ) ); ?></td>
		</tr>
	</table>
	<?php
}
...
if ( ! has_action( 'wp_wc_invoice_pdf_item_image' ) ) {
	add_action( 'wp_wc_invoice_pdf_item_image', function( $item, $item_id, $item_i, $_product = null, $can_use_order = true, $order = null, $wc_price_args = array(), $args = array() ) {
			...
			<tr class="<?php echo apply_filters( 'wp_wc_invoice_pdf_tr_class', $tr_class, $item );?>"><?php
				...
				foreach ( $item_rows as $key => $row ) {
					...
					if ( isset( $row[ 'active' ] ) && $row[ 'active' ] ) {
						if ( isset( $row[ 'td' ] ) ) {
							$hook_suffix = $row[ 'td' ];
							do_action( "wp_wc_invoice_pdf_item_{$hook_suffix}", $item, $item_id, $item_i, $_product, $can_use_order, $order, $wc_price_args, $args );
						}
					}
				}
...
do_action( 'wp_wc_invoice_pdf_before_fine_print', $order, $args );
