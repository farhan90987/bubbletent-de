	...
	class WP_WC_Invoice_Pdf_Email_Attachment {
		...
		*/
		public static function add_download_link( $order, $sent_to_admin, $plain_text = true, $email = null ) {
			...
			if ( isset( $status ) && in_array( $status, $allowed_stati ) && apply_filters( 'wp_wc_invoice_pdf_allowed_order', true, $order, $status ) ) {
		...
		public static function add_refund_download_link( $order, $sent_to_admin, $plain_text = true, $email = null ) {
			...
			if ( WGM_Helper::is_email_status_for_refund( $status ) ) {
				...
				// get refund id
				$refund = WGM_Helper::get_current_refund_by_order( $order );
		...
		*/
		...
		/**
		 * Init refund attachments
		 * 
		 * @since 3.52
		 * @return void
		 */
		public static function init_refund_attachments() {

			if ( 'on' === get_option( 'wp_wc_invoice_pdf_emails_customer_refunded_order' ) ) {
				...
				if ( 'attachment' === $invoice_attachment_format ) {
				...
				} else if ( 'link' === $invoice_attachment_format ) {
					...
					if ( 'before_details' === $invoice_download_link_position ) {
						add_action( 'woocommerce_email_order_details', array( 'WP_WC_Invoice_Pdf_Email_Attachment', 'add_refund_download_link' ), 9, 4 );
					} else if ( 'after_details' === $invoice_download_link_position ) {
						add_action( 'woocommerce_email_order_meta', array( 'WP_WC_Invoice_Pdf_Email_Attachment', 'add_refund_download_link' ), 99, 4 );
					}
				}
			}

			if ( 'on' === get_option( 'wp_wc_invoice_pdf_emails_customer_refunded_order_add_pdfs' ) ) {
		...
		*/
		public static function trigger_refund_for_additional_pdfs( $attachments, $status, $order ) {
			...
			if ( 'customer_refunded_order' !== $status && WGM_Helper::is_email_status_for_refund( $status ) ) {
				$attachments = self::additional_email_attachments( $attachments, 'customer_refunded_order', $order );
			}
