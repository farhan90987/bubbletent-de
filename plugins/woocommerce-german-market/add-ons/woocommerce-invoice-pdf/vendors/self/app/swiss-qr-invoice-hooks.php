	...
	class WP_WC_Invoice_Pdf_Swiss_Qr_Invoice_Hooks {
		...
		*/
		public function __construct() {
			if ( Woocommerce_Invoice_Pdf::has_php_8_2_for_swiss_qr_codes() ) {
				add_filter( 'wp_wc_invoice_pdf_set_callbacks', array( $this, 'save_last_position_before_swiss_qr_invoice_in_pdf' ), 10, 2 );
		...
		public function rerender_pdf_with_new_page( $invoice, $html, $args ) {
			...
			if ( $this->order_needs_swiss_qr_invoice( $args[ 'order' ], $args ) ) {
				...
				if ( isset( $GLOBALS[ "new_page_before_swiss_qr_code" ] ) && 'yes' === $GLOBALS[ "new_page_before_swiss_qr_code" ] ) {
					...
					remove_filter( 'wp_wc_invoice_pdf_set_callbacks', array( $this, 'save_last_position_before_swiss_qr_invoice_in_pdf' ), 10, 2 );
		...
		* @return void
		*/
		public function save_last_position_before_swiss_qr_invoice_in_pdf( $callbacks, $args ) {
					...
					$callbacks[] = array(
						...
						'f' => function ( $infos ) {
							...
							if ( is_array( $infos ) ) {
								$frame = $infos[ 'frame' ];
								$pdf   = $infos[ 'canvas' ];
							} else if ( is_object( $infos ) && method_exists( $infos, 'get_frame' ) ) {
								$frame = $infos->get_frame();
								$dompdf = $infos->get_dompdf();
							...
							if ( 'before-fine-print' === $class ) {
								...
								if ( isset( $padding_box[ 'y' ] ) ) {
									...
									$extra_space = apply_filters( 'wp_wc_invoice_pdf_swiss_qr_invoice_extra_space', 0.1 );
	  								if ( $page_height_in_cm - $position_in_cm < apply_filters( 'wp_wc_invoice_pdf_swiss_qr_invoice_height', 10.5 + $extra_space ) ) {
