...
/**
 * The php-swiss-qr-bill library relies on the symfony/intl component to validate
 * ISO country codes. The original Symfony\Component\Intl\Countries class ships
 * a large dataset which increases the plugin size considerably. When that
 * component is stripped from the vendor directory we provide a lightweight
 * fallback that mimics the few methods the validator actually uses. This keeps
 * the validation layer happy without pulling the entire symfony/intl package.
 */
if ( ! class_exists( 'Symfony\Component\Intl\Countries', false ) ) {
        class WP_WC_Invoice_Pdf_Swiss_Qr_Invoice_Countries_Stub {

                /**
                 * Check if a given ISO 3166-1 alpha-2 country code exists.
                 *
                 * The Swiss QR bill validator only verifies that the code is
                 * formally valid, therefore it is sufficient to ensure a
                 * two-letter alphabetical uppercase string.
                 *
                 * @param string $country_code Two letter country code.
                 * @return bool
                 */
                public static function exists( $country_code ) {
                        $country_code = strtoupper( (string) $country_code );

                        return 2 === strlen( $country_code ) && ctype_alpha( $country_code );
                }

                /**
                 * Check if a given ISO 3166-1 alpha-3 country code exists.
                 *
                 * The validator only needs to know that the format is valid,
                 * therefore validating the length and character set is
                 * sufficient.
                 *
                 * @param string $country_code Three letter country code.
                 * @return bool
                 */
                public static function alpha3CodeExists( $country_code ) {
                        $country_code = strtoupper( (string) $country_code );

                        return 3 === strlen( $country_code ) && ctype_alpha( $country_code );
                }
        }

        class_alias( 'WP_WC_Invoice_Pdf_Swiss_Qr_Invoice_Countries_Stub', 'Symfony\Component\Intl\Countries' );
}
	...
	class WP_WC_Invoice_Pdf_Swiss_Qr_Invoice {
		...
		*/
		public function get_qr_bill_object_by_order() {
			...
			if ( ! $this->is_test ) {
				...
				$address_1 				= __( 'Example Avenue 42', 'woocommerce-german-market' );
				$postcode 				= __( '61109', 'woocommerce-german-market' );
				$city 					= __( 'Springfield', 'woocommerce-german-market' );
				$country 				= 'CH';
			}
			...
			$full_street_and_number = get_option( 'wp_wc_invoice_pdf_swiss_qr_invoice_remit_recipient_address', get_option( 'woocommerce_store_address', '' ) );
			...
			$qrBill->setCreditor(
			    QrBill\DataGroup\Element\StructuredAddress::createWithStreet(
			        ...
			        self::get_street_name( $full_street_and_number ),
			        self::get_house_no( $full_street_and_number ),
			        get_option( 'wp_wc_invoice_pdf_swiss_qr_invoice_remit_recipient_postcode', get_option( 'woocommerce_store_postcode', '' ) ),
			        get_option( 'wp_wc_invoice_pdf_swiss_qr_invoice_remit_recipient_city', get_option( 'woocommerce_store_city', '' ) ),
			...
			$qrBill->setUltimateDebtor(
			    QrBill\DataGroup\Element\StructuredAddress::createWithStreet(
			        $name,
			        self::get_street_name( $address_1 ),
			        self::get_house_no( $address_1 ),
			        $postcode,
			        $city,
			        $country,
			    ));
		...
		*/
		public function make_markup() {
			...
			do_action( 'wp_wc_invoice_pdf_swiss_qr_before_fine_print_before_html' );
			echo $html;
		...
		* @return void
		*/
		private function log_error( $e, $qrBill = null ) {
			...
			if ( is_object( $qrBill ) && method_exists( $qrBill, 'getViolations' ) ) {
