...
use MarketPress\German_Market\E_Invoice\E_Invoice_Merge_And_Save;
	...
	class WP_WC_Invoice_Pdf_Create_Pdf {
		...
		*/
		public function __construct ( $args = array() ) {
			...
			$invoice = new Ebs_Pdf_Wordpress( 'wp_wc_invoice_pdf_', $args );
			do_action( 'wp_wc_invoice_pdf_after_new_dompdf', $invoice, $args );
			...
			if ( ! isset( $args[ 'refund' ] ) ) {
				...
				if ( ! $is_test && ! has_filter( 'wp_wc_invoice_pdf_template_invoice_content' ) ) {
					...
					$invoice_pdf_order_meta = new WP_WC_Invoice_Pdf_Order_Meta( $args[ 'order' ] );

					if ( in_array( $args[ 'order' ]->get_status(), $always_create_new_pdf_status ) ) {
						$invoice_pdf_order_meta->delete_meta();
					} else {
						
						$maybe_saved_html = $invoice_pdf_order_meta->get_meta();
						...
						if ( ! has_filter( 'wp_wc_invoice_pdf_template_invoice_content' ) ) {
							if ( ! $create_new_but_dont_save ) {
 								...
 								if ( ! in_array( $args[ 'order' ]->get_status(), $always_create_new_pdf_status ) ) {
 									...
 									$invoice_pdf_order_meta->add_meta( $html );
 								}
				...
				// for refunds
				$invoice_pdf_order_meta =new WP_WC_Invoice_Pdf_Order_Meta( $args[ 'refund' ] );
				$maybe_saved_html = $invoice_pdf_order_meta->get_meta();
			...
			require_once( apply_filters( 'wp_wc_invoice_pdf_template_page_numbers_dompdf3', $page_numbers_core_template ) );

			$invoice->pdf->setCallbacks(
				
				apply_filters( 'wp_wc_invoice_pdf_set_callbacks',
					array(
						array(
							'event' => 'end_document', 
							'f' => function( $pageNumber, $pageCount, $canvas, $fontMetrics ) use ( $args ) {

								do_action( 'wp_wc_invoice_pdf_before_get_template_page_numbers', $args );
								WP_WC_Invoice_Pdf_Page_Numbers::add_page_numbers( $pageNumber, $pageCount, $canvas, $fontMetrics );
								do_action( 'wp_wc_invoice_pdf_after_get_template_page_numbers', $args );
								do_action( 'wp_wc_invoice_pdf_after_set_callbacks_end_document' );
							}
						)
					), $args )
			);
			...
			// echo html
			if ( ( ( isset( $args[ 'output_format' ] ) ) && ( $args[ 'output_format' ] == 'html' ) ) || ( WP_WC_Invoice_Pdf_Create_Pdf::use_html_output( $args ) == 'yes'  ) ) {
			...
			// render to pdf
				...
				$invoice = apply_filters( 'wp_wc_invoice_pdf_before_pdf_generation_invoice_var', $invoice, $html, $args );
				...
				if ( ( isset( $args[ 'output' ] ) ) && ( $args[ 'output' ] == 'inline' ) ) {
					...
					$e_invoice_download = new E_Invoice_Merge_And_Save( 
						isset( $args[ 'refund' ] ) ? $args[ 'refund' ] : $args[ 'order' ], 
						$invoice->pdf->output(),
						$filename,
						'browser_inline'
					);
					...
					$e_invoice_download = new E_Invoice_Merge_And_Save( 
						...
						$filename,
						'save_temp',
						WP_WC_INVOICE_PDF_CACHE_DIR . $directory_name . DIRECTORY_SEPARATOR
					);

					if ( false === $e_invoice_download->get_is_saved_temp() ) {
					...
					$e_invoice_download = new E_Invoice_Merge_And_Save( 
						...
						$filename,
						'save_temp',
						$directory_name . DIRECTORY_SEPARATOR
					);
		...
		*/
		public static function get_footer_or_header( $part, $args = array() ) {
			...
			return apply_filters( 'wp_wc_invoice_pdf_header_footer_content_result', $content, $part, $args );
		}
		...
		*/
		public static function include_background_image( $part, $height = NULL, $width = NULL, $args = array() ) {
			...
			$file = apply_filters( 'wp_wc_invoice_include_background_image_url', $file, $part, $args );
			if ( $file == '' ) {
				return false;
			} else {
				$inline_style	= ( isset( $args[ 'inline_style' ] ) ) ? $args[ 'inline_style' ] : true;
				...
				if ( $html_output == 'no' ) { // for html debugging => outout url
					...
					if ( ! $is_remote ) {
						...
						if ( is_file( $file_path ) ) {
							...
							$file_parts = explode( '/uploads', $file );
							if ( is_array( $file_parts ) && isset( $file_parts[ 1 ]  ) ) {
								$file_path = $path . $file_parts[ 1 ];
				...
				// first build div
				if ( $inline_style ) {
					$div_style = 'position: fixed; z-index: -1; overflow: hidden; height: ' . $height . '; width:' . $width . '; content: "";';
				...
				if ( $inline_style ) {
					...
					if ( in_array( $height, array( 0, '' ) ) && in_array( $width, array( 0, '' ) ) ) {
						...
						if ( 'background' === $part ) {
							...
							$height = self::convert_to_css_numeric( $original_height ) . $user_unit;
						}
				...
				return apply_filters( 'wp_wc_invoice_include_background_image_return_value', str_replace( '[image]', $img, $div ), $img, $div, $file );
			}
		}
		...
		*/
		public static function use_html_output( $args = array() ) {
			...
			if ( is_array( $args ) && isset( $args[ 'output'] ) && 'cache' === $args[ 'output' ] ) {
				$use_html_output = 'no';
			} else if ( is_array( $args ) && isset( $args[ 'output'] ) && 'cache-zip' === $args[ 'output' ] ) {
				$use_html_output = 'no';
			} else if ( is_array( $args ) && isset( $args[ 'frontend'] ) && 'yes' === $args[ 'frontend' ] ) {
				$use_html_output = 'no';
			} else {
					$use_html_output = get_option( 'wp_wc_invoice_pdf_force_html_output', 'no' );
			}

			return $use_html_output;
		}
