...
class Internetmarke {
		...
		*/
		...
		if ( is_object( $order ) && method_exists( $order, 'get_shipping_address_1' ) ) {
			...
			*/
				...
				*/
				...
				curl_close( $ch );
                               ...
                               if ( $data ) {
                                       ...
                                       $internetmarke_meta = new Internetmarke_Label_Meta( $order );
                                       $internetmarke_meta->add_meta( '_wgm_internetmarke_binary_label_data', $data );
			...
			} else if ( isset( $result->faultcode ) && isset( $result->faultstring ) ) {
			...
			} else if ( isset( $result->statusCode ) && '400' && $result->statusCode && isset( $result->description ) ) {

				$response = array(
					'result_code'    => 'error',
					'api_error_code' => '400',
					'api_error_msg'  => $result->description,
					'result'         => $result,
				);
	...
	public static function download_label()
	{
		...
		// init
                ...
                $meta       = new Internetmarke_Label_Meta( $order );
                $stamp_data = $meta->get_meta( '_wgm_internetmarke_binary_label_data' );
	...
	public function backend_icon_download( array $actions, WC_Order $order ) : array
	{
		...
		if ( ! Helper::check_order_for_shipping_provider_methods( $order, $this->id ) ) {
			return $actions;
                ...
                $meta              = new Internetmarke_Label_Meta( $order );
                $has_internetmarke = $meta->has_meta( '_wgm_internetmarke_binary_label_data' );

                if ( $has_internetmarke ) {
			$create_pdf = array(
