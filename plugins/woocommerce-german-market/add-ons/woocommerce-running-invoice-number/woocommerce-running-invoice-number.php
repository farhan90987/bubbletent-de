	...
	class Woocommerce_Running_Invoice_Number {
		...
		*/
		public function autoload( $class ) {
			...
			if ( strpos( $class, 'wp_wc_running_invoice_number_backend_' ) === 0 ){
				...
				if ( false !== strpos( $class, 'wp_wc_running_invoice_number' ) ) {
		...
		*/		
		private static function init() {	
			...
			if ( get_option( 'wp_wc_running_invoice_email_activation', 'on' ) == 'on' ) {
				...
				add_filter( 'woocommerce_email_heading_customer_refunded_order', 		array( 'WP_WC_Running_Invoice_Number_Email', 'get_heading_refunded_order' ), 10, 3 );
				add_filter( 'woocommerce_email_subject_customer_refunded_order',		array( 'WP_WC_Running_Invoice_Number_Email', 'get_subject_refunded_order' ), 10, 3 );
			...
			if ( self::is_wp_wc_invoice_pdf_activated() ) {
				...
				add_filter( 'wp_wc_invoice_pdf_girocode_placeholders_text', array( 'WP_WC_Running_Invoice_Number_Invoice_Pdf', 'extra_texts' ), 10, 2 );
			...
			if ( is_admin() ) {
				...
				add_action( 'admin_init', function() {
					add_filter( WGM_Hpos::get_hook_for_manage_shop_order_posts_columns(), array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'shop_order_columns' ), 20 );
					add_filter( WGM_Hpos::get_hook_for_manage_edit_shop_order_sortable_columns(), array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'shop_order_sortable_columns' ) );
					add_filter( WGM_Hpos::get_hook_manage_shop_order_custom_column(), array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'render_shop_order_columns' ), 10, 2 );
				});

				
				if ( WGM_Hpos::is_hpos_enabled() ) {
					add_filter( 'woocommerce_order_list_table_prepare_items_query_args', array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'shop_order_sort_hpos' ) );
				} else {
					add_action( 'pre_get_posts', array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'shop_order_sort' ) ); // HPOS: Does not work, of course
				}
				...
				add_action( 'wp_ajax_wp_wc_running_invoice_number_show_after_invoice_pdf_clicked', 	array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'shop_order_ajax_after_invoice_pdf_clicked' ) );
				...
				add_filter( 'woocommerce_order_table_search_query_meta_keys', array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'search_query' ) );
				add_action( 'woocommerce_hpos_admin_search_filters', array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'add_hpos_full_text_search_option' ) );
				add_action( 'woocommerce_hpos_generate_where_for_search_filter', array( 'WP_WC_Running_Invoice_Number_Backend_Output_Shop_Order', 'add_hpos_full_text_search_filter' ), 10, 4 );
			...
			if ( is_admin() ) {
				...
				add_action( 'wp_ajax_wp_wc_running_invoice_number_delete', 					array( 'WP_WC_Running_Invoice_Number_Backend_Output_Post', 'delete_invoice_number' ) );	
				...
				add_action( 'woocommerce_after_order_refund_item_name',						array( 'WP_WC_Running_Invoice_Number_Backend_Output_Post', 'show_refund_invoice_number_in_order_refunds' ) );
			}
			...
			add_filter( 'sevdesk_woocommerce_api_voucher_description',						array( 'WP_WC_Running_Invoice_Number_Online_Bookkeeping', 'sevdesk_voucher_number' ), 10, 2 );

			add_filter( 'lexoffice_woocommerce_api_order_voucher_date', 					array( 'WP_WC_Running_Invoice_Number_Online_Bookkeeping', 'lexoffice_sevdesk_voucher_date' ), 10, 2 );
			add_filter( 'sevdesk_woocommerce_api_voucher_date',								array( 'WP_WC_Running_Invoice_Number_Online_Bookkeeping', 'lexoffice_sevdesk_voucher_date' ), 10, 2 );
		...
		*/	
		public static function load_admin_js() {
			...
			if ( ( ( ( get_current_screen()->id == apply_filters( 'german_market_screen_id_slug', 'woocommerce_page_german-market' ) ) && isset( $_GET[ 'tab' ] ) && ( $_GET[ 'tab' ] == 'preferences-wp-wc-running-invoice-number' ) ) || ( ( WGM_Hpos::get_edit_shop_order_screen() ) && ( self::is_wp_wc_invoice_pdf_activated() ) ) || ( get_current_screen()->id == 'shop_order' || WGM_Hpos::get_edit_shop_order_screen() ) ) || ( get_current_screen()->id == 'woocommerce_page_wgm-refunds' ) ) {	
