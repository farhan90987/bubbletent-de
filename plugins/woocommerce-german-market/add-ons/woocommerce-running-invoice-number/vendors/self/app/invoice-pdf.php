	...
	class WP_WC_Running_Invoice_Number_Invoice_Pdf {
		...
		*/
		public static function get_frontend_filename_refund( $filename, $refund ) {
			$search 		= array( '{{order-number}}', '{{order-date}}', '{{invoice-number}}', '{{invoice-date}}', '{{refund-number}}', '{{refund-id}}', '{{refund-date}}' );
			return self::replace( get_option( 'wp_wc_running_invoice_pdf_file_name_frontend_refund', __( 'Refund-{{refund-number}}-for-order-{{order-number}}', 'woocommerce-german-market' ) ), $refund, $search );
		}
		...
		*/
		public static function get_subject_refund_line_2( $subject_line, $refund ) {
			$search 		= array( '{{order-number}}', '{{order-date}}', '{{invoice-number}}', '{{invoice-date}}', '{{refund-number}}', '{{refund-id}}', '{{refund-date}}' );
			return self::replace( get_option( 'wp_wc_running_invoice_pdf_refund_start_subject_small', __( 'For order {{order-number}}', 'woocommerce-german-market' ) ), $refund, $search );
		}
					...
					if ( $order->get_type() == 'shop_order' || $order->get_type() == 'shop_order_refund' ) {
						...
						if ( $order->get_type() == 'shop_order_refund' ) {
							...
							$replace = array( 
								...
								date_i18n( get_option( 'date_format' ), $order->get_date_created()->getTimestamp() ), // order-date
								$running_invoice_number->get_invoice_number(), // invoice-number
								date_i18n( get_option( 'date_format' ), intval( $running_invoice_number->get_invoice_timestamp() ) ),  // invoice-date
								$running_refund_number->get_invoice_number(), // refund-number
								$refund->get_id(), // refund-id
								date_i18n( get_option( 'date_format' ), $refund->get_date_created()->getTimestamp() ) // refund-date
							);
							...
							$replace = array( 
								...
								$running_invoice_number->get_invoice_number(), // invoice-number
								date_i18n( get_option( 'date_format' ),	intval( $running_invoice_number->get_invoice_timestamp() ) ), // invoice-date
								wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ) // blog name
							);
