...
//        $colunm = array_map(function($vl){ return array('title'=>$vl, 'description'=>$vl); }, $arr); 
    ...
    public static function get_all_metakeys() {        
        ...
        $meta = $wpdb->get_col(
            ...
            WHERE p.post_type IN ( 'product', 'product_variation' ) $limit_query_string"
        );
        sort($meta);
    ...
    public function exporter_alter_advanced_fields($fields, $base, $advanced_form_data) {
        ...
        $out['only_last_modified'] = array(
            ...
            'help_text' => __("Only export Products that have been modified since last export.", 'wt-import-export-for-woo'),
        );

        $out['export_children_sku'] = array(
            'label' => __("Export SKU of grouped products, up-sells, and cross-sells", 'wt-import-export-for-woo'),
            'type' => 'radio',
            'radio_fields' => array(
...
//                'import' => __('Import as new item'),
        ...
        $out['conflict_with_existing_post'] = array(
            ...
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('Selecting this option skips the import of that particular product if there is a conflict in Post ID with an existing post.', 'wt-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_id_conflict', 'value'=>'skip')
                    )
                ),
                array(
                    'help_text'=> __('This option will import the product with a new ID', 'wt-import-export-for-woo'),
                    'condition'=>array(
...
//        );
        ...
        $out['delete_existing'] = array(
            ...
            'help_text' => __('Check this if you need to remove products from your store which are not present in the input file. For e.g, if you have a product A in your store and your import file has products B, C; the product A is deleted from the store prior to importing B and C.', 'wt-import-export-for-woo'),
        );
    ...
    public function exporter_alter_filter_fields($fields, $base, $filter_form_data) {
        ...
        $fields['stock_status'] = array(
            'label' => __( 'Stock status', 'wt-import-export-for-woo' ),
