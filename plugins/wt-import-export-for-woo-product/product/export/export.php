...
class Wt_Import_Export_For_Woo_Product_Export {
    ...
    public function prepare_data_to_export($form_data, $batch_offset,$step) {
		...
		$only_last_modified = !empty($form_data['advanced_form_data']['wt_iew_only_last_modified']) ? $form_data['advanced_form_data']['wt_iew_only_last_modified'] : 0;
...
//            }
			...
			// Export all langauge products if WPML is active and language selected is all
			if ( function_exists('icl_object_id') && isset( $_SERVER["HTTP_REFERER"] ) && strpos($_SERVER["HTTP_REFERER"], 'lang=all') !== false ) {
				 $args['suppress_filters'] = true;
			}			
			
			if ( !empty( $only_last_modified ) ) {
				// get last product export date.
				global $wpdb;
				$data = $wpdb->get_row( $wpdb->prepare( "SELECT created_at FROM {$wpdb->prefix}wt_iew_action_history WHERE status=%d AND item_type=%s AND template_type=%s ORDER BY id DESC LIMIT 0,1", 1, 'product', 'export' ), ARRAY_A );
				if( !empty( $data ) ){
					$last_exported_date = gmdate( 'Y-m-d H:i:s', $data['created_at'] );
					$args['date_query'] = array(
						array(
							'after' => $last_exported_date,
...
//            'review_count' => '',
//            );
//            if(empty($prod_types) || in_array('variable', $prod_types) ){ //PIEPFW-1143,PIEPFW-1145,PIEPFW-1114,PIEPFW-1113
...
//            }
    ...
    public static function getProductImages($product) {
        ...
        if(apply_filters('wt_ier_export_product_gallery_images', true)){
            // Images
