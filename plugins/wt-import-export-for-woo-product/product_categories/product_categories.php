...
if(!class_exists('Wt_Import_Export_For_Woo_Product_Categories')){
class Wt_Import_Export_For_Woo_Product_Categories {
    ...
    */
    public function importer_steps($steps, $base)
    {
        ...
        {
            $steps['advanced']['description']=__('Use advanced options from below to decide updates to existing categories, batch import count or schedule an import. You can also save the template file for future imports.', 'wt-import-export-for-woo');
        }
        return $steps;
    }
    ...
    public function importer_do_import($import_data, $base, $step, $form_data, $selected_template_data, $method_import, $batch_offset, $is_last_batch) {        
        ...
        $import = new Wt_Import_Export_For_Woo_Categories_Import($this);
    ...
    public function exporter_do_export($export_data, $base, $step, $form_data, $selected_template_data, $method_export, $batch_offset) {
        ...
        $export = new Wt_Import_Export_For_Woo_Categories_Export($this);
    ...
    public function wt_iew_exporter_post_types($arr) {
        $arr['product_categories'] = __('Product Categories', 'wt-import-export-for-woo');
        return $arr;
    }
    ...
    public static function get_categories_sort_columns() {
            $sort_columns = array(
                'id' => __('Category ID', 'wt-import-export-for-woo'),
                'name' => __('Category name', 'wt-import-export-for-woo'),
                'slug' => __('Category slug', 'wt-import-export-for-woo'),
            );
...
//        $colunm = array_map(function($vl){ return array('title'=>$vl, 'description'=>$vl); }, $arr); 
    ...
    public function wt_get_found_product_cat_meta() {
            ...
            $term_args = array('taxonomy' => 'product_cat', 'hide_empty' => false);
    ...
    public function importer_alter_advanced_fields($fields, $base, $advanced_form_data) {
        ...
        $out['merge'] = array(
            ...
            'value' => '0',
            'field_name' => 'merge',
            'help_text' => __('Categories are matched by their ID/slugs.', 'wt-import-export-for-woo'),
            'help_text_conditional'=>array(
                array(
                    'help_text'=> __('Retains the categories in the store as is and skips the matching category from the input file.', 'wt-import-export-for-woo'),
                    'condition'=>array(
                        array('field'=>'wt_iew_merge', 'value'=>0)
                    )
                ),
                array(
                    'help_text'=> __('Update category as per data from the input file', 'wt-import-export-for-woo'),
                    'condition'=>array(
