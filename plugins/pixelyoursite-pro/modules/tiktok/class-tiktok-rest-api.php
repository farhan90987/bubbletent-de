...
class TikTok_REST_API {
    ...
    public function register_rest_routes() {
        register_rest_route( 'pys-tiktok/v1', '/event', array(
            'methods'             => 'POST',
            'callback'            => array( $this, 'handle_tiktok_event' ),
     ...
     * Handle TikTok event
     */
    public function handle_tiktok_event( $request ) {
        ...
        $singleEvent = $this->data_to_single_event( $event, $data, $eventID, $ids, $woo_order, $edd_order, $event_slug );

        // Send event using existing TikTok server logic
        TikTokServer()->sendEventsNow( array( $singleEvent ) );
     ...
     */
    private function data_to_single_event( $event_name, $params, $event_id, $ids, $woo_order, $edd_order, $event_slug = "" ) {
        $singleEvent = new SingleEvent( $event_slug, "" );
    ...
    public function enqueue_scripts() {
        if ( ! Tiktok()->isServerApiEnabled() ) {
            return;
        }

        wp_localize_script( 'jquery', 'pysTikTokRest', array(
            'restApiUrl' => rest_url( 'pys-tiktok/v1/event' ),
