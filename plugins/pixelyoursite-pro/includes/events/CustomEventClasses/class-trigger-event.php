...
class TriggerEvent {
	...
	public function getEventTriggers() {
		...
		switch ( $this->getTriggerType() ) {
            ...
            case 'form_field':
                {
                    $allFields = $this->getAllFieldsAnyForms();
                    $field = $this->getAnyFormField();
                    $form = $this->getAnyForm();

                    $selectedFieldArr = array_filter($allFields, function($fieldValue) use ($field) {
                        return isset($fieldValue['selector']) && $fieldValue['selector'] == $field;
                    });

                    $selectedField = reset($selectedFieldArr);

                    $payload['data'][] = array(
                        'form' => $form,
                        'field' => $selectedField['selector'] ?? '',
