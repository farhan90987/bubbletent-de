...
var wt_iew_email_settings_form=
{
	Set:function(prnt_selector)
	{ 
		var wf_prnt_obj=jQuery(prnt_selector);
		wf_prnt_obj.find('.wt_iew_email_settings_form').submit(function(e){
			e.preventDefault();
			var data=jQuery(this).serialize();
			...
			jQuery.ajax({
				...
				type:'POST',
				dataType:'json',
				data:data+'&action=wt_iew_save_email_settings',
				success:function(data)
				{
					spinner.css({'visibility':'hidden'});
					submit_btn.css({'opacity':'1','cursor':'pointer'}).prop('disabled',false);
	...
	{ 
                jQuery('.wt_ier_delete_template').unbind('click').click(function (e) {
                    ...
                    var data = {
                        ...
                        action: 'wt_iew_delete_template_pro',
...
var wt_iew_popover=
{
		...
		jQuery('[data-wt_iew_popover="1"]').unbind('click').click(function(){
			...
			if(cr_elm.attr('data-popup-opened')==1)
			{
				...
				wt_iew_popover.closePop();
                                jQuery('.wt_iew_mapping_column_preview').parents('td').css({'background':'#f6f6f6'});
				return false;
			}else
			{
				jQuery('[data-wt_iew_popover="1"]').attr('data-popup-opened',0);
			...
			pp_elm.css({'display':'block','opacity':0, 'top':cr_elm_pos_t,'left':cr_elm_pos_l}).stop(true,true).animate({'left':cr_elm_pos_l+20,'opacity':1}, animation_speed, function(){
		...
		jQuery('[name="wt_iew_popover_do_action"]').unbind('click').click(function(){
			...
			if(wt_iew_popover.action_module=='import')
			{
				...
				}else
				{
					click_elm.parents('tr').find('.wt_iew_mapping_checkbox_sub').prop('checked', true);
                                ...
                                wt_iew_import.mapping_field_editor_output_preview_el(click_elm.parents('tr').find('.wt_iew_mapping_column_preview'),html_vl );
			...
			if(wt_iew_popover.action_module=='import')
			{
				...
				}else
				{
					click_elm.parents('tr').find('.wt_iew_mapping_checkbox_sub').prop('checked', true);
                                        if(!jQuery(this).hasClass('dashicons')){
                                            var choosen_str = '{'+jQuery(this).attr('data-val')+'}';
                                            wt_iew_import.mapping_field_editor_output_preview_el(click_elm.parents('tr').find('.wt_iew_mapping_column_preview'),choosen_str );
                                        }
	...
	closePop:function()
	{
		...
		if(pp_elm.length>0)
		{
			...
			jQuery('[data-wt_iew_popover="1"]').attr('data-popup-opened',0);
			pp_elm.stop(true,true).animate({'opacity':0, 'left':pp_lft},0,function(){
...
jQuery(document).ready(function(){
	...
	wt_field_group.Set();
        ...
        wt_iew_email_settings_form.Set('#'+wt_iew_params.plugin_id);
});
