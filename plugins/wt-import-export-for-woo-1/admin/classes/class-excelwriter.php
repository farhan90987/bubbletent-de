...
class Wt_Import_Export_For_Woo_Excelwriter
{
	public $file_path='';
	public $data_ar='';
	...
	private function set_content($export_data, $delm=',')
	{
		if(isset($export_data) && isset($export_data['body_data']) && count($export_data['body_data'])>0)
		{
			...
			foreach($row_datas as $row_data)
			{
				...
				$this->spreadsheet->getActiveSheet()->insertNewRowBefore($row);
				$this->spreadsheet->getActiveSheet()->fromArray($row_data,NULL,'A'.$row);
			}
