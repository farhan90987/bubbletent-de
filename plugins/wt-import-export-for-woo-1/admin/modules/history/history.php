...
class Wt_Import_Export_For_Woo_History
{
	...
	public function __construct()
	{
		...
		self::$status_label_arr=array(
			0=>__('Running/Incomplete', 'wt-import-export-for-woo'),
			1=>__('Finished', 'wt-import-export-for-woo'),
			2=>__('Failed', 'wt-import-export-for-woo'),
		);

		self::$action_label_arr=array(
			'export'=>__('Export', 'wt-import-export-for-woo'),
			'import'=>__('Import', 'wt-import-export-for-woo'),
			'export_image'=>__('Image Export', 'wt-import-export-for-woo'),
		);
	...
	*/
	public function add_admin_pages($menus)
	{
		$menus[$this->module_base]=array(
			'submenu',
			WT_IEW_PLUGIN_ID,
			__('History', 'wt-import-export-for-woo'),
			__('History', 'wt-import-export-for-woo'),
		...
		//if(Wt_Import_Export_For_Woo_Common_Helper::get_advanced_settings('enable_import_log')==1){
			$menus[$this->module_base.'_log']=array(
				'submenu',
				WT_IEW_PLUGIN_ID,
				__('Import Logs', 'wt-import-export-for-woo'),
				__('Import Logs', 'wt-import-export-for-woo'),
				apply_filters('wt_import_export_allowed_capability', 'import'),
				$this->module_id.'_log',
	...
	public function ajax_main()
	{
		if(Wt_Iew_Sh_Pro::check_write_access(WT_IEW_PLUGIN_ID))
		{
			...
			$history_action=Wt_Iew_Sh_Pro::sanitize_item($_POST['history_action'], 'text');
					...
					{						
						...
						if($response['response'])
						{
							...
							$show_item_details=false;
                                                        if($history_item['item_type'] == 'product_categories' || $history_item['item_type'] == 'product_tags'){
                                                            $item_type_module_obj=Wt_Import_Export_For_Woo::load_sub_modules($history_item['item_type']);
                                                        }else{
	...
	*/ 
	public function advanced_setting_fields($fields)
	{
		...
		$fields['enable_history_auto_delete']=array(
			'label'=>__("Auto delete history", 'wt-import-export-for-woo'),
			'type'=>'checkbox',
			'checkbox_fields' => array( 1 => __( 'Enable', 'wt-import-export-for-woo' ) ),
            'value' =>1,
			'field_name'=>'enable_history_auto_delete',
			'field_group'=>'advanced_field',
			'help_text'=>__('Clicking ‘Yes’ will automatically delete the records in the history section', 'wt-import-export-for-woo'),
		...
		$fields['auto_delete_history_count']=array(
			'label'=>__("Maximum entries", 'wt-import-export-for-woo'),
			'type'=>'number',
            'value' =>100,
            'attr' =>array(
            	'style'=>'width:30%;',
	...
	{	
		...
		if(isset($_GET['wt_iew_delete_log'])) 
		{
			if(Wt_Iew_Sh_Pro::check_write_access(WT_IEW_PLUGIN_ID))
			{
				...
				$log_file_arr = Wt_Iew_Sh_Pro::sanitize_item($log_file_arr, 'text_arr');
	...
	public function admin_settings_page($args)
	{
		...
		if(isset($_GET['wt_iew_delete_history'])) 
		{
			if(Wt_Iew_Sh_Pro::check_write_access(WT_IEW_PLUGIN_ID))
			{
				...
				$history_id_arr=Wt_Iew_Sh_Pro::sanitize_item($history_id_arr, 'absint_arr');
		...
		*/
		...
		$conf_arr=isset($_GET['wt_iew_history']) ? Wt_Iew_Sh_Pro::sanitize_item($_GET['wt_iew_history'], 'text_arr') : array();
		...
		{		
			...
			foreach ($filter_by as $filter_key => $filter_val)
			{
				if(isset($filter_by_conf[$filter_key]) && trim($filter_by_conf[$filter_key])!="") //current filter applied
				{
					...
					$filter_by[$filter_key]['selected_val']=Wt_Iew_Sh_Pro::sanitize_data($filter_by_conf[$filter_key], $filter_key, $filter_by_validation_rule);
		...
		if($record_count>=1)
	 	{
			...
			$latest_finished_entries = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}wt_iew_action_history WHERE status = 1 ORDER BY id DESC LIMIT 0, %d",  $limit_record_count ), ARRAY_A );

			$skip_from_delete_ids = array();
			if(!empty($latest_finished_entries)){
				foreach ($latest_finished_entries as $latest_finished_entry){
					$skip_from_delete_ids[] = $latest_finished_entry['id'];
				}
			}
			$history_entries_to_delete = false;
			if(!empty($skip_from_delete_ids)){
				$history_entries_to_delete = $wpdb->get_results( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}wt_iew_action_history WHERE id NOT IN( " . implode( ',', array_fill( 0, count( $skip_from_delete_ids ), '%d' ) ) . ' )', $skip_from_delete_ids) , ARRAY_A);
			}
			if($history_entries_to_delete && is_array($history_entries_to_delete))
			{
				unset($history_entries_to_delete[count($history_entries_to_delete)-1]); // Do not remove the current importing entry.
				foreach ($history_entries_to_delete as $history_entry) {
					if(isset($history_entry['id'])){
						self::delete_history_by_id($history_entry['id']);
					}
				}
			}
	 	}
