        ...
        public function __construct()
	{
		...
		$this->steps=array(
			'post_type'=>array(
				...
				'description'=>__('Import the respective post type from a CSV/XML. As a first step you need to choose the post type to start the import.', 'wt-import-export-for-woo'),
			),
			'method_import'=>array(
			...
			'mapping'=>array(
				...
				'description'=>__('Map the standard/meta/attributes/taxonomies and hidden meta columns with your CSV/XML column names.', 'wt-import-export-for-woo'),
			),
			'advanced'=>array(
	...
	*/
        ...
        protected function evaluate_data($key, $value, $data_row, $mapping_fields, $input_date_format)
	{
		...
		$value = $this->add_input_file_data($key, $value, $data_row, $mapping_fields, $input_date_format,true);  
                ...
                if(isset($maping_key) && isset($data_row[$maping_key])){
...
//				return $str;
        ...
        public function get_skip_from_evaluation(){
            return apply_filters('wt_iew_importer_skip_from_evaluation', array('post_title', 'description','post_content','short_description','post_excerpt','line_item_','shipping_items', 'fee_items', 'customer_note', 'meta:wc_productdata_options'));             
        }
