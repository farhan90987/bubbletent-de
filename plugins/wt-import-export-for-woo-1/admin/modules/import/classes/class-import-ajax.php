...
class Wt_Import_Export_For_Woo_Import_Ajax
{
	...
	public function __construct($import_obj, $to_import, $steps, $import_method, $selected_template, $rerun_id, $cron_edit_id)
	{
	...
	*/
	public function get_meta_mapping_fields($out)
	{
		...
		{
			if(empty($this->import_obj->form_data) && 0 === $this->cron_edit_id){
			...
			foreach($meta_mapping_screen_fields as $meta_mapping_screen_field_key=>$meta_mapping_screen_field_val)
			{
				...
				if(count($file_heading_meta_fields)>0)
				{
					...
					foreach($file_heading_meta_fields as $key=>$sample_val)
					{
						...
						if($is_include==1)
						{
							$label=Wt_Iew_Sh_Pro::sanitize_item($key);
							$description=$this->prepare_field_description($key);
							$type='';
							$val='{'.$key.'}';
							$checked=1; /* import this column? */
	...
	*/
	public function method_import_page()
	{
		...
		if($this->to_import!="")
		{
			...
			$this->prepare_step_footer_html();
			$page_html_out = ob_get_clean();
			return Wt_Iew_IE_Helper::sanitize_and_minify_html($page_html_out);
		}else
		{
			return '';
		}
		...
		if($this->to_import!="")
		{
			...
			if($temp_import_file!="" && file_exists($file_path))
			{
				...
				if(isset($this->import_obj->allowed_import_file_type[$ext])) /* file type is in allowed list */ 
				{
					...
					$csv_delimiter=(isset($form_data['method_import_form_data']['wt_iew_delimiter']) ? $form_data['method_import_form_data']['wt_iew_delimiter'] : ',');
					...
					{
						include_once WT_IEW_PLUGIN_PATH.'admin/classes/class-csvreader.php';
						$delimiter=(isset($_POST['delimiter']) ? ($_POST['delimiter']) : $csv_delimiter); //no sanitization 
						if( 'tsv' == $ext ){
							$delimiter = "\t";
						}
						$reader=new Wt_Import_Export_For_Woo_Csvreader($delimiter);
	...
	*/
	protected function get_template_form_data($id)
	{
		...
		if($template_data)
		{
			...
			$decoded_form_data = apply_filters( 'wt_iew_importer_saved_template_data', $decoded_form_data );
		...
		if($current_index!==false) /* step exists */
		{
			...
			if($this->import_method=='quick' || $this->import_method=='template' || $last_page) //template method, or last page, or quick import
			{
				...
				$out['download']=array(
					...
					'icon'=>'',
					'type'=>'button',
					'text'=>__('Import', 'wt-import-export-for-woo'),
				);
			}
		}		
	...
	*/
	protected function _prepare_for_quick($import_id)
	{
		...
		if( 'xml' == strtolower(end($ext_arr)) ) /* no chance for XML. anyway we are not skipping the future possibilities */
		{
			include_once WT_IEW_PLUGIN_PATH.'admin/classes/class-xmlreader.php';
			$reader=new Wt_Import_Export_For_Woo_Xmlreader();

		}elseif( 'xls' == strtolower(end($ext_arr)) || 'xlsx' == strtolower(end($ext_arr)) )
		{
			include_once WT_IEW_PLUGIN_PATH.'admin/classes/class-excelreader.php';
			$reader=new Wt_Import_Export_For_Woo_Excelreader();
		}
