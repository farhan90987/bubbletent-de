...
class Wt_Import_Export_For_Woo_Cron
{
	...
	public function __construct()
	{
		...
		self::$status_label_arr=array(
			0=>__('Not started', 'wt-import-export-for-woo'),
			1=>__('Finished', 'wt-import-export-for-woo'),
			2=>__('Disabled', 'wt-import-export-for-woo'),
			3=>__('Running', 'wt-import-export-for-woo'),
			4=>__('Uploading', 'wt-import-export-for-woo'),
			5=>__('Downloading', 'wt-import-export-for-woo'),
		);
	...
	*/
	public function advanced_setting_fields($fields)
	{
		$fields['default_time_zone']=array(
			'label'=>__("Switch to website timezone", 'wt-import-export-for-woo'),
			'type' => 'checkbox',
			'checkbox_fields' => array( 1 => '' ),
			'value' => 0,
			'field_name' => 'default_time_zone',
			'css_class'=> 'wt_iew_checkbox_toggler wt_ier_toggler_blue',
			'help_text'=>__("Turn on to switch to your wesbite\'s timezone (local timezone). By default the timezone will be in UTC.", 'wt-import-export-for-woo'),
	...
	*/
	public function add_admin_pages($menus)
	{
		$menus[$this->module_base]=array(
			'submenu',
			WT_IEW_PLUGIN_ID,
			__('Scheduled Actions', 'wt-import-export-for-woo'),
			__('Scheduled Actions', 'wt-import-export-for-woo'),
	...
	{		
		foreach ($this->action_modules as $key => $value) 
		{
			...
			{ 
				...
				{
		            $start_time=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("now +1 minutes");
	...
	*/
	...
	{		
		...
		return apply_filters('wt_iew_cron_interval_details', $schedules);
	}
	...
	public function enqueue_assets()
	{
		if(isset($_GET['page']))
		{
			if($_GET['page']==Wt_Import_Export_For_Woo::get_module_id('export') || $_GET['page']==Wt_Import_Export_For_Woo::get_module_id('import') || $_GET['page']==$this->module_id)
			{
				...
				$params=array(
					'msgs'=>array(
						'invalid_date'=>__('Chosen date is invalid', 'wt-import-export-for-woo'),
						'date_selected_info'=>__(sprintf('You have selected %d as the date but this date is not available in all months. In that case, last date of the month will be taken. Proceed?', 30), 'wt-import-export-for-woo'),
						'specify_file_name'=>__('Please specify a file name.', 'wt-import-export-for-woo'),
						'saving'=>__('Saving', 'wt-import-export-for-woo'),
						'sure'=>__('Are you sure?', 'wt-import-export-for-woo'),
						'invalid_custom_interval'=>__('Please enter a valid interval.', 'wt-import-export-for-woo'),
                        'invalid_time_hr'=>__('Please enter a valid time in hours(1-12).', 'wt-import-export-for-woo'),
                        'invalid_time_mnt'=>__('Please enter a valid time in minutes(0-60).', 'wt-import-export-for-woo'),
						'use_url'=>__('Use the generated URL to run cron.', 'wt-import-export-for-woo'),
					),
		...
		foreach($step_btns as $step_btnk=>$step_btnv)
		{
			if($step_btnk=='download') /* in import download is the primary step before import */
			{
				$out['import_schedule']=array(
					...
					'text'=>__('Import/Schedule', 'wt-import-export-for-woo'), 
					'items'=>array(
						$step_btnk=>$step_btnv,
						'schedule'=>array(
							'key'=>'schedule_import',
							'text'=>__('Schedule', 'wt-import-export-for-woo'), //popups
						)
					)
				);
			}else
		...
		*/
		...
		{ 
			...
			foreach($cron_list as $cron_listv) 
			{
				...
				*/			
				$update_data=array(
						'last_run'=> Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz('now'),
	...
	*/
	private static function prepare_start_time($cron_data, $last_start_time=0)
	{
		...
		if($cron_data['interval']=='month')
		{
			...
			{
				$time_stamp=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("$time_vl Last day of +0 Month");
				if($time_stamp<$tme)
				{
					$out=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("$time_vl Last day of +1 Month");
				}else
				{
					$out=$time_stamp;	
				}
			...
			{		
				...
				$time_stamp=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("$time_vl $y-$M-$date_vl");
				if($time_stamp<$tme)
				{ 
					$out=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("+1 Month", $time_stamp);
				}else
				{
					$out=$time_stamp;
				}
		...
		}elseif($cron_data['interval']=='week')
		{
			...
			$time_stamp=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("This week $day_vl $time_vl");
			if($time_stamp<$tme)
			{
				$out=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("Next week $day_vl $time_vl");
			}else
			{
				$out=$time_stamp;	
			}
		...
		{
			$time_stamp=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz($time_vl);
			if($time_stamp<$tme)
			{
				$out=Wt_Import_Export_For_Woo_Common_Helper::wt_strtotimetz("+1 day $time_vl");
			}else
			{
				$out=$time_stamp;
			}
	...
	*/
	...
	{
		global $wpdb;
		$cron_data=(isset($_POST['schedule_data']) ? Wt_Iew_Sh_Pro::sanitize_item($_POST['schedule_data'], 'text_arr') : null );
		if(!$cron_data)
		{
			return $out;
		}
	...
	public function edit_schedule( $out ) {
		...
		global $wpdb;				
		$cron_id = (isset($_POST['cron_id']) ? Wt_Iew_Sh_Pro::sanitize_item($_POST['cron_id'], 'int') : null );
		if(!$cron_id)
		{
			return $out;
		}
