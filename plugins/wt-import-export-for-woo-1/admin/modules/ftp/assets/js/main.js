var wt_iew_ftp=(function( $ ) {
	...
	var wt_iew_ftp=
	{
		...
		toggle_ftp_path:function(file_into)
		{
			...
			$('[name="wt_iew_use_default_path"]').unbind('click').click(function(){
				wt_iew_ftp.ftp_custom_path_toggle();
			});

			jQuery('[name="wt_iew_ftp_profile"]').unbind('change').on('change',function(){
		...
		{			
			$('.wt_iew_ftp_profiles').unbind('click').click(function(){
		...
		regEvents:function()
		{
			$('.wt_iew_ftp_edit').click(function(){
			...
			$('.wt_iew_sub_tab li[data-target="ftp-profiles"]').click(function(){
				wt_iew_ftp.switchTab(wt_iew_ftp_params.msgs.add_new, wt_iew_ftp_params.msgs.add_new_hd,false);	
			});

			$('.wt_iew_sub_tab li[data-target="add-new-ftp"]').click(function(){
			...
			$('.wt_iew_ftp_delete').click(function(){
			...
			if(this.poPupCrud==1)
			{
				$('.wt_iew_ftp_use').unbind('click').click(function(){
		...
		updateSelectBox:function()
		{
			...
			if($('.ftp_list_tb').length>0)
			{
				...
				$('.ftp_list_tb').find('.wt_iew_data_dv').each(function(){
		...
		{
			$('.wt_iew_abort_test_ftp').unbind('click').click(function(){
		...
		testFtp:function()
		{
			$('.wt_iew_test_ftp_form').unbind('click').click(function(){
				var ftp_form=$('#wt_iew_ftp_form');
				var er=0;
				ftp_form.find('input[type="text"], input[type="number"], input[type="password"]').each(function(){
				...
				if(er==0)
				{
					ftp_form.find('input[type="radio"]').each(function(){
		...
		saveData:function()
		{
			$('#wt_iew_ftp_form').submit(function(e){
				e.preventDefault();
