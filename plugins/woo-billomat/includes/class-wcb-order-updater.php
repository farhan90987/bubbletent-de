...
class WCB_Order_Updater
{
    ...
    private function init_hooks()
    {
        if ('yes' != get_option('wcb_disable_invoice_creation') && get_option('wcb_connected')) {
            add_action('woocommerce_order_status_pending', array($this, 'export_create_or_send_invoice'), 20, 1);
            add_action('woocommerce_order_status_on-hold', array($this, 'export_create_or_send_invoice'), 20, 1);
            add_action('woocommerce_order_status_processing', array($this, 'export_create_or_send_invoice'), 20, 1);
            add_action('woocommerce_order_status_completed', array($this, 'export_create_or_send_invoice'), 20, 1);

            add_action('woocommerce_order_status_cancelled', array($this, 'export_cancel_invoice'), 20, 1);
            add_filter('woocommerce_email_attachments', array($this, 'email_attachments'), 20, 3);
    ...
    public function export_create_invoice($order, $add_payment = true)
    {
        ...
        if ($order_status !== $invoice_creation_order_status) {
            return;
        }
    ...
    private function send_invoice($order)
    {
        ...
        if ($order->get_status() == $invoice_mail_status) {
            ...
            if ($order_data['billing']['email']) {
                ...
                WCB()->client->mail_invoice($billomat_invoice['invoice_id'], $email_data, array("Order ID" => $order->get_id()));
            }
        }
    }
        ...
        switch ($status) {
            ...
            case 'PAID':
                ...
                if ($complete_orders_status && $complete_orders_status != 'none') {
                    if ($order && $order->get_status() != $complete_orders_status) {
    ...
    public function email_attachments($attachments, $status, $order)
    {
        ...
        // Do nothing if status don't match
        if ($order && $order->get_status() !== $invoice_mail_status) {
            return $attachments;
        }
