...
function _Op_DepItem_Process( $dependItem, $cb, $obj = null, $url = '', $priorityChange = 0 )
{
	if( !$dependItem )
	{
		$aUrl = array();
	...
	{
		foreach( $p as $pi )
			if( _Op_DepItem_Process( array_merge( $pi, $dependItem ), $cb, $obj, $url, $priorityChange ) === false )
				return( false );
		return( null );
	}

	if( ($p -> name??null) === null )
		return( _Op_DepItem_Process( $dependItem, $cb, $obj, $url . ($p -> args[ 0 ]??null), $priorityChange ) );
	...
	case 'IF':
		...
		if( !is_array( ($dependItem[ 0 ]??null) ) )
			return( $isTrue ? _Op_DepItem_Process( $dependItem, $cb, $obj, $url, $priorityChange ) : null );

		$pi = array_shift( $dependItem );
		$pi = ($pi[ $isTrue ? 0 : 1 ]??null);
		if( !is_array( $pi ) )
			$pi = array();
		return( _Op_DepItem_Process( array_merge( $pi, $dependItem ), $cb, $obj, $url, $priorityChange ) );
	...
	case 'P':
		...
		return( _Op_DepItem_Process( $dependItem, $cb, $obj, $url, $priorityChange ) );

	case 'home':
		return( _Op_DepItem_Process( $dependItem, $cb, $obj, Wp::GetSiteRootUrl( '', false ), $priorityChange ) );
	...
	case 'path':
		...
		return( _Op_DepItem_Process( $dependItem, $cb, $obj, Net::UrlDeParse( $urlComps ), $priorityChange ) );

	case 'arg':
		return( _Op_DepItem_Process( $dependItem, $cb, $obj, _Op_DepItem_UrlAddArgs( $url, array( rawurldecode( _Op_DepItem_Process_GetValFromObj( $obj, ($p -> args[ 0 ]??null) ) ) => rawurldecode( _Op_DepItem_Process_GetValFromObj( $obj, ($p -> args[ 1 ]??null) ) ) ) ), $priorityChange ) );
	...
	case 'commentPageNums':
		...
		if( !$aPagesNum )
			return( _Op_DepItem_Process( $dependItem, $cb, $obj, str_replace( '99999', '*', $url ), $priorityChange ) );
		...
		for( $iPage = $aPagesNum[ 0 ]; $iPage <= $aPagesNum[ 1 ]; $iPage++ )
			if( _Op_DepItem_Process( $dependItem, $cb, $obj, str_replace( '99999', ( string )$iPage, $url ), $priorityChange ) === false )
				return( false );
	...
	case 'post':
		...
		foreach( _Op_DepItem_Process_GetValsFromObj( $obj, ($p -> args[ 0 ]??null) ) as $postId )
		{
			...
			$res = _Op_DepItem_Process( $dependItem, $cb, $post, get_permalink( $post ), $priorityChange );
	...
	case 'posts':
		return( _Op_DepItem_Process_Posts( _Op_DepItem_Process_GetValsFromObj( $obj, ($p -> args[ 0 ]??null) ), $dependItem, $cb, $obj, ($p -> args[ 1 ]??null), $priorityChange ) );
	...
	case 'postsViewable':
		...
		return( _Op_DepItem_Process_Posts( $postTypes, $dependItem, $cb, $obj, ($p -> args[ 0 ]??null), $priorityChange ) );
	...
	case 'postsWithTerms':
		...
		for( $i = 0; ; $i++ )
		{
			...
			{
				if( _Op_DepItem_Process( $dependItem, $cb, $item, get_permalink( $item -> ID ), $priorityChange ) === false )
					return( false );
	...
	case 'terms':
		...
		if( $postId )
		{
			...
			foreach( $terms as $term )
				if( _Op_DepItem_Process( $dependItem, $cb, array( $term, $obj ), get_term_link( $term ), $priorityChange ) === false )
					return( false );
			if( _Op_DepItem_EnumTermsParents( $dependItem, $cb, $obj, $taxonomies, $terms, $priorityChange ) === false )
				return( false );
		}
		else
		{
			...
			for( $i = 0; ; $i++ )
			{
				...
				foreach( $terms as $term )
				{
					...
					if( _Op_DepItem_Process( $dependItem, $cb, $term, get_term_link( $term -> term_id ), $priorityChange ) === false )
						return( false );
				}
				...
				foreach( Gen::GetArrField( Wp::GetPostsTaxonomiesByClass( $taxonomyClass, array( 'public' => true, 'hasRewriteSlug' => true, 'postType' => $postType ) ), array( $postType ), array() ) as $taxonomy )
				{
					...
					if( $postId )
					{
						...
						foreach( $terms as $term )
							if( _Op_DepItem_Process( $dependItem, $cb, array( $term, $obj ), get_term_link( $term ), $priorityChange ) === false )
								return( false );
						if( _Op_DepItem_EnumTermsParents( $dependItem, $cb, $obj, $taxonomy, $terms, $priorityChange ) === false )
							return( false );
					}
					else
					{
	...
	case 'postsBase':
		...
		foreach( $postTypes as $postType )
		{
			if( function_exists( 'get_post_type_archive_link' ) )
			{
				...
				if( $link && _Op_DepItem_Process( $dependItem, $cb, $obj, $link, $priorityChange ) === false )
					return( false );
			}
			else if( $postType == 'post' )
			{
				if( _Op_DepItem_Process( $dependItem, $cb, $obj, get_permalink( get_option( 'page_for_posts' ) ), $priorityChange ) === false )
					return( false );
			}
		}
	...
	case 'author':
		...
		foreach( $aId as $idAuthor )
			if( _Op_DepItem_Process( $dependItem, $cb, $obj, get_author_posts_url( $idAuthor ), $priorityChange ) === false )
				return( false );
	...
	case 'sitemapItems':
		...
		if( _Op_DepItem_EnumSitemapUrls( $dependItem, $cb, ( strpos( $sitemapUri, '://' ) === false ) ? _Op_DepItem_UrlAddPath( Wp::GetSiteRootUrl(), $sitemapUri ) : $sitemapUri, $priorityChange, $sitemapUri ) === false )
			return( false );
	...
	case 'wooProdVars':
		...
		foreach( $variations as $variation )
		{
			...
			{
				if( $attrs && _Op_DepItem_Process( $dependItem, $cb, $item, _Op_DepItem_UrlAddArgs( $url, $attrs ), $priorityChange ) === false )
					return( false );
	...
	case 'iter':
		...
		for( $i = ( int )_Op_DepItem_Process_GetValFromObj( $obj, ($p -> args[ 1 ]??null) ); $i < $n; $i += $nStep )
			if( _Op_DepItem_Process( $dependItem, $cb, $obj, $url . $i, $priorityChange ) === false )
				return( false );
		return( null );
	}
...
function _Op_DepItem_Process_Posts( $postTypes, $dependItem, $cb, $obj, $argMaxItems, $priorityChange )
{
...
function _Op_DepItem_EnumSitemapUrls( $dependItem, $cb, $urlSitemap, $priorityChange, $tryLocalUri = null )
{
	...
	foreach( $doc -> getElementsByTagName( 'sitemap' ) as $child )
		foreach( $child -> getElementsByTagName( 'loc' ) as $loc )
			if( _Op_DepItem_EnumSitemapUrls( $dependItem, $cb, $loc -> nodeValue, $priorityChange ) === false )
				return( false );

	foreach( $doc -> getElementsByTagName( 'url' ) as $child )
		foreach( $child -> getElementsByTagName( 'loc' ) as $loc )
			if( $loc -> tagName == 'loc' && _Op_DepItem_Process( $dependItem, $cb, null, $loc -> nodeValue, $priorityChange ) === false )
				return( false );

	return( null );
}

function _Op_DepItem_EnumTermsParents( $dependItem, $cb, $obj, $taxonomies , $terms, $priorityChange, array &$alreadyProcessed = array() )
{
	$termIds = array();
	foreach( $terms as $term )
	{
	...
	if( _Op_DepItem_EnumTermsParents( $dependItem, $cb, $obj, $taxonomies, $terms, $priorityChange, $alreadyProcessed ) === false )
		return( false );
...
function Op_DepItem_Process( $dependItem, $cb, $obj = null )
{
	...
	return( _Op_DepItem_Process( $dependItem, $cb, $obj, $url ) );
}
...
function CacheOpGetViewsHeaders( $settCache, $viewId = null )
{
	...
	foreach( $viewId === null ? array( 'cmn' ) : $viewId as $viewIdI )
		if( CacheOpViewsHeadersGetViewId( $viewIdI ) == 'cmn' )
			$res[ $viewIdI ] = array( 'User-Agent' => 'Mozilla/99999.9 AppleWebKit/9999999.99 (KHTML, like Gecko) Chrome/999999.0.9999.99 Safari/9999999.99 seraph-accel-Agent/2.27.48' );
