...
final class Smoobu_Settings {
	...
	private function save_general_settings() {
		if (
			...
			wp_verify_nonce( sanitize_key( $_POST['smoobu_settings_nonce'] ), 'smoobu_settings_nonce' )
		) {
			if ( isset( $_POST['smoobu_general_settings_save'] ) ) {
				...
				return array(
					'success' => true,
					'message' => __( 'API key was saved and data updated.', 'smoobu-calendar' ),
				);
			}
		}
...
function smoobu_refresh_data() {
    ...
    if ( false === $check_result ) {
        ...
        if ( ! empty( $properties_error ) || ! empty( $availability_error ) || ! empty( $user_error ) ) {
            ...
            $message = 'API key was saved, but we encountered an error while trying to update properties & availability information: ' . $error;
        } else {
            $message = 'API key was saved and properties list & availability were successfully updated.';
        }
    } else {
        $message = 'Settings saved';
    }

    $to      = 'noman@mathesconsulting.de';
    $subject = 'Smoobu Refresh Executed';

    wp_mail( $to, $subject, $message );
}

// Hook function into cron
add_action( 'smoobu_refresh_api_data', 'smoobu_refresh_data' );