...
class LicenseHandler {
    ...
    <div class="error">     
        ...
        printf( esc_html__( '%1$s license key is required. %2$sPlease enter your license key to start using the plugin%3$s.', 'yaymail' ), 'YayMail', '<a href="' . esc_url( admin_url( 'admin.php?page=yaycommerce-licenses' ) ) . '">', '</a>' );
        ?>
        </p>
    </div>
        <?php
    ...
    public static function get_plugin_data( $plugin_info = [] ) {
        $plugin_data = [
            'Name'       => $plugin_info['name'] ?? 'YayMail Pro - WooCommerce Email Customizer',
    ...
    public static function get_licensing_plugins() {
        ...
        $plugins     = [
            [
                ...
                'slug'     => CorePlugin::get( 'slug' ),
                'dir_path' => CorePlugin::get( 'path' ),
                'basename' => CorePlugin::get( 'basename' ),
                'file'     => CorePlugin::get( 'path' ) . basename( CorePlugin::get( 'basename' ) ),
                'url'      => CorePlugin::get( 'link' ),
                'item_id'  => CorePlugin::get( 'download_id' ),
            ],
        ];
