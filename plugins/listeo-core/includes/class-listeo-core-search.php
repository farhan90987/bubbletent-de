...
public function ajax_get_listings() {
		...
		$listing_reigon   	= (isset($_REQUEST['listing_reigon'])) ?  sanitize_text_field(  $_REQUEST['listing_reigon']  ) : '';
...
// 
		...
		<form action="<?php echo $action; ?>" id="listeo_core-search-form" class="listeo-form-<?php echo esc_attr($source); if($dynamic_filters == 'on') { echo esc_attr(' dynamic'); }  ?> <?php if($dynamic_taxonomies == 'on') { echo esc_attr('dynamic-taxonomies'); }  ?>  <?php echo esc_attr($custom_class) ?> <?php echo esc_attr($ajax) ?>" method="GET">
		<?php } 
			...
			foreach ($search_fields as $key => $value) {
				...
				if(isset($value['place']) && $value['place'] == 'main') {
					...
					if($form_type == 'split') {
						...
						if($form_type == 'sidebar') { echo '<div class="row with-forms" id="listeo-search-form_'.$value['name'].'">'; }
							$template_loader->set_template_data( $value )->get_template_part( 'search-form/'.$value['type']);
						if($form_type == 'sidebar') { echo '</div>'; }
