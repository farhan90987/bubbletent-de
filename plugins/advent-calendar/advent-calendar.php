...
class Advent_Calendar_Manager {
    ...
    private function send_verification_email($post_id) {
        ...
        $subject = __('Verify Your Advent Calendar Entry', 'advent-calendar');
        $body = "
        <p>" . sprintf(__('Hi %s', 'advent-calendar'), esc_html($name)) . ",</p>
        <p>" . __('Thank you for submitting your entry to our Advent Calendar!', 'advent-calendar') . "</p>
        <p>" . __('Please verify your email address by clicking the link below:', 'advent-calendar') . "</p>
        <p><a href='" . esc_url($verification_url) . "' style='background:#007cba; color:white; padding:12px 24px; text-decoration:none; border-radius:4px; display:inline-block;'>" . __('Verify Your Email', 'advent-calendar') . "</a></p>
        <p>" . __('Or copy and paste this URL in your browser:', 'advent-calendar') . "<br><code>" . esc_url($verification_url) . "</code></p>
        <p>" . __('If you didn\'t submit this entry, please ignore this email.', 'advent-calendar') . "</p>
		...
		if(isset($_GET['ac_verify']) && isset($_GET['entry_id'])){
			if ($stored_token === $token) {
				if (!$is_verified) {
					...
					// Set success message
					set_transient('ac_verification_success', __('Your email has been successfully verified! You are now entered in the Advent Calendar.', 'advent-calendar'), 60);
				} else {
					set_transient('ac_verification_info', __('Your email was already verified.', 'advent-calendar'), 60);
				}
			} else {
				set_transient('ac_verification_error', __('Invalid verification link. Please try again or request a new verification email.', 'advent-calendar'), 60);
			}	
		}
    }
    ...
    public function ajax_resend_verification_email() {
        ...
        if (!$entry_id) {
            wp_send_json_error(['message' => __('Invalid entry ID.', 'advent-calendar')]);
            wp_die();
        }

        $email = get_post_meta($entry_id, '_ac_email', true);
        if (!$email) {
            wp_send_json_error(['message' => __('Entry not found.', 'advent-calendar')]);
            wp_die();
        }
        ...
        if ($this->send_verification_email($entry_id)) {
            wp_send_json_success(['message' => __('Verification email sent! Please check your inbox.', 'advent-calendar')]);
        } else {
            wp_send_json_error(['message' => __('Failed to send verification email. Please try again.', 'advent-calendar')]);
        }
    ...
    public function render_advent_calendar() {
        ...
        echo '<h1>';
        esc_html_e('Use your chance and win a magical night in the Bubble Tent', 'advent-calendar');
        echo '</h1>';
        ...
        <div class="ac-popup" id="ac-popup" role="dialog" aria-hidden="true">
            ...
            <div class="ac-popup-content">
                ...
                <div class="ac-popup-container">
                    <p><?php esc_html_e('Your door for today is open', 'advent-calendar'); ?></p>
                    <h3><?php esc_html_e('Use your chance and win a magical night in the Bubble Tent.', 'advent-calendar'); ?></h3>
                    <img class="tentform-img" src="<?php echo plugin_dir_url(__FILE__); ?>assets/img/tent-form.png" alt="">
                    <p><?php esc_html_e("This is what's in today's advent calendar:",'advent-calendar') ?></p>
                    <p id="ac-day-specific-text" class="ac-day-specific-text"></p>
                    <h3><?php esc_html_e('Just one more small step:', 'advent-calendar'); ?></h3>
                    <p><?php esc_html_e('Enter your data and get into the draw. Tomorrow we will tell you if luck was on your side.', 'advent-calendar'); ?></p>
                    <form id="ac-entry-form">
                        <input type="hidden" name="day" id="ac-day">
                        <p class="input-col">
                            <input type="text" name="first_name" placeholder="<?php esc_attr_e('First Name', 'advent-calendar'); ?>" required>
                            <input type="text" name="last_name" placeholder="<?php esc_attr_e('Last Name', 'advent-calendar'); ?>" required>
                        </p>
                        <p>
                            <input type="email" name="email" placeholder="<?php esc_attr_e('E-Mail', 'advent-calendar'); ?>" required>
                        </p>
                        <p>
                            <button type="submit"><?php esc_html_e('SUBMIT', 'advent-calendar'); ?></button>
                        </p>
                    </form>
                </div>
            </div>
    ...
    public function ajax_submit_entry() {
        ...
        if (!$day || $day < 1 || $day > 24) {
            wp_send_json_error(['message' => __('Invalid day selected.', 'advent-calendar')]);
            wp_die();
        }
        if (!$first_name || !$last_name || !$email) {
            wp_send_json_error(['message' => __('Please fill in all required fields.', 'advent-calendar')]);
            wp_die();
        }

        // Check if day is active (enabled + date is today)
        if (!$this->is_day_active($day)) {
            wp_send_json_error(['message' => sprintf(__('Day %d is not active today.', 'advent-calendar'), $day)]);
            wp_die();
        }
        ...
        if ($existing) {
            ...
            if ($is_verified) {
                wp_send_json_error(['message' => __('You have already submitted a verified entry for this day.', 'advent-calendar')]);
            } else {
                wp_send_json_error(['message' => __('You have already submitted an entry for this day. Please check your email to verify your entry.', 'advent-calendar')]);
            }
            wp_die();
        }
        ...
        if (!$post_id) {
            wp_send_json_error(['message' => __('Error creating entry.', 'advent-calendar')]);
            wp_die();
        }
        ...
        if ($email_sent) {
            wp_send_json_success([
                'message' => __('Your participation has been submitted! Please check your emails to confirm your address. You must confirm your email to be eligible for the prize.', 'advent-calendar'),
                'entry_id' => $post_id
            ]);
        } else {
            wp_send_json_error(['message' => __('Your entry was created but we could not send the confirmation email. Please contact support.', 'advent-calendar')]);
        }
    ...
    private function get_winner_subject($entry_id) { 
        ...
        if ($language === 'de') {
            $subject = '🎉 Sie sind der heutige Gewinner!';
        } elseif ($language === 'en') {
    ...
    private function get_loser_subject($entry_id) { 
        ...
        if ($language === 'de') {
            $subject = 'Vielen Dank für Ihre Teilnahme an unserem Adventskalender!';
        } elseif ($language === 'en') {
            $subject = 'Thanks for participating in our Advent Calendar!';
        }else{
    ...
    private function get_winner_email_body($entry_id, $coupon_code, $name = '') {
        ...
        // Path of template
        if($language === 'de'){
            $template = __DIR__ . '/email/winner-email-de.php';
        }elseif($language === 'en'){
            $template = __DIR__ . '/email/winner-email-en.php';
        }else{
    ...
    private function get_loser_email_body($entry_id, $coupon_code, $name = '') {
        ...
        if($language === 'de'){
            $template = __DIR__ . '/email/loser-email-de.php';
        }elseif($language === 'en'){
            $template = __DIR__ . '/email/loser-email-en.php';
        }else{
