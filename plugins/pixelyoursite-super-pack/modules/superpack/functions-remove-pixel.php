...
function saveRemovePixelMetaBox( $post_id, $data ) {
	...
	// Reddit
	if ( isset( $data['pys_super_pack_remove_reddit_pixel'] ) ) {
		update_post_meta( $post_id, '_pys_super_pack_remove_reddit_pixel', $data['pys_super_pack_remove_reddit_pixel'] );
	} else {
		delete_post_meta( $post_id, '_pys_super_pack_remove_reddit_pixel' );
	}
...
function maybeRemovePixel( $remove, $context ) {
    ...
    if(is_object($current_page) && property_exists($current_page, 'post_type') && ($current_page->post_type != 'archive' && $current_page->post_type != 'category'))
    {
        switch ( $context ) {
	        ...
	        case 'reddit':
		        if ($current_page) {
			        return (array) get_post_meta($current_page->ID, '_pys_super_pack_remove_reddit_pixel', true);
		        }
		        break;
