        ...
        if ((typeof wpwoof_current_page !== 'undefined')) {
            ...
            switch (wpwoof_current_page) {
                case 'dashboard':
                    ...
                    $(document).on('change', '.do_filter_categories', function () {
                        filter_categories();
                    });
                    filter_categories();
...
//            row.append($("<span>", { class:"extra-link-2-wrapper"}).html(wpwoof_help_links[tagName]));
            ...
            var $linkReddit = $('<div>', {class: "extra-link__item"});
            $linkReddit.append($("<input>", {
                id: "wpfoof-box-media[extra]" + loopStr + "[" + tagName + "][feed_type][reddit]",
                name: "wpfoof-box-media[extra]" + loopStr + "[" + tagName + "][feed_type][reddit]",
                type: "checkbox",
                checked: "checked"
            }));
            $linkReddit.append($("<label>", {for: "wpfoof-box-media[extra]" + loopStr + "[" + tagName + "][feed_type][reddit]"}).html('Reddit'));
            $linkWrap.append($linkReddit);
    ...
    function filter_categories() {
        //$('.wpwoof-popup-wrap').first().show();
        var lang = $("select[name='feed_use_lang']").val();
        if (!lang) lang = 'all';
        let is_not_default_lang = lang !== 'all' && lang !== wpwoof_wpml_default_lang;

        if (is_not_default_lang) {
            $("#feed-setting-translations-row").show();
        } else {
            $("#feed-setting-translations-row").hide();
        }

        var translatable_fallback = is_not_default_lang && $("#translatable_fallback").is(":checked");
        $("#lang_wpwoof_categories li.language_all").each(function () {
            var elm = $(this);
            var hide_cat = translatable_fallback && !elm.hasClass('translated_category');
            if (elm.hasClass('language_' + lang) && !hide_cat) {
                if (translatable_fallback) {
                    $('span.default-category-name', elm).show();
                } else {
                    $('span.default-category-name', elm).hide();
                }
                elm.show();
            } else {
                elm.hide();
