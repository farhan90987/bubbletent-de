...
class FeedFBGooglePro {
	...
	public function wpwoof_render_taxonomy( $fieldkey, $field, $wpwoof_values ) {
		 ...
		 */
		?>
        <div class="stl-facebook stl-google stl-pinterest stl-tiktok stl-reddir">
...
//                                if (isset($field['canSetCustomValue']) && $field['canSetCustomValue'])
	...
	public function render_UTM_fields( $wpwoof_values ) {
		?>
        <div class="stl-facebook stl-google stl-pinterest stl-adsensecustom stl-tiktok stl-fb_country stl-google_local_inventory stl-reddit"
             style="display: block;">
            <hr class="wpwoof-break">
            <label>
            ...
            <table class="form-table  stl-facebook stl-google stl-pinterest stl-adsensecustom stl-tiktok wpwoof-addfeed-top stl-fb_localize stl-reddit">
                <tr>
                    <th>utm_source:</th>
                    <td>
	...
	public function render_product_type_options( $feed_settings ) {
		?>
        <div class="stl-facebook stl-google stl-pinterest stl-adsensecustom stl-tiktok stl-reddit" style="display: block;">
            <hr class="wpwoof-break">
            <label>
    ...
    /**
     * Renders the feed metadata section in the user interface. This allows the user
     * to specify feed-level metadata such as generation date, generator version,
     * or internal comments. The metadata can be applied in various formats, either
     * using custom namespaces or XML comments.
     *
     * @param array $wpwoof_values An array of values and settings used for rendering
     *                             the metadata form section, such as feed options
     *                             and custom user inputs.
     *
     * @return void Outputs the HTML content for configuring feed-level metadata.
     */
    public function render_feed_metadata( $wpwoof_values ) {
        ?>
        <div class="stl-facebook stl-google stl-pinterest stl-adsensecustom stl-tiktok stl-fb_localize stl-fb_country stl-google_local_inventory stl-reddit"
             style="display: block;">
            <hr class="wpwoof-break">
            <h4 class="wpwoofeed-section-heading">Metadata for this feed</h4>
            <br/><br/>
            <h4>Add feed-level metadata here, such as a generation date or an internal comment. This information applies
                globally to the entire file and does not affect how individual products are processed by external
                systems.</h4>
            <hr class="wpwoof-break"/>

            <label>
                <input name="add_metadata_to_feed" type="radio"
                       value="off"<?php checked( ! isset( $wpwoof_values['add_metadata_to_feed'] ) || ( isset( $wpwoof_values['add_metadata_to_feed'] ) && $wpwoof_values['add_metadata_to_feed'] === 'off' ) ) ?>>
                <h3 style="display: inline-block;">Do not add custom metadata</h3>
            </label>
            <br>

            <label>
                <input name="add_metadata_to_feed" type="radio"
                       value="namespace"<?php checked( ( ! empty( $wpwoof_values['add_metadata_to_feed'] ) && $wpwoof_values['add_metadata_to_feed'] === 'namespace' ) ) ?>>
                <h3 style="display: inline-block;">Add metadata using a custom namespace</h3>
            </label>
            <pre>    &lt;pcf:metadata&gt;
        &lt;pcf:generation_date&gt;2025-10-08 20:25:52&lt;/pcf:generation_date&gt;
        &lt;pcf:generator_version&gt;v2.7&lt;/pcf:generator_version&gt;
        &lt;pcf:comment&gt;Feed for autumn campaign.&lt;/pcf:comment&gt;
    &lt;/pcf:metadata&gt;</pre>
            <label>
                <input name="add_metadata_to_feed" type="radio"
                       value="comment"<?php checked( ( ! empty( $wpwoof_values['add_metadata_to_feed'] ) && $wpwoof_values['add_metadata_to_feed'] === 'comment' ) ) ?>>
                <h3 style="display: inline-block;">Add metadata as XML comments</h3>
            </label>
            <pre>   &lt;!-- metadata:
    generation_date="2025-10-08 20:25:52"
    generator_version="v2.7"
    comment="Feed for autumn campaign."
    --&gt;</pre>
            <br>

            <br>
            <h4>Comment</h4>
            <textarea name="feed_comment" rows="3" cols="57" placeholder="Comment for this feed"><?php
                if ( ! empty( $wpwoof_values['feed_comment'] ) ) {
                    echo esc_textarea( $wpwoof_values['feed_comment'] );
                }
                ?></textarea>
        </div>
        <?php
