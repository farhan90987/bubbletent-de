...
function LazyTaxonomyReader() {
    ...
    foreach ( $lines as $line ) {
        ...
        if ( count( $tarr ) > 1 ) {
            $val = end( $tarr );
            array_pop( $tarr ); // Remove last element

            // Build hierarchical structure using reference
            $current = &$categories;
            foreach ( $tarr as $category ) {
...
function wpwoofcategories_wmpl( $options ) {
    ...
    $default_language_code = icl_get_default_language();
    $default_language_cat_names = array();
...
//    trace( $options );
    ...
    <ul id="lang_wpwoof_categories">
        ...
        $default_cat_map = wpwoof_get_all_term_translations_map_to_default( 'product_cat' );

        //default lang must always be first
        if ( ! empty( $aLanguages ) ) {
            if ( isset( $aLanguages[ $default_language_code ] ) ) {
                $default_language_data = $aLanguages[ $default_language_code ];
                unset( $aLanguages[ $default_language_code ] );
                $aLanguages = [ $default_language_code => $default_language_data ] + $aLanguages;
            }
        }
        ...
        foreach ( $aLanguages as $lang ) {
            ...
            foreach ( $terms as $term ) {
                if ( $lang['language_code'] == $default_language_code && in_array( $term->term_id, $default_cat_map ) ) {
                    $default_language_cat_names[$term->term_id] = $term->name;
                }
                $excluded_car_text = in_array( $term->term_id, $excluded_cat_ids ) ? ' <span
  class="dashicons dashicons-warning warning-icon"
  title="This category is excluded in the category settings."
></span>' : "";
                if(isset($default_cat_map[$term->term_id]) && isset($default_language_cat_names[$default_cat_map[$term->term_id]])) {
                    $default_cat_text = ' <span class="default-category-name">('.$default_language_cat_names[$default_cat_map[$term->term_id]].')<span>';
                } else {
                    $default_cat_text = '';
                }
                ?>
                <li class="language_<?php echo $lang['language_code'] ?> language_all<?=isset($default_cat_map[$term->term_id])?' translated_category':''?>">
                    ...
                    <label for="<?php echo 'feed_category_' . $term->term_id; ?>"><?php echo $term->name ?>
                        &nbsp;(<?php echo $term->count; ?>)<?= $excluded_car_text.$default_cat_text ?></label>
                </li>
                <?php
            } ?>
            <?php
...
/**
 * Gets a map of all translated term IDs to their default language equivalent for a given taxonomy.
 *
 * @param string $taxonomy The taxonomy (e.g., 'product_cat').
 * @return array A map of [translated_term_id => default_lang_term_id].
 */
function wpwoof_get_all_term_translations_map_to_default( $taxonomy ) {
    ...
    // This single query joins the translation table to itself to create pairs of
    // (translated term) => (default language term) for the entire taxonomy.
    $results = $wpdb->get_results( $wpdb->prepare( "
        SELECT
            t_trans.element_id AS translated_id,
            t_default.element_id AS default_id
        FROM
            {$wpdb->prefix}icl_translations AS t_default
        JOIN
            {$wpdb->prefix}icl_translations AS t_trans ON t_default.trid = t_trans.trid
        WHERE
            t_default.element_type = %s
            AND t_trans.element_type = %s
            AND t_default.language_code = %s
            AND t_trans.language_code != %s
    ", $element_type, $element_type, $default_language, $default_language ) );

    if ( empty( $results ) ) {
        return [];
    }

    // Creates an associative array: [translated_id => default_id]
    return wp_list_pluck( $results, 'default_id', 'translated_id' );
}
...
></span>' : "";?>
            <li><input type="checkbox" value="<?php echo $term->term_id; ?>" name="feed_category[]"
                ...
                <label
                    ...
                    (<?php echo $term->count; ?>)<?=$excluded_car_text?></label></li>
        <?php } ?>
    </ul>
    <br>
    <div id="wpwoof-popup-bottom"><a href="#done" class="button button-secondary wpwoof-popup-done">Done</a></div>
