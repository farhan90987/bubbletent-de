...
class WPwoofGenerator {
	...
	private $_visibility = array( "publish" );
	...
	private $_wpwoof_custom_fields; // list of custom fields for mapping
	private $_aelia_currency_filter = null;
	private $_all_categories = array();
	private $_localized_feed_settings = null;
	private $_localized_main_product_id;
	private $_feed_settings_for_save;
	private $_feed_id;
	private $_feed_name;
	private $_feed_file_name;
	public $wpml_fallback_enabled = false;

	function __construct( $wpwoofeed_settings ) {
		global $woocommerce, $woocommerce_wpml, $sitepress, $wpWoofProdCatalog,
		       $woocommerce_wpwoof_common, $store_info, $wp_query, $wpdb, $post, $_wp_using_ext_object_cache;

		if ( is_int( $wpwoofeed_settings ) ) {
			$feed_config = wpwoof_get_feed( $wpwoofeed_settings );
			if ( is_wp_error( $feed_config ) ) {
				$woocommerce_wpwoof_common->remove_feed_gen_schedule( (int) $wpwoofeed_settings );
				$this->_feed_id = null;
				if ( WPWOOF_DEBUG ) {
					file_put_contents( $woocommerce_wpwoof_common->feedBaseDir . 'cron-wpfeed.log', date( "Y-m-d H:i:s" ) . "\tError loading feed. " . $feed_config->get_error_message() . "\n", FILE_APPEND );
				}

				return false;
			} else {
				$wpwoofeed_settings = $feed_config;
			}
		}
		$this->_feed_id                = $wpwoofeed_settings['edit_feed'];
		$this->_feed_name              = $wpwoofeed_settings['feed_name'];
		$this->_feed_file_name         = isset( $wpwoofeed_settings['feed_file_name'] ) ? sanitize_text_field( $wpwoofeed_settings['feed_file_name'] ) : strtolower( str_replace( ' ', '-', trim( $wpwoofeed_settings['feed_name'] ) ) );
		$this->_feed_settings_for_save = $wpwoofeed_settings;
		if ( $this->is_feed_localized( $wpwoofeed_settings ) ) {
			$this->_wpwoofeed_settings = wpwoof_get_feed( $wpwoofeed_settings['main_feed_id'] );
			if ( is_wp_error( $this->_wpwoofeed_settings ) ) {
				$woocommerce_wpwoof_common->remove_feed_gen_schedule( (int) $this->_feed_id );
				$this->_feed_id = null;

				return false;
			}

			$this->set_config_for_localized_feed( $wpwoofeed_settings );
		} else {
		...
		$this->_wpwoof_custom_fields      = wpwoof_get_custom_fields();

		if ( empty( $this->_feed_name ) || ! isset( $this->_wpwoofeed_settings['feed_type'] ) || ! isset( $this->_wpwoofeed_settings['field_mapping'] ) ) {
			exit( "Error format" );
		}
	...
	private function _storedebug( $obj, $mode = 0 ) {
		if ( ! WPWOOF_DEBUG ) {
			return false;
		}
		if ( $mode === 0 ) { // save only to our debug file
			wpwoofStoreDebug( $obj, $this->_wpwoofeed_wpml_debug );
		} elseif ( $mode === 1 ) { // seve only to error_log()
			error_log( $obj );
		} elseif ( $mode === 2 ) { // seve to error_log() and to our debug file
			wpwoofStoreDebug( $obj, $this->_wpwoofeed_wpml_debug );
			error_log( $obj );
		}
	}
	...
	private function getExludedCats( $isTags = false ) {
		$excluded = $include = array();
		if ( ! $isTags && ! empty( $this->_wpwoofeed_settings['feed_category_excluded'] ) ) {
			$excluded = $this->_wpwoofeed_settings['feed_category_excluded'];
		}

		if ( ! isset( $this->_wpwoofeed_settings["feed_include_excluded"] ) || ! $this->_wpwoofeed_settings["feed_include_excluded"] ) {
			//don`t exclude categories included in feed
			if ( empty( $this->_wpwoofeed_settings['feed_category_all'] ) && ! empty( $this->_wpwoofeed_settings['feed_category'] ) ) {
				$include = $this->_wpwoofeed_settings['feed_category'];
			}
	...
	private function getTermsWMPL( $isTag = false ) {
		...
		if ( count( $array_terms ) > 0 ) {
			foreach ( $array_terms as $idx => $term ) {
				if ( isset( $term->term_id ) ) {
					$array_terms[ $idx ]->{'meta_data'} = $this->getCategoryMetaData( $term->term_id );
				}
			}
		}
		...
		if ( ! in_array( $termID, $this->_aExcludedCats ) ) {
			foreach ( $this->_aTerms as $_term ) {
				if ( $_term->term_id == $termID ) {
					if ( isset( $_term->meta_data ) && ! empty( $_term->meta_data[ $metaKey ][0] ) ) {
						$result = $_term->meta_data[ $metaKey ][0];
					}
	...
	function _query_vars_filter( $vars ) {
		...
		remove_filter( 'posts_where', array( &$this, '_query_vars_filter' ), 99 );
	...
	private function _terminate_text( $text, $use_cdata = true, $notStripTags = false ) {
		...
		if ( ! empty( $text ) ) {
			...
			if ( ! $notStripTags ) {
				$text = preg_replace( '/[\x{0a}-\x{1f}]/u', '*', $text );
			}
			...
			if ( empty( $post_thumbnail_id ) ) {
				...
				if ( is_array( $attributes ) ) {
					foreach ( $attributes as $attr ) {
						if ( ! empty( $this->_wpwoofeed_settings["prod_images"] ) && is_array( $this->_wpwoofeed_settings["prod_images"] ) ) {
							foreach ( $this->_wpwoofeed_settings["prod_images"] as $key => $aimg ) {
								if ( $first_key === false ) {
									$first_key = $key;
								}
				...
				if ( ! $post_thumbnail_id && isset( $this->_wpwoofeed_settings["prod_images"][ $first_key ][0] ) ) {
					$post_thumbnail_id = $this->_wpwoofeed_settings["prod_images"][ $first_key ][0];
				}
			}
...
//        if($prodID==804) $this->_storedebug( $this->_metaData);
	...
	private function searchBrandValue() {
		foreach ( $this->_metaData as $prop => $vals ) {
			if ( stripos( $prop, 'brand' ) !== false && ! empty( $vals[0] ) && $prop != 'wpfoof-brand' ) {
		...
		switch ( $tag ) {
			...
			case 'description_short':
				...
				if ( empty( $description ) ) {
					...
					if ( empty( $description ) ) {
						$description = $this->_product_get_short_description( $mainpr );
					}
				}
			...
			case 'short_description':
				if ( isset( $this->_wpwoofeed_settings["field_mapping"]['add_short_description'] ) && empty( $this->_wpwoofeed_settings["field_mapping"]['add_short_description'] ) ) {
					return "";
				}
			...
			case 'link':
				...
				if ( isset( $this->_wpwoofeed_settings['feed_utm_enabled'] ) && $this->_wpwoofeed_settings['feed_utm_enabled'] ) {
					$this->_storedebug( 'feed_utm_enabled' );
					if ( isset( $this->_wpwoofeed_settings['feed_utm_type'] ) ) {
						switch ( $this->_wpwoofeed_settings['feed_utm_type'] ) {
							case 'customTemplate':
								if ( isset( $this->_wpwoofeed_settings['feed_utm_template'] ) ) {
									$params = array_map( 'rawurlencode', $this->_wpwoofeed_settings['feed_utm_template'] );
									$url    = add_query_arg( $params, $url );
								}
								break;
							case 'custom':
								$utm_params = isset( $this->_wpwoofeed_settings['feed_utm_custom'] ) ? $this->_wpwoofeed_settings['feed_utm_custom'] : '';
								$url        = ( strpos( $url, '?' ) !== false ) ? $url . $utm_params : $url . '?' . $utm_params;
								break;
							...
							default:
								$def_params = array(
									...
									"utm_medium"   => "Catalog",
									"utm_campaign" => $this->_wpwoofeed_type,
									"utm_content"  => $this->_woocommerce_wpwoof_common->feed_type_name[ $this->_wpwoofeed_settings['feed_type'] ],
									"utm_term"     => $this->_feed_name
								);
								$params     = array_map( 'rawurlencode', $def_params );
								$url        = add_query_arg( $params, $url );
								break;
						}
					}
			...
			case 'sku':
				if ( $this->_wpwoofeed_settings['feed_type'] === 'googleReviews' && empty( $this->_wpwoofeed_settings['add-sku'] ) ) {
					return '';
				}
			case '_sku':
				$data = $product->get_sku();
			...
			case 'product_type':
				...
				if ( isset( $this->_wpwoofeed_settings['feed_product_type_enabled'] ) && empty( $this->_wpwoofeed_settings['feed_product_type_enabled'] ) ) {
					return '';
				}

				if ( ! isset( $this->_wpwoofeed_settings['feed_product_type_option'] ) || $this->_wpwoofeed_settings['feed_product_type_option'] == 'woocommerce_categories' ) {

					$parent_ID = $this->_getParentID( $product );
					if ( ! $parent_ID ) {
						$parent_ID = $this->_get_id( $product );
					}
					$categories = wp_get_object_terms( $parent_ID, 'product_cat' );

					$primary_product_cat = false;
					if ( is_plugin_active( WPWOOF_YSEO ) ) {
						$primary_product_cat = $this->_getMeda( '_yoast_wpseo_primary_product_cat', $product );
					}
					...
					if ( ! is_wp_error( $categories ) ) {
						foreach ( $categories as $category ) {
							...
							while ( $cat->parent !== 0 && isset( $this->_all_categories[ $cat->parent ] ) ) {
								$cat                 = $this->_all_categories[ $cat->parent ];
								$categories_string[] = $cat->name;
							}
							krsort( $categories_string );
							if ( ! empty( $primary_product_cat ) && $primary_product_cat == $category->term_id ) {
//                            $cats[] = implode(' > ', $categories_string);
								array_unshift( $cats, implode( ' > ', $categories_string ) );
							} else {
								$cats[] = implode( ' > ', $categories_string );
							}
						}
					...
					return implode( ' , ', array_slice( $cats, 0, 5 ) );
				} elseif ( $this->_wpwoofeed_settings['feed_product_type_option'] == 'google_categories' ) {
					return $this->_wpwoofeed_google_product_category( $product, $tag, $tagid, $tag );
				} elseif ( $this->_wpwoofeed_settings['feed_product_type_option'] == 'custom' ) {
					// check by priority

					//1. check product
					$result = $this->_getMeda( 'wpfoof-product_type', $product );
					if ( ! empty( $result ) ) {
						$this->_storedebug( "product_type: product level" );

						return $result;
					}

					//2. check the main product
					$parent_ID = $this->_getParentID( $product );
					if ( ! empty( $parent_ID ) ) {
						$main_product = $this->_load_product( $parent_ID );
						if ( ! empty( $main_product ) ) {
							$result = $this->_getMeda( 'wpfoof-product_type', $main_product );
							if ( ! empty( $result ) ) {
								$this->_storedebug( "product_type: main product level" );
					...
					//3. check category
					$result = $this->_getCategoryMeta( $product, 'wpfoof-product_type' );
					if ( ! empty( $result ) ) {
						$this->_storedebug( "product_type: category level" );

						return $result;
					}

					//4. check feed settings
					$result = ! empty( $this->_wpwoofeed_settings['feed_product_type_custom'] ) ? $this->_wpwoofeed_settings['feed_product_type_custom'] : "";
					if ( ! empty( $result ) ) {
						$this->_storedebug( "product_type: feed level" );
			...
			case 'sale_price_effective_date':
				if ( isset( $this->_wpwoofeed_settings["product_tmp_data"]['prices']->sale_price_effective_date ) ) {
					return $this->_wpwoofeed_settings["product_tmp_data"]['prices']->sale_price_effective_date;
				}
//                if( in_array( $this->_product_get_type($product), $this->_aVariationsType ) )
//                    $product_id = version_compare( WC_VERSION, '3.0', '>=' ) ? $this->_get_id($product) : $product->variation_id;
//                else
...
//                $to = get_post_meta($product_id, '_sale_price_dates_to', true);
				...
				$from = $product->get_date_on_sale_from() ? $product->get_date_on_sale_from()->getTimestamp() : '';
				$to   = $product->get_date_on_sale_to() ? $product->get_date_on_sale_to()->getTimestamp() : '';
				...
				if ( ! empty( $from ) && empty( $to ) ) {
					if ( $from >= time() ) {
						$to = $from + 31536000; // one year
					} else {
						$to = time() + 31536000;
					}
					$date = date_i18n( 'Y-m-d\TH:iO', $from ) . "/" . date_i18n( 'Y-m-d\TH:iO', $to );
				...
				// sale_schedule logic
				if ( ! empty( $to ) && $to < time() ) {
					$this->_wpwoofeed_settings["product_tmp_data"]['dont_show_sale_price'] = true;
					$date                                                                  = '';
				} elseif ( ( $this->_wpwoofeed_settings['feed_type'] === 'pinterest'
				             || ( isset( $this->_wpwoofeed_settings['sale_schedule'] ) && $this->_wpwoofeed_settings['sale_schedule'] == 'current' ) )
				           && ! empty( $from ) && $from > time() ) {
					$this->_wpwoofeed_settings["product_tmp_data"]['dont_show_sale_price'] = true;
					$date                                                                  = '';
				}
			...
			case 'cost_of_goods_sold':
				if ( isset( $this->_wpwoofeed_settings["product_tmp_data"]['cost_of_goods_sold'] ) ) {
					return $this->_wpwoofeed_settings["product_tmp_data"]['cost_of_goods_sold'];
				}
				if ( ! empty( $this->_wpwoofeed_settings['field_enable_cost_of_goods_sold'] ) && WoocommerceWpwoofCommon::isActivatedCOG()
				     && $this->_store_info->currency == $this->_store_info->default_currency ) {
					$cog = COG\pixel_wc_cog()->get_product_cost_of_goods( $this->_get_id( $product ) );
					if ( ! empty( $cog ) ) {
						return $this->_wpwoofeed_settings["product_tmp_data"]['cost_of_goods_sold'] = $this->_price_format( $cog );
					}
				}
			...
			case 'auto_pricing_min_price':
				$cog = $this->get_tagvalue( 'cost_of_goods_sold', $product, $tagid );
				$this->_storedebug( "auto_pricing_min_price cog " . $cog );
				if ( ! empty( $cog ) ) {
					// check by priority

					//1. check the product level
					$result = $this->_getMeda( 'wpfoof-auto_pricing_min_price', $product );
					if ( ! empty( $result ) ) {
						$this->_storedebug( "auto_pricing_min_price: product level" );

						return $this->_price_format( $result );
					}

					$price = $this->_woocommerce_wpwoof_common->get_numeric_price_from_string( $this->_wpwoofeed_price( $product, null, null, $tagid ) );
					if ( empty( $price ) ) {
						$this->_storedebug( "auto_pricing_min_price: price is empty" );

						return '';
					}

					//2. check category
					$result = $this->_get_percent_auto_pricing_min_price_from_categories( $product );
					if ( ! empty( $result ) ) {
						$this->_storedebug( "auto_pricing_min_price category level: " . $result . '%' );

						return $this->_price_format( $price - ( $price * $result / 100 ) );
					}

					//3. check feed settings
					$result = ! empty( $this->_wpwoofeed_settings['auto_pricing_min_price'] ) ? $this->_wpwoofeed_settings['auto_pricing_min_price'] : "";
					if ( ! empty( $result ) ) {
						$this->_storedebug( "auto_pricing_min_price: feed level" );

						return $this->_price_format( $price - ( $price * $result / 100 ) );
					}

					//4.global
					if ( ! empty( $this->_wpwoofeed_settings['extra']['auto_pricing_min_price']['value'] ) ) {
						$this->_storedebug( "auto_pricing_min_price: global level" );

						return $this->_price_format( $price - ( $price * $this->_wpwoofeed_settings['extra']['auto_pricing_min_price']['value'] / 100 ) );
					}
	...
	private function _custom_user_function( $product, $item, $field, $id, $data = array() ) {
		...
		if ( empty( $field_values ) && isset( $field['rules']['woocommerce_default']['value'] ) && isset( $field['rules']['woocommerce_default']['automap'] ) ) {
			$field_values = array( 0 => array( 'value' => $field['rules']['woocommerce_default']['value'] ) );
		}
		...
		foreach ( $field_values as $value ) {
			if ( ( $value['value'] == 'custom_value' || $value['value'] == 'custom_value_editor' ) && isset( $value['custom_value'] ) ) {
				$result[] = $value['custom_value'];
			} elseif ( is_string( $value['value'] ) && strpos( $value['value'], 'wpwoofattr_' ) !== false && $data['wpwoofattr'] ) {
				$result[] = $this->_wpwoofeed_attr( $product, $item, $value, $id );
			} else {
				$result[] = $this->get_tagvalue( $value['value'], $product, $id, $value );
			}
		}
		$this->_storedebug( '_custom_user_function ' . ( $value['value'] ?? '' ) . ' $result' );
		$this->_storedebug( $result );
		if ( empty( $result ) ) {
			return false;
...
//        $this->_storedebug("ID:".$id."|ITEM:".print_r($item,true)."|FIELD:".print_r($field,true));
		$taxonomy = ( isset( $field['define'] ) && isset( $field['define']['option'] ) ) ? $field['define']['option'] : $field['value'];

		if ( strpos( $taxonomy, 'wpwoofattr_' ) !== false ) {
			$taxonomy = str_replace( 'wpwoofattr_', '', $taxonomy );

			// if field in custom fields list
			if ( is_array( $this->_wpwoof_custom_fields ) && in_array( $taxonomy, $this->_wpwoof_custom_fields ) ) {
				$wpwoofextra = get_post_meta( $this->_get_id( $product ), 'wpwoofextra', true );
				if ( is_array( $wpwoofextra ) && count( $wpwoofextra ) ) {
					foreach ( $wpwoofextra as $value ) {
						if ( isset( $value['custom_tag_name'] ) && isset( $value['value'] ) && $value['custom_tag_name'] == $taxonomy ) {
							if ( ! empty( $value['type'] ) && $value['type'] == 'editor' ) {
								$this->_field_rules[ $id ]['CDATA']        = true;
								$this->_field_rules[ $id ]['NotStripTags'] = true;
							}
				...
				$gd = $this->_woocommerce_wpwoof_common->getGlobalData();
				if ( isset( $gd['extra'] ) && count( $gd['extra'] ) ) {
					foreach ( $gd['extra'] as $extraValue ) {
						if ( isset( $extraValue['feed_type'] ) && isset( $extraValue['feed_type']['mapping'] ) && isset( $extraValue['custom_tag_name'] )
						     && isset( $extraValue['custom_value'] ) && $extraValue['custom_tag_name'] == $taxonomy ) {
							if ( $extraValue['value'] == 'custom_value_editor' ) {
								$this->_field_rules[ $id ]['CDATA']        = true;
								$this->_field_rules[ $id ]['NotStripTags'] = true;
							}

							return $extraValue['custom_value'];
						}
					}
				}
			}
		...
		// Check fields from Advanced Custom Fields (ACF) plugin.
		if ( WoocommerceWpwoofCommon::is_active_acf() ) {
			$tag_value = get_field( $taxonomy, $this->_get_id( $product ) );
			if ( ! empty( $tag_value ) ) {
				$tag_value = wp_list_pluck( $tag_value, 'name' );
				$tag_value = implode( " > ", $tag_value );
				if ( $tag_value !== "" ) {
					$this->_storedebug( '_wpwoofeed_attr ACF field ' . $taxonomy );
		...
		if ( ! empty( $taxonomy ) && is_string( $taxonomy ) && strpos( $taxonomy, 'pa_' ) !== false && $this->_product_get_type( $product ) == 'variation' ) {
			...
			foreach ( $attributes as $attribute => $attribute_value ) {
				if ( is_string( $attribute ) && strpos( $attribute, $txnmy ) !== false ) {
					...
					return ( $term && isset( $term->name ) ) ? $term->name : strval( $attribute_value );
				}
			}
		}
	...
	private function _wpwoofeed_condition( $product, $item, $field, $id ) {
		...
		$product_condition = $this->_getMeda( 'wpfoof-condition', $product );
		if ( ! empty( $product_condition ) ) {
			return $product_condition;
		}
		if ( isset( $field['define']['missingvalue'] ) ) {
			return $this->wpwoofeed_old_condition( $product, $item, $field, $id );
		}
		$result = '';
		if ( count( $field ) > 0 ) {
			foreach ( $field as $elmID ) {
	...
	private function _wpwoofeed_availability( $product, $item = false, $field = false, $id = false ) {
		...
		if ( $product->has_child() ) {
			...
			foreach ( $children as $child ) {
				...
				if ( $this->get_product_availability( $child ) == 'in stock' ) {
					return 'in stock';
				}
			}
			...
			return $this->get_product_availability( $product );
		}
	}
...
//        $this->_storedebug("is_on_backorder():" . $product->is_on_backorder());
//        $this->_storedebug("get_backorders():" . $product->get_backorders());
//        $this->_storedebug("managing_stock():" . $product->managing_stock());
		if ( $product->is_in_stock() ) {
			if ( $product->is_on_backorder() ) {
				if ( empty( $product->managing_stock() ) || $product->managing_stock() === 'parent' ) {
					if ( $this->_wpwoofeed_settings['feed_on_backorders'] == 'instock' ) {
						$stock = 'in stock';
					}
				} else {
					...
					switch ( $backorders ) {
						case 'yes':
							if ( $this->_wpwoofeed_settings['feed_backorders_allow'] === 'instock' ) {
								$stock = 'in stock';
							}

							break;
						case 'notify':
							if ( $this->_wpwoofeed_settings['feed_backorders_notify'] === 'instock' ) {
								$stock = 'in stock';
							}
							break;
					}
	...
	private function _wpwoofeed_inventory( $product, $item, $field, $id ) {
		...
		$result = null;
		$field  = isset( $this->_wpwoofeed_settings['field_mapping']['inventory'] ) ? $this->_wpwoofeed_settings['field_mapping']['inventory'] : array(
			"value"   => 1,
			"default" => 5
		);
	...
	private function _wpwoofeed_brand( $product, $item, $field, $id ) {
		...
		if ( isset( $wpwoof_values['brand'] ) ) {
			$field = $wpwoof_values['brand'];
		}
		...
		if ( ! empty( $field['custom'] ) || ! isset( $field['custom'] ) ) {
			$result = $this->_getMeda( 'wpfoof-brand', $product );
			if ( ! empty( $result ) ) {
				return $result;
			}
		}
		...
		/* old version complete */
		if ( count( $field ) > 0 ) {
			if ( ( ! empty( $field['woo_brand'] ) || ! isset( $field['woo_brand'] ) ) && $this->_woocommerce_wpwoof_common->is_woocomerce_brand_active() ) {

				$brands = get_the_terms( $this->_get_id( $product ), 'product_brand' );
			...
			if ( ( ! empty( $field['WPWOOF_BRAND_PRWB'] ) || ! isset( $field['WPWOOF_BRAND_PRWB'] ) ) && $this->_woocommerce_wpwoof_common->is_PRWB_active() ) {
				$brands = get_the_terms( $this->_get_id( $product ), 'product_brand' );
			...
			if ( is_plugin_active( WPWOOF_BRAND_PBFW ) && isset( $field["WPWOOF_BRAND_PBFW"] ) ) {
				$term = get_the_terms( $this->_get_id( $product ), "product_brand" );
			...
			if ( ( ! empty( $field['linked'] ) || ! isset( $field['linked'] ) ) && ! empty( $field['value'] ) ) {
		...
		$result = empty( $result ) ? $value : $result;
		if ( empty( $result ) ) {
			$this->_xml_has_error( 'Brand missing in some products' );
		}
	...
	/**
	 * Retrieves item_group_id field.
	 *
	 * @param mixed $product The product object or ID.
	 * @param mixed $item The item object or ID.
	 * @param array $field The field configuration.
	 * @param string $tag The tag used for the field.
	 *
	 * @return string item_group_id field, or an empty string if not applicable.
	 */
	private function _wpwoofeed_item_group_id( $product, $item, $field, $tag ) {

		if ( ( ( in_array( $this->_product_get_type( $product ), $this->_aVariationsType )
		         || $product->has_child() ) || $this->_wpwoofeed_settings['feed_type'] != 'facebook' )
		     && ! empty( $field['define'] ) ) {
			return $field['define'];
		} else {
			return '';
...
//    }
				...
				switch ( $tag ) {
					...
					case "top-30-days":
						...
						if ( is_null( $this->top_30 ) ) {
							...
							if ( WoocommerceWpwoofCommon::isEnabledHPOS() ) {
								$sSQL = "SELECT order_item_meta__product_id.meta_value as prod_id,SUM( order_item_meta__qty.meta_value) as qty "
								        . "FROM {$this->_wpdb->prefix}wc_orders AS orders "
								        . "INNER JOIN {$this->_wpdb->prefix}woocommerce_order_items AS order_items ON (orders.ID = order_items.order_id) AND (order_items.order_item_type = 'line_item') "
								        ...
								        . "WHERE orders.type IN ( 'shop_order','shop_order_refund' ) AND orders.status IN ( 'wc-completed','wc-processing','wc-on-hold','wc-refunded') AND orders.date_created_gmt >= '" . $stardDate . "' AND orders.date_created_gmt < '" . $endDate . "' "
								        . "GROUP BY prod_id "
								        . "ORDER BY qty DESC "
								        . "LIMIT 100;";
							} else {
	...
	private function _wpwoofeed_expand_more_images( $product, $item, $field, $tag ) {
		...
		//Additional Variation Images Gallery for WooCommerce plugin integration
		if ( in_array( $this->_product_get_type( $product ), $this->_aVariationsType ) && class_exists( 'Woo_Variation_Gallery', false ) ) {
			$gallery_images = (array) get_post_meta( $this->_get_id( $product ), 'woo_variation_gallery_images', true );
			$this->_storedebug( 'Additional Variation Images Gallery for WooCommerce plugin integration' );
			if ( ! empty( $gallery_images ) && ! empty( $gallery_images[0] ) ) {
				$this->_storedebug( $gallery_images );
				$imgIds = array_merge( $imgIds, $gallery_images );
			}
		}
	...
	private function _wpwoofeed_image_0_url( $product, $item, $field, $tag ) {
		$images = array( $this->get_tagvalue( 'image_link', $product, $item ) );
		if ( ! empty( $images ) && empty( $this->_wpwoofeed_settings['expand_more_images'] ) ) {
			$more_images = $this->_wpwoofeed_expand_more_images( $product, $item, $field, $tag );
		...
		foreach ( $images as $image ) {
			$array_for_output[] = array( 'url' => $image ); //array( 'url' => $image, 'tag' => $tags_array );
		}

		return $array_for_output;
	}

	private function _wpwoofeed_override( $product, $item, $field, $tag ) {
		if ( $this->is_feed_localized() ) {
			if ( ! empty( $this->_wpwoofeed_settings['custom_override'] ) ) {
				return $this->_wpwoofeed_settings['custom_override'];
			}
			if ( $this->_localized_feed_settings['feed_type'] == 'fb_localize' ) {
				if ( ! empty( $this->_localized_feed_settings['feed_use_lang'] ) ) {
					global $sitepress;

					return $sitepress->get_locale( $this->_localized_feed_settings['feed_use_lang'] );
				} else {
					return 'language not selected';
				}

			} elseif ( $this->_localized_feed_settings['feed_type'] == 'fb_country' ) {
				if ( ! empty( $this->_localized_feed_settings['country'] ) ) {
					return $this->_localized_feed_settings['country'];
				} else {
	...
	private function _generate_prices_for_product( $woocommerce_product ) {
		...
		$prices->sale_price_effective_date = $this->get_tagvalue( 'sale_price_effective_date', $woocommerce_product, 'sale_price_effective_date' );
		...
		$rate = ( $this->_store_info->currencyRate > 0 ) ? (float) $this->_store_info->currencyRate : 1.0;


		if ( $woocommerce_product->is_type( 'composite' ) && method_exists( $woocommerce_product, 'get_composite_price' ) ) {
			if ( class_exists( 'WPCleverWooco' ) ) {
				// WPC Composite Products for WooCommerce
			} else {
				$min_or_max = ( isset( $this->_wpwoofeed_settings['feed_composite_price'] ) && $this->_wpwoofeed_settings['feed_composite_price'] == 'max' ) ? 'max' : 'min';
				$this->_storedebug( "composite price: " . $min_or_max );
				$prices->sale_price    = $woocommerce_product->get_composite_price( $min_or_max );
				$prices->regular_price = $woocommerce_product->get_composite_regular_price( $min_or_max );
				if ( $prices->regular_price == $prices->sale_price ) {
					$prices->sale_price = null;
				}

				return $this->_apply_post_price_logic( $prices );
			}
		} elseif ( $woocommerce_product->is_type( "subscription" ) || $woocommerce_product->is_type( "subscription_variation" ) ) {
				...
				switch ( $calc ) {
					...
					case "feeplusprice":
						...
						if ( $sale_price || $sale_price == "0" ) {
							$prices->sale_price = (float) $sale_price + ( $rerular ? (float) $rerular : 0.0 );
						}
						$prices->regular_price = (float) $price + ( $rerular ? (float) $rerular : 0.0 );
						break;
			...
			WoocommerceWpwoofCommon::isActivatedWPML() && ! empty( $this->_woocommerce_wpml->settings )
			&& ! empty( $this->_woocommerce_wpml->settings['enable_multi_currency'] )
			...
			// see #3401; get prices from original post.
			$origProductId = SitePress::get_original_element_id( $id, 'post_' . $woocommerce_product->post_type );
			$meta          = get_post_meta( $id );

			$this->_storedebug( "---------------------------------------" );
			if ( $origProductId != $genID ) {
				$this->_storedebug( 'WPML get price from original post. ID: ' . $origProductId );
				$id = $origProductId;
			}
			...
			if ( get_post_meta( $id, '_wcml_custom_prices_status', true ) == '1' ) {
				if ( ! $woocommerce_product->is_type( 'bundle' ) ) {
					...
					$prices->regular_price   = get_post_meta( $id, '_wc_pb_base_regular_price_' . $this->_store_info->currency, true );
					$prices->is_bundle_price = true;
				}
				$this->_storedebug( "this->_generate_prices_for_product regular_price:" . $prices->regular_price );

				if ( get_post_meta( $id, '_wcml_schedule_' . $this->_store_info->currency, true ) == '1' ) {
					$dt_from = get_post_meta( $id, '_sale_price_dates_from_' . $this->_store_info->currency, true );
					$dt_to   = get_post_meta( $id, '_sale_price_dates_to_' . $this->_store_info->currency, true );
					$this->_storedebug( "this->_generate_prices_for_product scheduled from:" . date( "Y-m-d", (int) $dt_from ) . ' to: ' . date( "Y-m-d", (int) $dt_to ) );
					if ( ( empty( $dt_from ) || $dt_from <= time() )
					     && ( empty( $dt_to ) || $dt_to >= time() )
					) {
						if ( ! $woocommerce_product->is_type( 'bundle' ) ) {
							$prices->sale_price = get_post_meta( $id, '_sale_price_' . $this->_store_info->currency, true );
						} else {
							$prices->sale_price = get_post_meta( $id, '_wc_pb_base_sale_price_' . $this->_store_info->currency, true );
						}
						$this->_storedebug( "this->_generate_prices_for_product sale_price scheduled:" . $prices->sale_price );
					} else {
						$prices->sale_price = null;
					}
				} else {
				...
				if ( $prices->sale_price > 0 ) {
					$prices->sale_price = $prices->sale_price / $rate;
				}
			...
			if ( '' != ( $prices->regular_price = get_post_meta( $the_product_id, '_' . 'alg_currency_switcher_per_product_regular_price_' . $this->_store_info->currency, true ) ) ) {
				...
				$prices->regular_price = get_post_meta( $the_product_id, '_regular_price', true );
				$prices->sale_price    = get_post_meta( $the_product_id, '_sale_price', true );
			}
			if ( $prices->sale_price > 0 ) {
				$prices->sale_price = $prices->sale_price / $rate;
			}
			...
			if ( $this->_getMeda( "_" . $this->_wpwoofeed_settings['feed_use_currency'] . "_price_method", $woocommerce_product ) == 'manual' ) {
				...
				if ( ! empty( $prices->regular_price ) ) {
					$prices->regular_price_isConverted = true;
				}
				...
				if ( ! empty( $prices->sale_price ) ) {
					$prices->sale_price_isConverted = true;
				}
		...
		} elseif ( WoocommerceWpwoofCommon::is_active_alg_wc_cpp() ) {
			$currency_product = alg_wc_cpp()->core->get_product_currency( $this->_get_id( $woocommerce_product ) );
			if ( $currency_product && $currency_product != $this->_store_info->currency ) {
				$regular_price = $woocommerce_product->get_regular_price();
				if ( '' != $regular_price ) {
					$prices->regular_price = $this->convert_price_alg_wc_cpp( $regular_price, $currency_product, $this->_store_info->currency );
				}

				$sale_price = $woocommerce_product->get_sale_price();
				if ( $sale_price != '' ) {
					$prices->sale_price = $this->convert_price_alg_wc_cpp( $sale_price, $currency_product, $this->_store_info->currency );
				}
				$this->_storedebug( "converted currency alg_wc_cpp:" . $currency_product . " to " . $this->_store_info->currency );
...
//		if ( WoocommerceWpwoofCommon::isActivatedWPML() ) {
//			$this->_sitepress->switch_lang( $this->_store_info->feed_language, true );
//		}
		...
		if ( WoocommerceWpwoofCommon::isActivatedAeliaCS() && isset ( $this->_wpwoofeed_settings['feed_use_currency'] )
		     && ! empty ( $this->_wpwoofeed_settings['feed_use_currency'] ) ) {
			if ( ! $this->_aelia_currency_filter ) {
				$this->_aelia_currency_filter = true;
				add_filter( 'wc_aelia_cs_selected_currency', function () {
					if ( isset ( $this->_wpwoofeed_settings['feed_use_currency'] ) && ! empty ( $this->_wpwoofeed_settings['feed_use_currency'] ) ) {
						return $this->_wpwoofeed_settings['feed_use_currency'];
					} else {
		...
		if ( $this->_product_get_type( $woocommerce_product ) == 'phive_booking' ) { // #3829#2 booking plugin return price only in  get_price() method.
			$regular_price = $woocommerce_product->get_price();
		} else {
			$regular_price = $woocommerce_product->get_regular_price();
		}
	...
	/**
	 * Converts the price from one currency to another using exchange rates (alg_wc_cpp plugin)
	 *
	 * @param float $price The price to be converted.
	 * @param string $currency_product The currency code of the product's price.
	 * @param string $currency_to The currency code to convert the price to.
	 *
	 * @return float The converted price.
	 */
	private function convert_price_alg_wc_cpp( $price, $currency_product, $currency_to ) {
		if ( ! WoocommerceWpwoofCommon::is_active_alg_wc_cpp() ) {
			return $price;
		}
		$rates = $this->_woocommerce_wpwoof_common->get_rates_alg_wc_cpp();
		if ( ! empty( $rates[ $currency_product ] ) && ! empty( $rates[ $currency_to ] ) ) {
			$exchange_rate_product = $rates[ $currency_product ];
			$exchange_rate_to      = $rates[ $currency_to ];
			$exchange_rate         = $exchange_rate_product / $exchange_rate_to;
	...
	private function _apply_post_price_logic( $prices ) {
		if ( $prices->sale_price !== null && ! empty( $this->_wpwoofeed_settings["product_tmp_data"]['dont_show_sale_price'] ) ) {
			$this->_storedebug( "this->_generate_prices_for_product: dont_show_sale_price" );
			$prices->sale_price = null;
		}
	...
	private function _wpwoofeed_calc_tax( $price, $product, $isPriceConverted = false ) {
		//$this->_storedebug("_wpwoofeed_calc_tax start price: ".$price.", isPriceConverted: ".($isPriceConverted?1:0));
		if ( ! $price ) {
			$price = 0.00;
		}
...
//        $this->_storedebug("_wpwoofeed_calc_tax tax_rate_tmp_data: ".$this->_wpwoofeed_settings['tax_rate_tmp_data']);
			...
			foreach ( $atax as $tax ) {
				if ( isset( $tax['rate'] ) ) {
					if ( ! empty( $tax['compound'] ) && $tax['compound'] == 'yes' ) {
						$aTax[0] += (float) $tax['rate'];
						//$i++;
					} else {
						$BaseTax += (float) $tax['rate'];
						$aTax[0] = $BaseTax;
					}
				}
			}
		...
		if ( $this->_wpwoofeed_settings['tax_rate_tmp_data'] == 'addbase' ) {
			...
			$oprice = $price;
			while ( $i > - 1 ) {
				$price = (float) $price / ( 1.0 + (float) $aTax[ $i ] / 100.0 );
				$i --;
			}
			...
			if ( isset( $sep[1] ) && ! empty( $sep[1] ) ) {
				...
				if ( strpos( $sValTMP, "-" ) ) {
					...
					if ( count( $this->_wpwoofeed_settings['atax_tem_val'] ) > 0 ) {
						$cl = $this->_wpwoofeed_settings['atax_tem_val'][0]['class'];
					}
					$country = $sValTMP[0];
...
//                    $this->_storedebug($prices);
			...
			if ( $price == $this->_wpwoofeed_settings['product_tmp_data']['prices']->regular_price ) {
				return false;
			}
	...
	private function check_fixed_price_by_WOOCS( $product, $get_sale_price = false ) {
		if ( WoocommerceWpwoofCommon::isActivatedWOOCS() && isset( $this->_wpwoofeed_settings['feed_use_currency'] ) and ! empty( $this->_wpwoofeed_settings['feed_use_currency'] ) ) {
			global $WOOCS;
			$currencies = $WOOCS->get_currencies();
			if ( ! $WOOCS->is_multiple_allowed || ! isset( $currencies[ $this->_wpwoofeed_settings['feed_use_currency'] ] ) ) {
				return false;
			}
			...
			if ( $price ) {
				$this->_storedebug( "WOOCS fixed price: " . $price );
			}
	...
	private function _wpwoofeed_sale_price( $product, $item, $field, $id ) {
		...
		$prices->sale_price = ( ! empty( $prices->sale_price ) && is_string( $prices->sale_price ) ) ? trim( $prices->sale_price ) : $prices->sale_price;
		...
		if ( empty( $price_WCDPD ) && empty( $price_WOOCS ) && ( $prices->sale_price == "" || $prices->sale_price == $prices->regular_price ) ) {
			return '';
		}
	...
	private function _price_format( $price ) {
		...
		if ( $WCPBCSettings = WoocommerceWpwoofCommon::isActivatedWCPBC( 'settings' ) ) {
			if ( isset( $this->_wpwoofeed_settings['feed_use_currency'] ) && $this->_wpwoofeed_settings['feed_use_currency']
			     && ! empty( $WCPBCSettings[ $this->_wpwoofeed_settings['feed_use_currency'] ] ) && ! empty( $WCPBCSettings[ $this->_wpwoofeed_settings['feed_use_currency'] ]['round_nearest'] )
			) {
				$round_nearest = floatval( $WCPBCSettings[ $this->_wpwoofeed_settings['feed_use_currency'] ]['round_nearest'] );
//                $this->_storedebug( 'WCPBC Round to Nearest: ' . $price . ' => ' . ceil($price / $round_nearest) * $round_nearest);
				$price = ceil( $price / $round_nearest ) * $round_nearest;
			}
		}
	...
	private function _id_format( $product, $item, $field, $id ) {
		if ( $this->is_feed_localized() && ! in_array( $this->_product_get_type( $product ), $this->_aVariationsType ) ) {
			$product = $this->_load_product( $this->_localized_main_product_id );
		}
	...
	private function _wpwoofeed_identifier_exists( $product, $item, $field, $id ) {
		...
		$result = $this->_getMeda( 'wpfoof-identifier_exists', $product );
	...
	private function _wpwoofeed_reviews( $product, $item, $field, $id ) {
		$result = array();
		$args   = array(
			'post_id' => $this->_get_id( $product ),
			'status'  => 'approve',
		);
		if ( isset( $this->_wpwoofeed_settings['feed_review_stars_filter'] ) && ! empty( $this->_wpwoofeed_settings['feed_review_stars_filter'] ) ) {
			$stars = apply_filters( 'product_reviews_ratings', $this->_wpwoofeed_settings['feed_review_stars_filter'] );
			if ( ! empty( $stars ) ) {
				$this->_storedebug( "product_reviews_ratings filter: " . print_r( $stars, true ) );
		...
		if ( ! empty( $reviews ) ) {
			foreach ( $reviews as $value ) {
				$result[] = array(
					'review_id'        => $value->comment_ID,
					'review_timestamp' => str_replace( ' ', 'T', $value->comment_date_gmt ) . 'Z',
					'reviewer_name'    => $value->comment_author,
					'reviewer_id'      => $value->user_id,
					'content'          => $value->comment_content,
					'overall'          => get_comment_meta( $value->comment_ID, 'rating', true ),
					'review_url'       => get_comment_link( $value ),
				);
			}
		}
//        $this->_storedebug( 'reviews');
//        $this->_storedebug( $reviews);
		$this->_storedebug( $result );
	...
	private function _wpwoofeed_store_code( $product, $item, $field, $tag ) {
		$value = '';

		if ( ! empty ( $this->_wpwoofeed_settings['store_code'] ) ) {
			$value = $this->_wpwoofeed_settings['store_code'];
		}
	...
	function storeTicket( $id, $buf, $ttDir = "" ) {
		...
		if ( is_int( $id ) && $this->_wpwoofeed_settings['tmp_storage'] === 'db' ) {
			update_post_meta( $id, 'wpwoof_ticket_' . $this->_feed_id, $buf );
		...
		if ( ! file_exists( $this->_ticketDir ) ) {
			if ( mkdir( $this->_ticketDir, 0755, true ) ) {
				return $this->storeTicket( $id, $buf );
			} else {
				throw new Exception( "Can not create '" . $this->_ticketDir . "' folder." );
			}
		}
...
//        }
		if ( is_int( $id ) && is_array( $buf ) ) {
			if ( PHP_VERSION_ID < 70200 ) {
				foreach ( array_keys( $buf ) as $key ) {
					if ( ! is_array( $buf[ $key ] ) ) {
						$buf[ $key ] = iconv( 'UTF-8', 'UTF-8//IGNORE', $buf[ $key ] );
					}
				}
				$buf = json_encode( $buf );
			} else {
				$buf = json_encode( $buf, JSON_INVALID_UTF8_IGNORE );
			}
		}
		if ( ! empty( $id ) ) {
	...
	function compileXMLItem( $aItem ) {
		...
		foreach ( $this->_aCSVcolumns as $element ) {
			if ( isset( $this->_field_rules[ $element ] ) ) {
				$length        = false;
				$value_type    = false;
				$cdata         = ! empty( $this->_field_rules[ $element ]['CDATA'] );
				$no_strip_tags = $this->_field_rules[ $element ]['NotStripTags'] ?? false;
				...
				if ( preg_match( '/video_(\d+)_tag/', $element ) ) {
					continue;
				}
				if ( preg_match( '/video_(\d+)_url/', $element, $matches ) && ! empty( $aItem[ $element ] ) ) {
					$number   = $matches[1];
					$tagvalue = $this->_terminate_text( $this->_enforce_length( $aItem[ $element ], $length, true ), $cdata, $no_strip_tags );
					$item     .= "        <video>\n";
					$item     .= "        <url>" . $tagvalue . "</url>\n";
					if ( isset( $aItem[ "video_" . $number . "_tag" ] ) ) {
						$element  = "video_" . $number . "_tag";
						$tagvalue = $this->_terminate_text( $this->_enforce_length( $aItem[ $element ], $length, true ), $cdata, $no_strip_tags );
				...
				$values = array();
				if ( ! isset( $aItem[ $element ] ) ) {
					if ( $value_type == 'int' ) {
						$values[] = 0;
					} else {
						$values[] = '';
					}
				} elseif ( is_array( $aItem[ $element ] ) ) {
					$values = $aItem[ $element ];
				} else {
					$values[] = $aItem[ $element ];
				}

				foreach ( $values as $tagvalue ) {
					if ( isset( $this->_field_rules[ $element ]['xml'] ) ) {
						if ( is_array( $this->_field_rules[ $element ]['xml'] ) ) {
							foreach ( $this->_field_rules[ $element ]['xml'] as $xml_element => $xml_sub_elements ) {
								$sub_elements_xml = '';
								if ( is_array( $xml_sub_elements ) ) {
									foreach ( $xml_sub_elements as $sub_element_tag_name ) {
										if ( isset( $tagvalue[ $sub_element_tag_name ] ) ) {
											$tag_values_array = is_array( $tagvalue[ $sub_element_tag_name ] ) ? $tagvalue[ $sub_element_tag_name ] : array( $tagvalue[ $sub_element_tag_name ] );
											foreach ( $tag_values_array as $tag_value ) {
												$sub_element_value = $this->_enforce_length( $tag_value, $length, true );
												$sub_element_value = $this->_terminate_text( $sub_element_value, $cdata, $no_strip_tags );
												$sub_elements_xml  .= "<" . $sub_element_tag_name . ">" . $sub_element_value . "</" . $sub_element_tag_name . ">";
											}
										}
									}
								}
								if ( ! empty( $sub_elements_xml ) ) {
									$item .= "<" . $xml_element . ">" . $sub_elements_xml . "</" . $xml_element . ">";
								}
							}
						} else {
							$tagname = isset( $this->_field_rules[ $element ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_xml' ] ) ? $this->_field_rules[ $element ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_xml' ] : $this->_field_rules[ $element ]['xml'];

							if ( ! $this->_woocommerce_wpwoof_common->is_valid_full_xml_tag_name( $tagname ) ) {
								// ignored fields with wrong tag name.
								continue;
							}
							if ( $element == "expand_more_images" ) {
								if ( ! empty( $tagvalue ) ) {
									$tagval = $this->_terminate_text( $tagvalue, $cdata, $no_strip_tags );
									if ( ! empty( $tagval ) ) {
										$item .= "        <" . $tagname . ">" . $tagval . "</" . $tagname . ">" . "\n";
									}
								}
							} else {
								if ( $value_type == 'int' ) {
									$item .= "        <" . $tagname . ">" . $tagvalue . "</" . $tagname . ">" . "\n";
								} else {
									if ( ! empty( $tagvalue ) ) {
										$tagvalue = $this->_terminate_text( $this->_enforce_length( $tagvalue, $length, true ), $cdata, $no_strip_tags );
									}
	...
	function compileCSVItem( $aItem, $aFields ) {
		...
		foreach ( $aFields as $element ) {
			if ( isset( $this->_field_rules[ $element ] ) && isset( $aItem[ $element ] ) ) {
				if ( isset( $this->_field_rules[ $element ]['csv'] ) && is_array( $this->_field_rules[ $element ]['csv'] ) ) {
					$composite_fields = $this->get_composite_field_key_value_for_csv( $aItem[ $element ], $element );
					if ( ! empty( $composite_fields ) ) {
						foreach ( $composite_fields as $composite_field_key => $composite_field_value ) {
							$aItem[ $composite_field_key ]                     = $composite_field_value;
							$this->_field_rules[ $composite_field_key ]['csv'] = $composite_field_key;
						}
					}
					$item[ $element ] = '';
					continue;
				...
				$no_strip_tags = $this->_field_rules[ $element ]['NotStripTags'] ?? false;
				if ( isset( $this->_field_rules[ $element ]['csv'] ) ||
				     isset( $this->_field_rules[ $element ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_csv' ] ) ) {
					$tagname   = $this->_field_rules[ $element ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_csv' ] ?? $this->_field_rules[ $element ]['csv'];
					$tag_value = "";
					if ( $element == "installment" && isset( $aItem[ $element ]['months'] ) && isset( $aItem[ $element ]['amount'] ) ) {
						$tag_value = $aItem[ $element ]['months'] . " " . $aItem[ $element ]['amount'];
					} else if ( $element == "expand_more_images" ) {
						$tag_value = empty( $aItem[ $element ] ) ? "" : implode( "%2C", $aItem[ $element ] );
					} elseif ( is_array( $aItem[ $element ] ) ) {
						foreach ( $aItem[ $element ] as $key => $value ) {
							if ( empty( $value ) && $value != 0 ) {
								unset( $aItem[ $element ][ $key ] );
							}
						}
						if ( ! empty( $aItem[ $element ] ) ) {
							$tag_value = implode( "%2C", $aItem[ $element ] );
						}
					} else {
						if ( empty( $aItem[ $element ] ) ) {
							$tag_value = "";
						} else if ( is_string( $aItem[ $element ] ) ) {
							$tag_value = $this->_enforce_length( $aItem[ $element ], $length, true );
							$tag_value = $this->_terminate_text( $tag_value, false, $no_strip_tags );
						} else {
							$tag_value = $aItem[ $element ];
						}
					}
					$item[ $tagname ] = $tag_value;
				}
			}
		}
	...
	function compileFeed() {
		if ( ! empty( $this->_ticketDir ) ) {
			...
			$upload_dir  = wpwoof_feed_dir( $this->_feed_file_name, 'xml' );
			$filepathXML = $upload_dir['path'];
			$upload_dir  = wpwoof_feed_dir( $this->_feed_file_name, 'csv' );
			...
			if ( file_exists( $this->_ticketDir . "/header.item" ) ) {
				...
				foreach ( $aCSVHeader as $i => $elm ) {
					if ( $this->_store_info->feed_type == "adsensecustom" && in_array( $aCSVHeader[ $i ], array(
							"item_group_id",
							"tax",
							"tax_countries",
							"mpn",
						...
						unset( $aCSVHeader[ $i ] );
					} else if ( isset( $this->_field_rules[ $elm ] ) && ( isset( $this->_field_rules[ $elm ]['csv'] ) && ! is_array( $this->_field_rules[ $elm ]['csv'] ) || isset( $this->_field_rules[ $elm ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_csv' ] ) ) ) {
						$header[] = isset( $this->_field_rules[ $elm ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_csv' ] ) ? $this->_field_rules[ $elm ][ 'tagname_' . $this->_wpwoofeed_settings['feed_type'] . '_csv' ] : $this->_field_rules[ $elm ]['csv'];
					} else if ( ! isset( $this->_field_rules[ $elm ] ) && strpos( $elm, '].' ) !== false ) {
						// composite field: image[0].url, image[0].tag[0]
						$header[] = $elm;
					}
				}
				...
				if ( $this->_store_info->feed_type == "facebook" ) {
					fwrite( $filep, "# ref_application_id 451257172939091\n" );
				}
			...
			if ( $this->_wpwoofeed_settings['tmp_storage'] === 'db' ) {
				global $wpdb;
				$query = "SELECT DISTINCT post_id FROM $wpdb->postmeta WHERE meta_key = 'wpwoof_ticket_" . $this->_feed_id . "'";

				$products = $wpdb->get_results( $query, ARRAY_N );
				if ( is_array( $products ) && ! empty( $products ) ) {
					$filep = fopen( $filepathCSV, "a+" );
					if ( ! $filepXML = fopen( $filepathXML, 'a' ) ) {
						$this->_storedebug( "Cannot open file: " . $filepathXML, 2 );
						exit;
					}
					$products = array_chunk( $products, 50 );
					foreach ( $products as $products_chunk ) {
						$products_chunk = array_column( $products_chunk, 0 );
						$query          = "SELECT post_id, meta_value FROM $wpdb->postmeta WHERE `meta_key` = 'wpwoof_ticket_" . $this->_feed_id . "' and post_id IN (" . implode( ',', $products_chunk ) . ")";
						$products_json  = $wpdb->get_results( $query, ARRAY_N );
						foreach ( $products_json as $product ) {
							$aItem = maybe_unserialize( $product[1] );
//                            $this->_storedebug($aItem);
							$item = $this->compileXMLItem( $aItem );
							if ( $item ) {
								if ( fwrite( $filepXML, $item ) === false ) {
									$this->_storedebug( "Cannot write file: " . $filepathXML, 2 );
									exit;
								}
							}
							...
							if ( count( $item ) ) {
								...
								$this->_storedebug( "================================================" );
								$csv_row = array();
								// sort items and add empty values;
								foreach ( $header as $header_item ) {
									$csv_row[] = $item[ $header_item ] ?? '';
								}
								fputcsv( $filep, $csv_row, $delimiter, $enclosure );
							}

							$wpdb->query( "DELETE FROM $wpdb->postmeta WHERE post_id = " . $product[0] . " AND meta_key = 'wpwoof_ticket_" . $this->_feed_id . "'" );
							$this->_storedebug( $wpdb->last_query );
						}
					}
					fclose( $filepXML );
					fclose( $filep );
				...
				$aItems = new FilesystemIterator( $this->_ticketDir, FilesystemIterator::SKIP_DOTS );
				$this->_storedebug( 'total files in _ticketDir:' . iterator_count( $aItems ) );
				$filep = fopen( $filepathCSV, "a+" );
				if ( ! $filepXML = fopen( $filepathXML, 'a' ) ) {
					$this->_storedebug( "Cannot open file: " . $filepathXML, 2 );
					exit;
				}
				foreach ( $aItems as $fileinfo ) {
					$filename = $fileinfo->getFilename();
					$this->_storedebug( 'file: /' . $filename );
					if ( ! in_array( $filename, array(
							"footer.item",
							"csvheader.item",
							"_field_rules.item"
						) ) && $fileinfo->getExtension() == "item" ) {
						$aItem = json_decode( file_get_contents( $this->_ticketDir . "/" . $filename ), true );
						...
						@unlink( $this->_ticketDir . "/" . $filename );
					}
			...
			if ( file_exists( $this->_ticketDir . "/footer.item" ) ) {
				...
				@unlink( $this->_ticketDir . "/_field_rules.item" );
			}
	...
	function compileFeedGoogleReviews() {
		$upload_dir   = wpwoof_feed_dir( $this->_feed_file_name, 'xml' );
		$filepathXML  = $upload_dir['path'];
		$total_review = 0;
		...
		$xmlWriter->startAttribute( 'xmlns:vc' );
		$xmlWriter->text( 'http://www.w3.org/2007/XMLSchema-versioning' );
		$xmlWriter->endAttribute();
		$xmlWriter->startAttribute( 'xmlns:xsi' );
		$xmlWriter->text( 'http://www.w3.org/2001/XMLSchema-instance' );
		$xmlWriter->endAttribute();
		$xmlWriter->startAttribute( 'xsi:noNamespaceSchemaLocation' );
		$xmlWriter->text( 'http://www.google.com/shopping/reviews/schema/product/2.3/product_reviews.xsd' );
		$xmlWriter->endAttribute();
		$xmlWriter->writeElement( 'version', '2.3' );
		$xmlWriter->startElement( 'publisher' );
		$xmlWriter->startElement( 'name' );
		$xmlWriter->writeCData( $this->_store_info->blog_name );
		$xmlWriter->endElement();
		$favicon = get_site_icon_url();
		if ( ! empty( $favicon ) ) {
			$xmlWriter->writeElement( 'favicon', get_site_icon_url() );
		}
		$xmlWriter->endElement();
		$xmlWriter->startElement( 'reviews' );
		file_put_contents( $filepathXML, $xmlWriter->flush( true ) );

		if ( $this->_wpwoofeed_settings['tmp_storage'] === 'db' ) {
			global $wpdb;
			$query    = "SELECT DISTINCT post_id,meta_value FROM $wpdb->postmeta WHERE meta_key = 'wpwoof_ticket_" . $this->_feed_id . "'";
			$products = $wpdb->get_results( $query, ARRAY_A );
			if ( is_array( $products ) && ! empty( $products ) ) {
				foreach ( $products as $product ) {
					$this->_storedebug( $product );
					$aItem = maybe_unserialize( $product['meta_value'] );
					if ( isset( $aItem['reviews'] ) && ! empty( $aItem['reviews'] ) ) {
						$this->_storedebug( $aItem );
						$total_review = + count( $aItem['reviews'] );
						$xmlWriter    = $this->product_reviews_to_xml( $xmlWriter, $aItem );
						file_put_contents( $filepathXML, $xmlWriter->flush( true ), FILE_APPEND );
					}
					$wpdb->query( "DELETE FROM $wpdb->postmeta WHERE post_id = " . $product['post_id'] . " AND meta_key = 'wpwoof_ticket_" . $this->_feed_id . "'" );
					$this->_storedebug( $wpdb->last_query );
				}
			}
		} else {
			$aItems = scandir( $this->_ticketDir );
			$this->_storedebug( 'total files in _ticketDir:' . count( $aItems ) );
			foreach ( $aItems as $i ) {
				if ( ! in_array( $i, array(
						".",
						"..",
		...
		$xmlWriter->endElement();
		file_put_contents( $filepathXML, $xmlWriter->flush( true ), FILE_APPEND );
		$this->_feed_settings_for_save['total_products'] = $total_review;
		wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );
	}

	function product_reviews_to_xml( $xmlWriter, $aItem ) {
		foreach ( $aItem['reviews'] as $review ) {
			$xmlWriter->startElement( 'review' );
			$xmlWriter->writeElement( 'review_id', $review['review_id'] );
			$xmlWriter->startElement( 'reviewer' );
			$xmlWriter->writeElement( 'name', $review['reviewer_name'] );
			$xmlWriter->writeElement( 'reviewer_id', $review['reviewer_id'] );
			$xmlWriter->endElement();
			$xmlWriter->writeElement( 'review_timestamp', $review['review_timestamp'] );

			$xmlWriter->startElement( 'content' );
			$xmlWriter->writeCData( $review['content'] );
			$xmlWriter->endElement();

			$xmlWriter->startElement( 'review_url' );
			$xmlWriter->writeAttribute( 'type', 'group' );
			$xmlWriter->text( $review['review_url'] );
			$xmlWriter->endElement();
			$xmlWriter->startElement( 'ratings' );

			$xmlWriter->startElement( 'overall' );
			$xmlWriter->writeAttribute( 'min', 1 );
			$xmlWriter->writeAttribute( 'max', 5 );
			$xmlWriter->text( $review['overall'] );
			$xmlWriter->endElement();
			...
			if ( isset( $aItem['gtin'] ) || isset( $aItem['sku'] ) || ( isset( $aItem['mpn'] ) && isset( $aItem['brand'] ) ) ) {
				$xmlWriter->startElement( 'product_ids' );
				if ( isset( $aItem['gtin'] ) ) {
					$xmlWriter->startElement( 'gtins' );
					if ( is_array( $aItem['gtin'] ) ) {
						foreach ( $aItem['gtin'] as $gtin ) {
							if ( ! empty( $gtin ) ) {
								$xmlWriter->writeElement( 'gtin', (string) $gtin );
							}
						}
					} else {
						$xmlWriter->writeElement( 'gtin', (string) $aItem['gtin'] );
					}

					$xmlWriter->endElement();
				}
				if ( isset( $aItem['sku'] ) ) {
					$xmlWriter->startElement( 'skus' );
					$xmlWriter->writeElement( 'sku', $aItem['sku'] );
					$xmlWriter->endElement();
				}
				if ( isset( $aItem['mpn'] ) && isset( $aItem['brand'] ) ) {
					$xmlWriter->startElement( 'mpns' );
					$xmlWriter->writeElement( 'mpn', $aItem['mpn'] );
					$xmlWriter->endElement();
					$xmlWriter->startElement( 'brands' );
					$xmlWriter->writeElement( 'brand', $aItem['brand'] );
					$xmlWriter->endElement();
				}
				$xmlWriter->endElement();
			}

			$xmlWriter->startElement( 'product_name' );
			$xmlWriter->writeCData( $aItem['title'] );
			$xmlWriter->endElement();

			$xmlWriter->writeElement( 'product_url', $aItem['link'] );
	...
	function checkAndAddCsvColumn( $aFields ) {
		$is_added = false;
		if ( ! is_array( $this->_aCSVcolumns ) ) {
			$this->_aCSVcolumns = array();
		}
		if ( count( $aFields ) ) {
			foreach ( $aFields as $fld ) {
				if ( ! in_array( $fld, $this->_aCSVcolumns ) && ! empty( $fld ) ) {
					$this->_aCSVcolumns[] = $fld;
					$is_added             = true;
				}
			}
		}
		if ( $is_added ) {
			$this->storeTicket( "csvheader", json_encode( $this->_aCSVcolumns ) );
		}
	}

	private function _wmc_disable_currency_by_country( $val ) {
		return 0;
	}

	function generate() {
		if ( empty( $this->_feed_id ) ) {
			return false;
		}
		...
		$feedStatus = $this->_woocommerce_wpwoof_common->get_feed_status( $this->_feed_id );
		if ( empty( $feedStatus["type"] ) ) {
			$feedStatus["type"] = $this->_wpwoofeed_type;
		}

		/* set path for feed and log file's */
		$upload_dir = wpwoof_feed_dir( $this->_feed_file_name, $this->_wpwoofeed_type );
		$filepath   = $upload_dir['path'];

		$ticketPath = str_replace( "/" . $this->_wpwoofeed_type, "", $upload_dir['pathtofile'] );

		/* create folder for tickets */
		$this->storeTicket( '', '', $ticketPath . "/" . $this->_feed_id );

		if ( WPWOOF_DEBUG ) {
			$this->_wpwoofeed_wpml_debug = $ticketPath . "/" . $this->_feed_id . ".log";
			$this->_woocommerce_wpwoof_common->setDebugFile( $this->_wpwoofeed_wpml_debug );
			echo "LOG FILE:" . $this->_wpwoofeed_wpml_debug . "\n";
			if ( empty( $feedStatus['products_left'] ) && file_exists( $this->_wpwoofeed_wpml_debug ) ) {
				@unlink( $this->_wpwoofeed_wpml_debug );
			}
			file_put_contents( $this->_woocommerce_wpwoof_common->feedBaseDir . 'cron-wpfeed.log', date( "Y-m-d H:i:s" ) . "\tStarting generating " . $this->_feed_id . "\n", FILE_APPEND );

		}
		/* end set path for feed and log file's */
		/* set cron task if generator stoped by server timeout */

		$this->_woocommerce_wpwoof_common->set_feed_gen_schedule( $this->_feed_id, time() + 60 );
		...
		foreach ( (array) get_categories( $args ) as $cat_value ) {
			$this->_all_categories[ $cat_value->term_id ] = $cat_value;
		}
		...
		if ( $feedStatus['parsed_products'] > 0 && file_exists( $this->_ticketDir . "/_field_rules.item" ) ) {
			$this->_field_rules = json_decode( file_get_contents( $this->_ticketDir . "/_field_rules.item" ), true );
		} else {
			$this->_field_rules = $this->_woocommerce_wpwoof_common->product_fields; /*wpwoof_get_product_fields();*/
		}

		if ( $feedStatus['parsed_products'] > 0 && file_exists( $this->_ticketDir . "/csvheader.item" ) ) {
			$this->_aCSVcolumns = json_decode( file_get_contents( $this->_ticketDir . "/csvheader.item" ), true );
		}
		...
		$this->_wpwoofeed_settings['tmp_storage']     = isset( $glData['data']['tmp_storage'] ) ? $glData['data']['tmp_storage'] : 'disk';
		if ( isset( $glData['data']['extra'] ) && count( $glData['data']['extra'] ) ) {
			...
			foreach ( $glData['data']['extra'] as $key => $value ) {
				if ( isset( $value['custom_tag_name'] ) && ! isset( $value['feed_type'][ $this->_wpwoofeed_settings['feed_type'] ] ) ) {
					continue;
				}
				if ( preg_match( '/^(.+)-\d+$/', $key, $matches ) ) {  //repeated fields
					$original_key = $matches[1];
					if ( isset( $this->_woocommerce_wpwoof_common->product_fields[ $original_key ] )
					     && ! empty( $this->_woocommerce_wpwoof_common->product_fields[ $original_key ]['repeated'] )
					     && in_array( $this->_wpwoofeed_settings['feed_type'], $this->_woocommerce_wpwoof_common->product_fields[ $original_key ]['feed_type'] ) ) {
						if ( ! isset( $this->_wpwoofeed_settings['extra'][ $original_key ] ) ) {
							$this->_wpwoofeed_settings['extra'][ $original_key ] = array( $value );
						} elseif ( is_array( $this->_wpwoofeed_settings['extra'][ $original_key ] ) ) {
							$this->_wpwoofeed_settings['extra'][ $original_key ][] = $value;
						} else {
							$this->_wpwoofeed_settings['extra'][ $original_key ] = array(
								$value,
								$this->_wpwoofeed_settings['extra'][ $original_key ]
							);
						}
						continue;
					}
				}
				if ( ! isset( $value['custom_tag_name'] ) && ( ! isset( $this->_woocommerce_wpwoof_common->product_fields[ $key ] )
				                                               || ! in_array( $this->_wpwoofeed_settings['feed_type'], $this->_woocommerce_wpwoof_common->product_fields[ $key ]['feed_type'] ) ) ) {
					continue;
				}
				$nkey                                        = strpos( $key, "custom-extra-field-" ) === 0 ? $value['custom_tag_name'] : $key;
				$this->_wpwoofeed_settings['extra'][ $nkey ] = $value;
				if ( ! isset( $this->_field_rules[ $nkey ] ) || $value['value'] == 'custom_value_editor' ) {
					$this->_field_rules[ $nkey ]['xml'] = 'g:' . $nkey;
					$this->_field_rules[ $nkey ]['csv'] = $nkey;
					if ( $value['value'] == 'custom_value_editor' ) {
						$this->_field_rules[ $nkey ]['CDATA']        = true;
						$this->_field_rules[ $nkey ]['NotStripTags'] = true;
					}
					$this->storeTicket( '_field_rules', json_encode( $this->_field_rules ) );
				}
			}
		}
		...
		try {
			$this->_feed_settings_for_save['status_feed'] = 'starting';
			wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );
			...
			if ( WoocommerceWpwoofCommon::isActivatedWPML() ) {
				...
				if ( empty( $this->_wpwoofeed_settings['feed_use_lang'] ) ) {
					$this->_wpwoofeed_settings['feed_use_lang'] = ICL_LANGUAGE_CODE;
				}
			...
			if ( /** WPML MULTYCURRENCY BLOCK */
				WoocommerceWpwoofCommon::isActivatedWPML() && ! empty( $this->_woocommerce_wpml->settings ) && ! empty( $this->_woocommerce_wpml->settings['enable_multi_currency'] ) ) {
				...
				$this->_storedebug( "\n-------------------------------------WOO-MULTI-CURRRENCY---------------------------------------------------------\n" );
				if ( apply_filters( 'wmc_get_enable_currency_by_country', 0 ) ) {
					// Disable currency_by_country
					add_filter( 'wmc_get_enable_currency_by_country', array(
						&$this,
						'_wmc_disable_currency_by_country'
					), 9999 );
				}
				...
				if ( ! isset( $settings ) ) {
					$this->_storedebug( "WOO-MULTI-CURRRENCY settings not loaded" );
				} else {
					$selected_currencies = $settings->get_list_currencies();

					$current_currency = ! empty( $this->_wpwoofeed_settings['feed_use_currency'] ) ? wp_unslash( $this->_wpwoofeed_settings['feed_use_currency'] ) : get_woocommerce_currency();
			...
			} elseif ( WoocommerceWpwoofCommon::isActivatedAeliaCS() ) {
				if ( isset ( $this->_wpwoofeed_settings['feed_use_currency'] ) && ! empty ( $this->_wpwoofeed_settings['feed_use_currency'] ) ) {
					$current_currency = $this->_wpwoofeed_settings['feed_use_currency'];
				} else {
					$current_currency = WC_Aelia_CurrencySwitcher::settings()->base_currency();
				}
			} else if ( WoocommerceWpwoofCommon::is_active_alg_wc_cpp() && isset( $this->_wpwoofeed_settings['feed_use_currency'] ) and ! empty( $this->_wpwoofeed_settings['feed_use_currency'] ) ) {
				$current_currency = $this->_wpwoofeed_settings['feed_use_currency'];
			}
			...
			$this->_wpwoofeed_settings = array_merge( array(
				...
				'feed_on_backorders'     => 'outofstock',
				'feed_backorders_allow'  => 'instock',
				'feed_backorders_notify' => 'outofstock'
			), $this->_wpwoofeed_settings );

			$remove_variations  = ! empty( $this->_wpwoofeed_settings['feed_remove_variations'] )
			                      || ( $this->is_feed_localized()
			                           && $this->_wpwoofeed_settings['main_feed_lang'] != $this->_wpwoofeed_settings['feed_use_lang'] );
			$feedStatus['file'] = $filepath;

			$fields = array();

			/* Filter fields only for a feed type */
			$this->_storedebug( "\n------------------------------------FIELDS-----------------------------------------------------\n" );
			...
			if ( isset( $this->_wpwoofeed_settings['extra'] ) ) {
				foreach ( $this->_wpwoofeed_settings['extra'] as $key => $value ) {
					...
					if ( ! isset( $fields[ $key ] ) || empty( $fields[ $key ]['value'] ) ) {  // so that gtin mpn from dashboard don`t replace feed settings
						if ( $key == 'mpn' && $this->_wpwoofeed_settings['feed_type'] == 'googleReviews' ) {
							$fields[ $key ] = ''; // don't use global settings for MPN in googleReviews feed
						} else {
							$fields[ $key ] = $value;
						}
			...
			if ( $this->_wpwoofeed_settings['field_mapping']['id']['value'] != 'id'
			     || ! empty( $this->_wpwoofeed_settings['field_mapping']['id_prefix'] )
			     || ! empty( $this->_wpwoofeed_settings['field_mapping']['id_prefix'] ) ) {
				$this->_storedebug( "remove auto_pricing_min_price field, because wrong ID" );
				unset( $this->_aCurrentlyFields['auto_pricing_min_price'] );
				unset( $fields['auto_pricing_min_price'] );
			}
			$this->_storedebug( "\n------------------------------------COMPILED FIELDS-----------------------------------------------------\n" );
			...
			$this->_store_info->currency_list    = ( is_object( $this->_woocommerce_wpml ) && is_object( $this->_woocommerce_wpml->multi_currency ) && method_exists( $this->_woocommerce_wpml->multi_currency, 'get_currencies' ) ) ? $this->_woocommerce_wpml->multi_currency->get_currencies( 'include_default = true' ) : null;
			...
			if ( WoocommerceWpwoofCommon::isActivatedWPML() && $this->_store_info->currency
			...
			/* get products caregories and tags*/
			/* ==================================================================================================== */
					...
					case 'true':/* Include */
						if ( $prices_include_tax == "yes" ) {
							if ( $tax_based_on == 'shipping' || $tax_based_on == 'billing' ) {
								...
								$this->_wpwoofeed_settings["tax_rate_tmp_data"] = "target:" . $this->_aCurrentlyFields['tax_countries']['value'];
							} else { /* base */
							...
							$tax_countries = isset( $fields['tax_countries']['value'] ) ? (string) $fields['tax_countries']['value'] : '';
							if ( $tax_based_on == 'shipping' || $tax_based_on == 'billing' ) {
								//Если установлено "Customer shipping/billing address"
								//- то прибавляем к цене величину налога TargetCountry - и выводим в фид.
								$this->_wpwoofeed_settings["tax_rate_tmp_data"] = "addtarget:" . $tax_countries;
							} else {
			...
			$this->_storedebug( "tax_rate_tmp_data: " . $this->_wpwoofeed_settings["tax_rate_tmp_data"] );
			...
			if ( empty( $feedStatus['parsed_products'] ) || $feedStatus['parsed_products'] === - 1 ) {
				if ( ! empty( $this->_wpwoofeed_settings['add_metadata_to_feed'] ) && $this->_wpwoofeed_settings['add_metadata_to_feed'] == 'namespace' ) {
					$custom_namespace_init = " xmlns:pcf=\"https://www.pixelyoursite.com/plugins/product-catalog-feed-for-woocommerce\"";
				} else {
					$custom_namespace_init = '';
				}
				$header = "<?xml version=\"1.0\" encoding=\"" . $this->_store_info->charset . "\" ?>\n";
				$header .= "<rss xmlns:g=\"http://base.google.com/ns/1.0\"" . $custom_namespace_init . " version=\"2.0\">\n";
				$header .= "  <channel>\n";
				...
				if ( $this->_wpwoofeed_settings['feed_type'] == 'facebook' ) {
					$header .= "    <metadata><ref_application_id>451257172939091</ref_application_id></metadata>\n";
				}

				if ( ! empty( $this->_wpwoofeed_settings['add_metadata_to_feed'] ) ) {

					$metadata_fields = array(
						'generation_date' => current_time('mysql'),
						'generator_version' => WPWOOF_SL_ITEM_NAME . ' v' . WPWOOF_VERSION,
					);

					if ( ! empty( $this->_wpwoofeed_settings['feed_comment'] ) ) {
						$metadata_fields['comment'] = $this->_wpwoofeed_settings['feed_comment'];
					}

					$metadata_fields = apply_filters( 'wpwoofeed_metadata_fields', $metadata_fields, $this->_wpwoofeed_settings );

					if ( $this->_wpwoofeed_settings['add_metadata_to_feed'] == 'namespace' ) {
						$header .= "    <pcf:metadata>\n";
						foreach ( $metadata_fields as $field_name => $field_value ) {
							$header .= "      <pcf:" . esc_html( $field_name ) . ">" . esc_html( $field_value ) . "</pcf:" . esc_html( $field_name ) . ">\n";
						}
						$header .= "    </pcf:metadata>\n";
					} elseif ( $this->_wpwoofeed_settings['add_metadata_to_feed'] == 'comment' ) {
						$header .= "    <!-- metadata:\n";
						foreach ( $metadata_fields as $field_name => $field_value ) {
							$header .= "    " . esc_html( $field_name ) . "=\"" . esc_attr( $field_value ) . "\"\n";
						}
						$header .= "    -->\n";
					}
				}
				...
				if ( ! empty( $this->_wpwoofeed_settings['feed_category'] ) ) {
					foreach ( $this->_wpwoofeed_settings['feed_category'] as $category_id ) {
				...
				if ( empty( $this->_wpwoofeed_settings['feed_category_all'] ) && count( $wpwoof_term ) > 0 ) {
				...
				if ( $this->_wpwoofeed_settings['feed_filter_stock'] == 'instock' || $this->_wpwoofeed_settings['feed_filter_stock'] == 'outofstock' ) {
					...
					/**
					 * #3400 Out of stock + allow backorders
					 * Include backorders products to InStock filter
					 * @link https://projects.inlisu.com/issues/3400
					 *
					 * #3590 Out of stock + allow backorders
					 * allow backorders logic
					 * @link https://projects.inlisu.com/issues/3590
					 */
					$meta_query[] = array(
						'key'      => '_stock_status',
						'value'    => 'onbackorder',
				...
				if ( isset( $this->_wpwoofeed_settings['feed_filter_visibility'] ) ) {
					if ( $this->_wpwoofeed_settings['feed_filter_visibility'] == 'all' ) {
						$this->_visibility = array( "publish", "private" );
					} elseif ( $this->_wpwoofeed_settings['feed_filter_visibility'] == 'private' ) {
						$this->_visibility = array( "private" );
					}

				}

				$args['post_type']      = 'product';
				$args['post_status']    = $this->_visibility;
				$args['fields']         = 'ids';
				$args['order']          = 'DESC';
				$args['orderby']        = 'ID';
				$args['posts_per_page'] = - 1;//110;
				if ( WoocommerceWpwoofCommon::isActivatedElasticPress() ) { #3529 get products from DB, not from ElasticSearch
					$args['ep_integrate'] = false;
				}
				if ( isset( $this->_wpwoofeed_settings['feed_filter_visibility'] ) && $this->_wpwoofeed_settings['feed_filter_visibility'] == 'password' ) {
					$args['has_password'] = true;
				}


				if ( WoocommerceWpwoofCommon::isActivatedWPML() ) {
					add_filter( 'wpml_should_use_display_as_translated_snippet', array(
						$this,
						'disable_display_translated_products'
					), 100, 2 );
					...
					if ( $this->_wpwoofeed_settings['feed_use_lang'] == 'all' ) {
						...
						if ( $this->is_feed_localized() ) {
							// for 'fb_localize' && 'fb_country' feeds get products in the language of the main product.
							do_action( 'wpml_switch_language', $this->_wpwoofeed_settings['main_feed_lang'] );
						}
					}
				}
				...
				$products = $this->get_products_with_wpml_fallback( $args );
				unset( $args );

				if ( WoocommerceWpwoofCommon::isActivatedWPML() ) {
					remove_filter( 'wpml_should_use_display_as_translated_snippet', array(
						$this,
						'disable_display_translated_products'
					), 100 );
					if ( $this->is_feed_localized() ) {
				...
				$products = $feedStatus['products_left'];
			}
...
//                    $fields['item_group_id']['define'] = null;
				...
				if ( $this->is_feed_localized() ) {
					$localized_product_id = apply_filters( 'wpml_object_id', $post, 'product', false, $this->_localized_feed_settings['feed_use_lang'] );
					if ( $localized_product_id ) {
						$this->_storedebug( "Localize product: " . $post . " => " . $localized_product_id );
						$this->_localized_main_product_id = $post;
						$post                             = $localized_product_id;
					} else {
						$this->_storedebug( "No Localize product for " . $post );
						continue;
					}
				...
				if ( empty( $product ) ) {
					if ( $key = array_search( $post, $feedStatus['products_left'] ) !== false ) {
						...
						$this->_woocommerce_wpwoof_common->upadte_feed_status( $this->_feed_id, $feedStatus );
					}
					$this->_storedebug( "PRODUCT: " . $post . " not loaded. Skipped." );
					continue;
				}

				if ( $feed_filter_stock == 'instock' && $this->_wpwoofeed_availability( $product ) === 'out of stock' ) {
					$this->_storedebug( "outofstock, skiped" );
					continue;
				}
				if ( $feed_filter_stock == 'outofstock' && $this->_wpwoofeed_availability( $product ) === 'in stock' ) {
					$this->_storedebug( "instock, skiped" );
					continue;
				}
				if ( $this->_wpwoofeed_settings['feed_filter_sale'] == 'sale' && ! $product->is_on_sale() && ! $product->has_child() ) {
					$this->_storedebug( "PRODUCT:" . $this->_get_id( $product ) . " Excluded by 'feed_filter_sale' == 'sale' filter" );
					continue;
				} elseif ( $this->_wpwoofeed_settings['feed_filter_sale'] == 'notsale' && $product->is_on_sale() && ! $product->has_child() ) {
					$this->_storedebug( "PRODUCT:" . $this->_get_id( $product ) . " Excluded by 'feed_filter_sale' == 'notsale' filter" );
					continue;
				}
				...
				if ( ( $this->_wpwoofeed_settings['tmp_storage'] === 'db' && get_post_meta( $post, "wpwoof_ticket_" . $this->_feed_id ) )
				     || ( $this->_wpwoofeed_settings['tmp_storage'] === 'disk' && file_exists( $this->_ticketDir . "/" . $this->_get_id( $product ) . ".item" ) ) ) {
					if ( ( $key = array_search( $post, $feedStatus['products_left'] ) ) !== false ) {
						unset( $feedStatus['products_left'][ $key ] );
					}
					$feedStatus['parsed_products'] ++;
					$this->_storedebug( "PRODUCT item already exist" );
					continue;
				}
				...
				if ( ! isset( $this->_wpwoofeed_settings["feed_include_excluded"] ) || ! $this->_wpwoofeed_settings["feed_include_excluded"] ) {
					...
					if ( ! empty( $_var ) ) {
						$this->_storedebug( "-------------------------------- PRODUCT " . $this->_get_id( $product ) . " Excluded ----------------------------" );
				...
				if ( in_array( $this->_wpwoofeed_settings['feed_type'], $this->_woocommerce_wpwoof_common->product_fields['item_group_id']['feed_type'] ) ) {
				...
				$children_count           = 0;
				$children_output          = 0;
				$exclude_parent_and_child = false;
				if ( $product->has_child() ) {
					...
					if ( $this->_wpwoofeed_settings['feed_filter_sale'] == 'sale' ) {
						$exclude_parent_and_child = true;
					}

					foreach ( $children as $child ) {
						$child = $this->_get_child( $product, $child );
						if ( ! $child || 'publish' !== get_post_status( $child->get_id() ) ) {
							continue;
						}
						if ( $this->_wpwoofeed_settings['feed_filter_sale'] == 'sale' && $child->is_on_sale() ) { #3330
							$this->_storedebug( "PRODUCT:" . $this->_get_id( $product ) . " (child: " . $child->get_id() . ") Included by 'feed_filter_sale' == 'sale' filter" );
							$exclude_parent_and_child = false;
						} else if ( $this->_wpwoofeed_settings['feed_filter_sale'] == 'notsale' && $child->is_on_sale() ) { #3330
							$this->_storedebug( "PRODUCT:" . $this->_get_id( $product ) . " (child: " . $child->get_id() . ") Excluded by 'feed_filter_sale' == 'notsale' filter" );
							$exclude_parent_and_child = true;
							break;
						}
						...
						if ( $feed_filter_stock == 'instock' && $this->_wpwoofeed_availability( $child ) === 'out of stock' ) {
							continue;
						}
						if ( $feed_filter_stock == 'outofstock' && $this->_wpwoofeed_availability( $child ) === 'in stock' ) {
							continue;
						}
						...
						$children_output ++;
						unset( $this->_wpwoofeed_settings["product_tmp_data"]['dont_show_sale_price'] );
						...
						if ( ( ! $prices->regular_price || $is_parent )
							...
							$is_parent                                                   = false;
							$this->_storedebug( "CHILD PARENT PRICES:" . print_r( $parent_prices, true ) );
					...
					if ( ! $remove_variations && $children_output > 0 && $children_count > 0 && ! $exclude_parent_and_child ) {
						foreach ( $children as $child ) {
							$child = $this->_get_child( $product, $child );
							if ( ! $child ) {
								continue;
							}
							if ( 'publish' !== get_post_status( $child->get_id() ) ) {
								$this->_storedebug( "PRODUCT:" . $this->_get_id( $product ) . " (child: " . $child->get_id() . ") Excluded by not active" );
								continue;
							}
							if ( ( $this->_wpwoofeed_settings['tmp_storage'] === 'db' && get_post_meta( $post, "wpwoof_ticket_" . $this->_feed_id ) )
							     || ( $this->_wpwoofeed_settings['tmp_storage'] === 'disk' && file_exists( $this->_ticketDir . "/" . $this->_get_id( $product ) . ".item" ) ) ) {
								$feedStatus['parsed_products'] ++;
								$this->_storedebug( "variation item already exist" );
								continue;
							}
							if ( ! isset( $this->_wpwoofeed_settings["feed_include_excluded"] ) || ! $this->_wpwoofeed_settings["feed_include_excluded"] ) {
...
//                                    $this->_storedebug($this->_wpwoofeed_settings["product_tmp_data"]['prices']);
							...
							// $aExistsProds[] =$this->_get_id($child);
							unset( $this->_wpwoofeed_settings["product_tmp_data"]['cost_of_goods_sold'] );
							$this->_woocommerce_wpwoof_common->upadte_feed_status( $this->_feed_id, $feedStatus );
					...
					if ( $product->is_type( 'bundle' ) && ( ! isset( $parent_prices->is_bundle_price ) || ! $parent_prices->is_bundle_price ) && method_exists( $product, 'get_bundle_price' ) ) {
...
//                    $this->_storedebug("feed_filter_price_bigger".$parent_prices . " => " . print_r($parent_prices, true),true );
				if (
					...
					&& (
						...
						&& ( isset( $this->_wpwoofeed_settings['feed_variation_show_main'] ) && ! empty( $this->_wpwoofeed_settings['feed_variation_show_main'] ) )
					)
			...
			if ( isset( $this->_wpwoofeed_settings["product_tmp_data"] ) ) {
				unset( $this->_wpwoofeed_settings["product_tmp_data"] );
			}
			if ( isset( $this->_wpwoofeed_settings["tax_rate_tmp_data"] ) ) {
				unset( $this->_wpwoofeed_settings["tax_rate_tmp_data"] );
			}
			if ( isset( $this->_wpwoofeed_settings['aBaseTax_tem_val'] ) ) {
				unset( $this->_wpwoofeed_settings['aBaseTax_tem_val'] );
			}
			if ( isset( $this->_wpwoofeed_settings['atax_tem_val'] ) ) {
				unset( $this->_wpwoofeed_settings['atax_tem_val'] );
			}

			$time_start_compile = microtime( true );
			if ( $this->_wpwoofeed_settings['feed_type'] == 'googleReviews' ) {
				$this->compileFeedGoogleReviews();
			} else {
				if ( is_array( $products ) && count( $products ) ) { // count products in feed
					if ( $this->_wpwoofeed_settings['tmp_storage'] === 'db' ) {
						global $wpdb;
						$total_files = $wpdb->get_var( "SELECT COUNT(*) FROM $wpdb->postmeta WHERE meta_key = 'wpwoof_ticket_" . $this->_feed_id . "'" );
					} else {
						$total_files = count( glob( $this->_ticketDir . '/*' ) );
						$total_files -= file_exists( $this->_ticketDir . "/_field_rules.item" ) ? 4 : 3;
					}
					$this->_feed_settings_for_save['total_products'] = $total_files < 1 ? 0 : $total_files;
					wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );
				}
				$this->compileFeed();
			}

			$this->_storedebug( 'compiled time: ' . ( microtime( true ) - $time_start_compile ) );

			$this->_feed_settings_for_save['status_feed']    = 'finished';
			$this->_feed_settings_for_save['generated_time'] = time();
			wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );


			$this->_woocommerce_wpwoof_common->delete_feed_status( $this->_feed_id );
			wpwoof_product_catalog::schedule_feed( $this->_feed_settings_for_save );

			if ( WoocommerceWpwoofCommon::isActivatedWPML() ) {
				do_action( 'wpml_switch_language', $general_lang );
			}

			if ( WoocommerceWpwoofCommon::isActivatedWMCL() ) {
				remove_filter( 'wmc_get_enable_currency_by_country', array(
					&$this,
					'_wmc_disable_currency_by_country'
				), 9999 );
			}
			...
			$this->_feed_settings_for_save['status_feed'] = 'error: ' . $t->getMessage() . ' (' . $t->getFile() . ':' . $t->getLine() . ')';

			wpwoof_update_feed( $this->_feed_settings_for_save, $this->_feed_id, true );

			$stack_trace_lines = explode( "\n", $t->__toString() );
			$short_stack_trace = implode( "\n", array_slice( $stack_trace_lines, 0, 5 ) );

			$this->_storedebug( "Throwable ERROR:" . $t->getMessage() );
			$this->_storedebug( $short_stack_trace );

			WPWOOF_Logger::instance()->log(
				sprintf( 'Feed generation error for feed ID %d: %s', $this->_feed_id, $short_stack_trace ),
				WPWOOF_Logger::LOG_TYPE_FEED,
				$this->_feed_id,
				WPWOOF_Logger::LOG_LEVEL_ERROR
			);
			if ( WPWOOF_DEBUG ) {
				exit( "ERROR" );
			}
		...
		if ( WPWOOF_DEBUG ) {
			$this->_storedebug( 'All SQL requests(' . $this->_wpdb->num_queries . '):' );
//                $this->_storedebug(print_r($this->_wpdb->queries,true));
			$this->_storedebug( 'GENERATED' );
			file_put_contents( $this->_woocommerce_wpwoof_common->feedBaseDir . 'cron-wpfeed.log', date( "Y-m-d H:i:s" ) . "\tGENERATED\t" . $this->_feed_id . "\n", FILE_APPEND );
			//copy($this->_wpwoofeed_wpml_debug,$this->_wpwoofeed_wpml_debug . '-'.date("Ymd-His").'.log');
		}


		WPWOOF_Logger::instance()->log(
			sprintf( 'Feed generation completed for "%s" (ID: %d). Products: %d',
				$this->_feed_name,
				$this->_feed_id,
				$this->_feed_settings_for_save['total_products']
			),
			WPWOOF_Logger::LOG_TYPE_FEED,
			$this->_feed_id,
		...
		WPWOOF_Logger::instance()->clear_feed_generation_logs( $this->_feed_id );
		do_action( 'wpwoof_feed_generated', array(
			'id'   => $this->_feed_id,
			'name' => $this->_feed_name,
			'type' => $this->_wpwoofeed_settings['feed_type'],
			'url'  => $upload_dir['url'],
	...
	private function _build_item( $fields, $product ) {
		...
		if ( empty( $this->_wpwoofeed_settings['feed_type'] ) ) {
			return '';
		}
		...
		if ( in_array( $product_type, $this->_aVariationsType ) ) {
			...
			if ( ! empty( $mainMeta ) ) {
				...
				foreach ( $mainMeta as $extraKey => $extraValue ) {
					if ( isset( $extraValue['custom_tag_name'] ) and ! isset( $extraValue['feed_type'][ $this->_wpwoofeed_settings['feed_type'] ] ) ) {
						continue;
					}
					if ( ! isset( $extraValue['custom_tag_name'] ) && ( ! isset( $this->_woocommerce_wpwoof_common->product_fields[ $extraKey ] ) || ! in_array( $this->_wpwoofeed_settings['feed_type'], $this->_woocommerce_wpwoof_common->product_fields[ $extraKey ]['feed_type'] ) ) ) {
						continue;
					}
					$nkey                 = isset( $extraValue['custom_tag_name'] ) ? $extraValue['custom_tag_name'] : $extraKey;
					$extraFields[ $nkey ] = $extraValue['value'];
					if ( isset( $extraValue['type'] ) && $extraValue['type'] == 'editor' ) {
						$flagEditor[] = $nkey;
					}
				}
			}
			...
			foreach ( $extraFields as $extraKey => $extraValue ) {
				if ( $extraValue !== "" ) {
					...
					if ( ! isset( $this->_field_rules[ $extraKey ] ) ) {
						...
						if ( in_array( $extraKey, $flagEditor ) ) {
							$this->_field_rules[ $extraKey ]['CDATA']        = true;
							$this->_field_rules[ $extraKey ]['NotStripTags'] = true;
						}
						$this->storeTicket( '_field_rules', json_encode( $this->_field_rules ) );
					}
		...
		$additional_keys_for_csv = array();

		foreach ( $fields as $element => $field ) {

			$this->_storedebug( "--------------------------------------\n" . $element . "=>" . print_r( $field, true ) );
			...
			} elseif ( ! empty( $this->_woocommerce_wpwoof_common->product_fields[ $element ]['product_type_exclude'] )
			           && in_array( $product_type, $this->_woocommerce_wpwoof_common->product_fields[ $element ]['product_type_exclude'] ) ) {
				$this->_storedebug( "product_type_exclude: " . $this->_wpwoofeed_settings['feed_type'] );
				continue;
			}

			$_custom_user_function_checked = false;
			if ( ! $custom_tag ) {
				/*  the element has tag */
				if ( ! isset( $this->_woocommerce_wpwoof_common->product_fields[ $element ][ $this->_wpwoofeed_type ] ) ) {
					continue;
				}
				...
				if ( method_exists( $this, $func ) ) {
					$tagvalue = call_user_func_array( array( $this, $func ), array(
						$product,
						$item,
						$field,
						$element
					...
					$tagvalue                      = $this->_custom_user_function( $product, $item, $field, $element );
					$_custom_user_function_checked = true;
				}
				...
				if ( is_array( $tagvalue ) && ! empty( $this->_woocommerce_wpwoof_common->product_fields[ $element ]['csv'] )
				     && is_array( $this->_woocommerce_wpwoof_common->product_fields[ $element ]['csv'] ) ) {
					$composite_value = $this->get_composite_field_key_value_for_csv( $tagvalue, $element );
					if ( ! empty( $composite_value ) ) {
						$this->_storedebug( ' composite_value ' );
						$this->_storedebug( array_keys( $composite_value ) );
						$additional_keys_for_csv = array_keys( $composite_value );
					}
				}
			...
			// global extra fields
			if ( ! $custom_tag && isset( $this->_wpwoofeed_settings['extra'][ $element ] ) && ( $tagvalue === "" || $tagvalue === false )
			     && ! $_custom_user_function_checked && $element != 'identifier_exists'
			     && in_array( $this->_wpwoofeed_settings['feed_type'], $this->_woocommerce_wpwoof_common->product_fields[ $element ]['feed_type'] ) ) {
				$tagvalue = $this->_custom_user_function( $product, $item, $field, $element );
				if ( $tagvalue != "" ) {
					$this->_storedebug( "Global!" );
				}
			} elseif ( ! $custom_tag && ( $tagvalue === "" || $tagvalue === false )
			           && ! $_custom_user_function_checked
			           && isset( $this->_wpwoofeed_settings['extra'][ $element ] ) ) { // #3768 custom_tag from an extra field list.
				$tagvalue = $this->_custom_user_function( $product, $item, $field, $element );
				$this->_storedebug( "Global! from extra" );
			}
			$this->_storedebug( "TAG:" . print_r( $element, true ) . "=='" . print_r( $tagvalue, true ) . "'" );
			if ( $tagvalue != "" || ( $element == 'inventory' && $tagvalue === 0 ) ) {
				$filtered_tag_value = apply_filters( 'pcf_feed_product_field_filter', $tagvalue, $element, $product_id );
				if ( $tagvalue != $filtered_tag_value ) {
					$this->_storedebug( "pcf_feed_product_field_filter, field: " . $element . "value changed:" . print_r( $tagvalue, true ) . ' => ' . print_r( $filtered_tag_value, true ) );
					$tagvalue = $filtered_tag_value;
				}
		...
		if ( count( $aItem ) ) {
			$this->storeTicket( $product_id, $aItem );
			$this->checkAndAddCsvColumn( array_merge( array_keys( $aItem ), $additional_keys_for_csv ) );
	...
	private function set_config_for_localized_feed( $wpwoofeed_settings ) {

		$this->_localized_feed_settings              = $wpwoofeed_settings;
		$this->_wpwoofeed_settings['main_feed_lang'] = $this->_wpwoofeed_settings['feed_use_lang'];
		$params_for_merge                            = array(
			'feed_type',
			'feed_use_currency',
			'feed_use_lang',
			'feed_variable_price',
		...
		foreach ( $params_for_merge as $param ) {
			if ( isset( $this->_localized_feed_settings[ $param ] ) ) {
				$this->_wpwoofeed_settings[ $param ] = $this->_localized_feed_settings[ $param ];
			} elseif ( isset( $this->_wpwoofeed_settings[ $param ] ) ) {
				unset( $this->_wpwoofeed_settings[ $param ] );
			}
		}
		$params_field_mapping_for_merge = array( 'tax', 'tax_countries', 'remove_currency', 'expand_more_images' );
		foreach ( $params_field_mapping_for_merge as $param ) {
			if ( isset( $this->_localized_feed_settings['field_mapping'][ $param ] ) ) {
				$this->_wpwoofeed_settings['field_mapping'][ $param ] = $this->_localized_feed_settings['field_mapping'][ $param ];
			} elseif ( isset( $this->_wpwoofeed_settings['field_mapping'][ $param ] ) ) {
				unset( $this->_wpwoofeed_settings['field_mapping'][ $param ] );
			}
		}
	...
	/**
	 * Processes the composite field key-value pairs for CSV export.
	 *
	 * @param mixed $tag_value The value of the tag, usually an array.
	 * @param string $element The element name used to access the relevant CSV configuration.
	 *
	 * @return array The processed key-value pairs ready for CSV export.
	 */
	private function get_composite_field_key_value_for_csv( $tag_value, $element ): array {
		$output = array();

		if ( is_array( $tag_value ) && is_array( $this->_woocommerce_wpwoof_common->product_fields[ $element ]['csv'] ) ) {
			foreach ( $this->_woocommerce_wpwoof_common->product_fields[ $element ]['csv'] as $parent_field_name => $sub_field_names ) {
				foreach ( $tag_value as $index1 => $fiedls_array ) {  // ['url' => 'http...', tag => ['red', green] ]
					foreach ( $fiedls_array as $sub_field_name => $sub_field_value ) {
						if ( in_array( $sub_field_name, $sub_field_names ) ) {
							if ( is_array( $sub_field_value ) ) {
								foreach ( $sub_field_value as $index2 => $sub_field_value2 ) {
									$full_sub_field_name            = $parent_field_name . '[' . $index1 . '].' . $sub_field_name . '[' . $index2 . ']';
									$output[ $full_sub_field_name ] = $sub_field_value2;
								}
							} else {
								$full_sub_field_name            = $parent_field_name . '[' . $index1 . '].' . $sub_field_name;
								$output[ $full_sub_field_name ] = $sub_field_value;
							}
						}
					}
				}
		...
		$this->_storedebug( $output );

		return $output;
	}

	/**
	 * Checks if the feed configuration is localized.
	 *
	 * This method determines whether a given feed configuration is localized based on
	 * specific criteria. If no feed configuration is provided, it attempts to use
	 * the localized feed settings.
	 *
	 * @param array|null $feed_config The feed configuration to check. If null, the method will use the
	 *                                internal localized feed settings.
	 *
	 * @return bool True if the feed is localized, false otherwise.
	 */
	private function is_feed_localized( array $feed_config = null ): bool {
		if ( empty( $feed_config ) && ! empty( $this->_localized_feed_settings ) ) {
			$feed_config = $this->_localized_feed_settings;
		}
		if ( ! empty( $feed_config ) && in_array( $feed_config['feed_type'], array(
				'fb_localize',
				'fb_country',
	...
	/**
	 * Filters out specific fields from the current feed configuration.
	 *
	 * This method excludes certain fields from the feed configuration based on
	 * the settings defined for the feed type 'fb_country'. It modifies the current
	 * fields by removing specified fields that are to be excluded.
	 *
	 * @return void
	 */
	private function feed_fields_filter() {
		if ( $this->_wpwoofeed_settings['feed_type'] == 'fb_country' && ! empty( $this->_localized_feed_settings['fb_country_exclude_fields'] ) ) {
			$fields_for_exclude = array(
				'color',
				'material',
				'pattern',
				'custom_label_0',
			...
			foreach ( $fields_for_exclude as $field_name ) {
				unset( $this->_aCurrentlyFields[ $field_name ] );
			}
		}
	}
	...
	/**
	 * Disables the display of translated products when the feed is generating
	 *
	 * @param bool $should_use Indicates the initial state of whether translated products should be displayed.
	 * @param array|null $post_type An array of post types to check. If null or empty, the method will return the original state.
	 *
	 * @return bool False if the post type includes 'product', otherwise returns the original state provided in $should_use.
	 */
	function disable_display_translated_products( $should_use, $post_type = null ) {
		// Early return if post type is not provided
		if ( empty( $post_type ) ) {
			return $should_use;
		}

		// Check if the product exists in post types array keys
		if ( array_key_exists( 'product', $post_type ) ) {
			return false;
		}
	...
	/**
	 * Retrieves the minimum or maximum percentage-based auto_pricing_min_price value from product categories metadata.
	 *
	 * @param mixed $product The product object or ID for which the minimum or maximum auto pricing value is to be determined.
	 *
	 * @return float|string The calculated minimum or maximum percentage value based on the categories' metadata, or an empty string if no valid value is found.
	 */
	private function _get_percent_auto_pricing_min_price_from_categories( $product ) {
		$prId = $this->_getParentID( $product );
		if ( ! $prId ) {
			$prId = $this->_get_id( $product );
		}

		$logic = ! empty( $this->_wpwoofeed_settings['auto_pricing_min_price_categories_logic'] ) ? $this->_wpwoofeed_settings['auto_pricing_min_price_categories_logic'] : 'max';  //'max' or 'min'
		$terms = get_the_terms( $prId, 'product_cat' );

		$values = array(); // Array to store valid numeric meta values

		if ( is_array( $terms ) ) {
			foreach ( $terms as $term ) {
				if ( $term instanceof WP_Term ) { // Ensure $term is a valid WP_Term object
					$meta_value = get_term_meta( $term->term_id, 'wpfoof-auto_pricing_min_price', true );

					// Check if meta_value is not empty and is a numeric value
					if ( ! empty( $meta_value ) && is_numeric( $meta_value ) ) {
						$values[] = floatval( $meta_value ); // Add as float to ensure a correct comparison
					}
				}
			}
		}

		$result = ""; // Default result if no valid values are found or no terms
	...
	/**
	 * Gets products with WPML fallback, correctly handling tax_query and meta_query.
	 * This version uses the correct language switching method and is robust for multi-language sites.
	 * NOTE: Requires a modern WPML version (4.0+) with all core modules active.
	 *
	 * @param array $args The original WP-Query arguments.
	 * @return WP_Query The final WP_Query object containing the correct products.
	 */
	public function get_products_with_wpml_fallback( $args ) {
		// --- Preliminary Checks ---
		if ( ! isset( $this->_wpwoofeed_settings['translatable_fallback'] ) || empty( $this->_wpwoofeed_settings['translatable_fallback'] ) ) {
		...
		$is_fallback_enabled = isset( $wpml_settings['custom_posts_sync_option']['product'] ) && $wpml_settings['custom_posts_sync_option']['product'] == 2;
		if ( ! $is_fallback_enabled ) {
			return new WP_Query( $args );
		}

		$this->_storedebug( 'wpml_fallback_enabled = true;' );
		$this->wpml_fallback_enabled = true;

		$original_fields = isset( $args['fields'] ) ? $args['fields'] : 'all';
		$original_post_type = isset( $args['post_type'] ) ? $args['post_type'] : 'product';

		// Step A: Get IDs of products that ARE translated in the current language.
		$args_current = $args;
		$args_current['lang'] = $current_language;
		$args_current['fields'] = 'ids';
		$translated_products_query = new WP_Query( $args_current );
		$translated_ids = $translated_products_query->posts;

		// Step B: Get candidate IDs from the DEFAULT language.
		// We temporarily switch the global language context so WPML handles tax_query translation automatically.
		$sitepress->switch_lang( $default_language );

		$args_default = $args;
		$args_default['fields'] = 'ids';

		$default_products_query = new WP_Query( $args_default );
		$default_ids = $default_products_query->posts;

		// CRUCIAL: Switch back to the original language immediately.
		$sitepress->switch_lang( $current_language );

		// Step C: Filter default language IDs to find only those without a translation in the current language.
		$fallback_ids = [];
		if ( ! empty( $default_ids ) ) {
			// 1. Create a "blacklist" of translation groups (trids) that already have a translation.
			$trids_of_translated_products = [];
			if ( ! empty( $translated_ids ) ) {
				$translated_ids_str = implode( ',', array_map( 'intval', $translated_ids ) );
				$trids_of_translated_products = $wpdb->get_col( "
                    SELECT trid FROM {$wpdb->prefix}icl_translations
                    WHERE element_type = 'post_product' AND element_id IN ({$translated_ids_str})
                " );
			}

			$default_ids_str = implode( ',', array_map( 'intval', $default_ids ) );

			// 2. Select candidates from the default language, but only if their trid is NOT in the blacklist.
			$query_untranslated = $wpdb->prepare("
                SELECT element_id FROM {$wpdb->prefix}icl_translations
                WHERE element_type = 'post_product' AND language_code = %s AND element_id IN ({$default_ids_str})
            ", $default_language);

			if ( ! empty( $trids_of_translated_products ) ) {
				$trids_str = implode( ',', array_map( 'intval', $trids_of_translated_products ) );
				$query_untranslated .= " AND trid NOT IN ({$trids_str})";
			}

			$fallback_ids = $wpdb->get_col( $query_untranslated );
		}

		// Step D: Combine the two sets of IDs.
		$final_ids = array_unique( array_merge( $translated_ids, $fallback_ids ) );

		if ( empty( $final_ids ) ) {
			return new WP_Query( $args );
		}

		// Step E: Perform the final query to fetch the product data for the collected IDs.
		// This query is clean of taxonomy/meta filters to avoid language conflicts.
		$final_args = [
			...
			'orderby'        => 'post__in',
			'order'          => 'ASC',
			'suppress_filters' => true, // Disable language filters for this query.
		];

		// Respect the original ordering if it was set.
		if (isset($args['orderby']) && $args['orderby'] !== 'post__in') {
			$final_args['orderby'] = $args['orderby'];
			$final_args['order'] = isset($args['order']) ? $args['order'] : 'DESC';
		}
