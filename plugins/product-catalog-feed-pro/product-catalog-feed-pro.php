...
require_once plugin_dir_path( __FILE__ ) . 'inc/logger.php';
 ...
 */private $rest;
	...
	static function wpwoof_addfeed_submit() {
		check_ajax_referer( 'wpwoof_settings' );
		$new_feed_setting = $_POST;
		unset( $new_feed_setting['wpwoof_addfeed_submit'] );
		unset( $new_feed_setting['action'] );
		$new_feed_setting['added_time'] = time();
        if ( ! empty( $_POST['edit_feed'] ) ) {
            $feed_id = (int) $_POST['edit_feed'];
            $old_feed_setting = wpwoof_get_feed( $feed_id);
            ...
            if ($old_feed_setting['feed_name'] != $new_feed_setting['feed_name']) {
                $new_feed_setting['feed_name'] = self::make_unique_name ($new_feed_setting['feed_name'] , $existingNames['feed_names'], true);
                wpwoof_update_feed( $new_feed_setting, $feed_id, false, $new_feed_setting['feed_name'] );
            }
            ...
            if ( (!empty($new_feed_setting['feed_file_name']) || $new_feed_setting['feed_file_name'] === 0) && $old_feed_setting['feed_file_name'] != $new_feed_setting['feed_file_name'] ) {
                $new_feed_setting['feed_file_name'] = self::make_unique_name ($new_feed_setting['feed_name'], $existingNames['file_names']);
                wpwoof_delete_feed_file( $feed_id );
            } elseif ( $new_feed_setting['feed_file_name'] == '' ) {
                $new_feed_setting['feed_file_name'] = $old_feed_setting['feed_file_name'];
            }
            $new_feed_setting = self::merge_feed_configs( $old_feed_setting, $new_feed_setting );
            wpwoof_create_feed( $new_feed_setting );
			update_option( 'wpwoof_message', 'Feed Updated Successfully.' );
            WPWOOF_Logger::instance()->log(
                sprintf( 'Feed configuration updated for "%s" (ID: %d)',
                    $new_feed_setting['feed_name'] ?? 'Unknown',
                    $feed_id
                ),
            ...
            $existingNames = self::get_all_existing_feed_name_and_file_name();
            if (in_array($new_feed_setting['feed_name'], $existingNames['feed_names'])) {
                $new_feed_setting['feed_name'] = self::make_unique_name ($new_feed_setting['feed_name'] , $existingNames['feed_names'], true);
            }
            if ( ( empty($new_feed_setting['feed_file_name']) && $new_feed_setting['feed_file_name'] !== 0 ) || in_array( $new_feed_setting['feed_file_name'],$existingNames['file_names']) ) {
                $new_feed_setting['feed_file_name'] = self::make_unique_name ($new_feed_setting['feed_file_name'], $existingNames['file_names']);
            }

			if ( update_option( 'wpwoof_feedlist_' . esc_sql($new_feed_setting['feed_name']), $new_feed_setting ) ) {
				global $wpdb;
				$sql    = "SELECT * FROM $wpdb->options WHERE option_name = 'wpwoof_feedlist_" . esc_sql( $new_feed_setting['feed_name'] ) . "' Limit 1";
				$result = $wpdb->get_results( $sql, 'ARRAY_A' );
				if ( count( $result ) == 1 ) {
					$new_feed_setting['edit_feed'] = $result[0]['option_id'];
					wpwoof_create_feed( $new_feed_setting );
				}
			}
		}
 ...
 */
	...
	static function check_plugin_version_and_migrate( $is_activate = false ) {
        ...
        if (strpos($db_version_original, "pro|") === 0) {
            $is_pro = true;
            $db_version = substr($db_version_original, 4);
        } else {
            $db_version = $db_version_original;
        }

        $log_file = self::$WWC->feedBaseDir . 'migrations.log';
        if ( WPWOOF_DEBUG ) {
            file_put_contents( $log_file, date( "Y-m-d H:i:s" ) . "\tdbMigration '". $db_version_original."' => 'pro|" . WPWOOF_VERSION."'\n", FILE_APPEND );
        }

        require_once plugin_dir_path(__FILE__) . 'inc/migrations.php';
        Wpwoof_Migration_Manager::run($db_version ?: '0.0.0', $is_activate, $log_file);

		if ( $db_version_original !== 'pro|' . WPWOOF_VERSION ) {
			WPWOOF_Logger::instance()->log(
				sprintf( 'Plugin updated from version %s to %s', $db_version_original ?: 'unknown', 'pro|' . WPWOOF_VERSION ),
...
//            $k = str_replace("0-","",$k);
    ...
    private static function extra_field_rendering($key, $value, $isMain, $gm, $loop, $loopStr, $is_repeated) {
        ...
        if ( $isCustomTag ):
            ...
            <div class="extra-link-wrapper">
                ...
                <div class="extra-link__item">
                    <input type="checkbox"
                           id="wpfoof-box-media[extra]<?= $loopStr ?>[<?= $key ?>][feed_type][reddit]"
                           name="wpfoof-box-media[extra]<?= $loopStr ?>[<?= $key ?>][feed_type][reddit]" <?php checked( isset( $value['feed_type']['reddit'] ) ); ?>>
                    <label for="wpfoof-box-media[extra]<?= $loopStr ?>[<?= $key ?>][feed_type][reddit]">Reddit</label>
                </div>
                <br>
                <div class="extra-link__item">
	...
	static function schedule_feed( $feed_config, $regenerateTime = false ) {
        ...
        $feed_id = !empty($feed_config['edit_feed'])?(int) $feed_config['edit_feed']:0;
...
//            if(WPWOOF_DEBUG) file_put_contents(self::$WWC->feedBaseDir.'cron-wpfeed.log',date("Y-m-d H:i:s")."\t schedule_feed exclude:".$feed_config['edit_feed']." products_left:".count($status['products_left'])."\n",FILE_APPEND);
		...
		if ( ! $regenerateTime && ( ! empty( $feed_config['noGenAuto'] ) || self::$interval * 1 == 0 ) ) {
            ...
            if (empty($regenerateTime)) {
                WPWOOF_Logger::instance()->log(
                    sprintf( 'Next run for feed "%s" (ID: %d) is %s',
                     $feed_config['feed_name'] ?? 'Unknown',
                     $feed_id,
                      date('D, d M Y H:i:s', $nextRun)
                    ),
                    WPWOOF_Logger::LOG_TYPE_FEED,
                    $feed_id,
                    WPWOOF_Logger::LOG_LEVEL_INFO
