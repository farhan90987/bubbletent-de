...
if ( ! class_exists( 'WOE\PhpOffice\PhpSpreadsheet\Spreadsheet' ) ) {
	if (version_compare(phpversion(), WOE_MIN_PHP_VERSION, '<')) {
		...
		echo esc_html(sprintf(__( 'PhpSpreadsheet requires PHP version %s or later.', 'woocommerce-order-export' ), WOE_MIN_PHP_VERSION));
		die();
	}
	...
	public function finish() {
		...
		if ( $this->mode === 'preview' ) {
			...
			if ( $this->settings['display_column_names'] && count( $this->rows ) < 2 || count( $this->rows ) < 1 ) {
				$this->rows[] = array( '<td colspan=10><b>' . __( 'No results', 'woocommerce-order-export' ) . '</b></td>' );
			}
			$image_preview_multiply = 5;
			$max_columns = 0;
			$summary_row = array();
