...
//		$extra_rows = array();
	...
	public static function get_shipping_methods() {
		...
		foreach ( $methods as $method ) {
			...
			$shipping_methods_zones[ $method->get_rate_id() ] = __( '[Rest of the World]',
					'woocommerce-order-export' ) . ' ' . $method->get_title();
		}
	...
	public static function get_shipping_zone( $order ) {
		...
		$value = __( 'Rest of the World','woocommerce-order-export' );
	...
	public static function get_origin_for_order( $order ) {
		...
		if ( empty( $origin ) ) {
			$origin = __( 'Unknown', 'woocommerce-order-export' );
		}
		...
		switch ( $source_type ) {
			case 'utm':
				$label = $translated ?
					...
					__( 'Source: %s', 'woocommerce-order-export' )
					: 'Source: %s';
				break;
			case 'organic':
				$label = $translated ?
					/* translators: %s is the source value */
					__( 'Organic: %s', 'woocommerce-order-export' )
					: 'Organic: %s';
				break;
			case 'referral':
				$label = $translated ?
					/* translators: %s is the source value */
					__( 'Referral: %s', 'woocommerce-order-export' )
					: 'Referral: %s';
				break;
			case 'typein':
				$label  = '';
				$source = $translated ?
					__( 'Direct', 'woocommerce-order-export' )
					: 'Direct';
				break;
			case 'mobile_app':
				$label  = '';
				$source = $translated ?
					__( 'Mobile app', 'woocommerce-order-export' )
					: 'Mobile app';
				break;
			case 'admin':
				$label  = '';
				$source = $translated ?
					__( 'Web admin', 'woocommerce-order-export' )
					: 'Web admin';
				break;
			...
			default:
				$label  = '';
				$source = $translated ?
					__( 'Unknown', 'woocommerce-order-export' )
					: 'Unknown';
				break;
		}
