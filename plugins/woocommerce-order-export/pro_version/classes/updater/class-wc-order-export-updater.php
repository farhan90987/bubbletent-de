...
class WC_Order_Export_Updater {
	...
	public function show_update_notification( $file, $plugin ) {
		...
		if ( ! empty( $update_cache->response[ $this->name ] ) && version_compare( $this->version,
			...
			if ( empty( $version_info->download_link ) ) {
				printf(
					/* translators: Notification about new version */
	...
	public function show_changelog() {
		...
		if ( empty( $_REQUEST['edd_sl_action'] ) || 'view_plugin_changelog' != $_REQUEST['edd_sl_action'] || empty( $_REQUEST['plugin'] ) || empty( $_REQUEST['slug'] )) {
			return;
		}

		if ( ! current_user_can( 'update_plugins' ) ) {
			wp_die( esc_html__( 'You do not have permission to install plugin updates', 'woocommerce-order-export' ),
				esc_html__( 'Error', 'woocommerce-order-export' ), array( 'response' => 403 ) );
		}
		...
		//phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$cache_key    = md5( 'edd_plugin_' . sanitize_key( wp_unslash($_REQUEST['plugin']) ) . '_version_info' );
		$version_info = get_transient( $cache_key );
